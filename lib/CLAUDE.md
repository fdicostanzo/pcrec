...
The only header file installed for embedding pcrec as a library. Declares the FOUR public functions and the options/output/error structs.

## Files

**[REVW.5] (lens 9's P4, 2026-09-19) adds the FOURTH function,
`pcrec_limits_tsv`, and it is a MOVE rather than a new entry point.** It has
always been compiled in and exported from `libpcrec.a`, and `--list-limits`
has always called it; only its DECLARATION was internal, so a library
consumer could not reach the one table that says what a RAISE-ONLY cap's
built-in default is. Its declaration LEFT `src/core/internal.h` in the same
change — that header includes this one, so every internal caller still sees
it, and two declarations of one contract is a second spelling. The values are
deliberately NOT also emitted here as `#define`s: `src/core/limits.def` is
D90's ruled single home and a macro copy would be free to drift from it.
`docs/spec/limits.md` §3.4a is the contract.
