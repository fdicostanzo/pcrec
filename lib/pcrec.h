 ...
 * 2026-09-22).
 *
 * [b2fix] (2026-09-23, Frank's directive) EVERY MEMBER IS NOW SPELLED
 * `PCREC_BIT(N)`, not a bare `1ull << N` — a respelling, not a value
 * change (`PCREC_BIT(N)` expands to exactly the `1ull << (N)` text above).
 * `PCREC_BIT` is PUBLIC (declared here, not in `src/core/internal.h`)
 * because the enumerators it builds are public. The two checks named
 * above derive their bit tables by grepping for this macro's call
 * shape now, not for the bare shift — see each script's own comment. */
#define PCREC_BIT(n) (1ull << (n))

enum {
    PCREC_CASELESS    = PCREC_BIT(0),  /* was pcrec_options.caseless */
    PCREC_EMIT_MAIN   = PCREC_BIT(1),  /* was pcrec_options.emit_main */
    PCREC_NO_CAPTURES = PCREC_BIT(2),  /* --no-captures (D42.1) */
    ...
    PCREC_NO_POSSESSIFY = PCREC_BIT(4),
    ...
    PCREC_NO_LENGTH_PRUNE = PCREC_BIT(7),
     ...
     * checked against. */
    PCREC_NO_PREFILTER    = PCREC_BIT(8),
    PCREC_FORCE_PREFILTER = PCREC_BIT(9),
    ...
    PCREC_NO_ALTCLS_MERGE  = PCREC_BIT(10),
    PCREC_NO_ALTCLS_FACTOR = PCREC_BIT(11),
     ...
     * exactly that. */
    PCREC_NO_ATOMIC_DISCHARGE = PCREC_BIT(12),
    ...
    PCREC_NO_SPLICE_CALLS = PCREC_BIT(13),
     ...
     * is checked against. */
    PCREC_NO_TIERED_ENTRY = PCREC_BIT(14),
    ...
    PCREC_NO_PREMUL_TABLE = PCREC_BIT(15),
    ...
    PCREC_NO_ANCHORED_DFA = PCREC_BIT(17),
    ...
    PCREC_NO_PREFILTER_COLLAPSE    = PCREC_BIT(19),
    PCREC_FORCE_PREFILTER_COLLAPSE = PCREC_BIT(20),
     ...
     * ask for it. */
    PCREC_NO_STARTPOS_GUARD = PCREC_BIT(25),
    ...
    PCREC_NO_COMMENTS   = PCREC_BIT(26),
    PCREC_FORCE_COMMENTS = PCREC_BIT(27),
...
/* [b2fix] (2026-09-23): `PCREC_NO_REQ_RUN` is a `#define`, NOT an enum
 * member like every bit above it, and that is forced rather than
 * stylistic. `PCREC_BIT(31)` is 2^31, which EXCEEDS `INT_MAX` and so sits
 * outside the range ISO C (before C23) allows an ENUMERATOR to hold —
 * `gcc-16 -Wpedantic` confirms it directly ("ISO C restricts enumerator
 * values to range of 'int' before C23"), a warning `make strict` does not
 * enable (this project's build discipline has no `-Wpedantic` gate
 * anywhere) but a downstream embedder's stricter build might. Pulling
 * this one bit out of the enum also returns the REST of the flags enum to
 * plain `int` range, so bits 0-30 above need no GNU widening extension at
 * all — `make strict CC=gcc-16` (`-Werror -Wshadow`) is clean either way,
 * measured both before and after this split.
 *
 * THE SAME TRAP AWAITS EVERY FUTURE BIT ABOVE 30: any bit from here to 63
 * numerically exceeds `INT_MAX` and must be a `#define`, never an enum
 * member, the day it is added — named here so nobody has to re-derive it
 * from a pedantic build failure. `PCREC_ENGINE_DFA`/`PCREC_ENGINE_VM`
 * below are the tree's existing `#define` precedent, for an unrelated
 * reason (an artifact's own `#define` of the identical name) — this is a
 * second, independent reason to reach the same spelling.
 *
 * [D77] THE SUCCESSOR BEYOND 64 BITS, when the count approaches this
 * word's width, is a word-array bitset behind the same `PCREC_BIT`-family
 * macro shape, not sixty-four individually-numbered `#define`s — that is
 * the trigger to build it, written down rather than guessed at. */
