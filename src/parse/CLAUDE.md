...
- **mod_uprops.c** — module `unicode-props`: the `\p`/`\P` doorway's BODY
  SCANNER and, since **[M5.0] stage 3, its PRODUCER**. `\p{L}` compiles.

  **THE SCANNER IS UNCHANGED IN EVERY BYTE OF ITS ALGORITHM** — the
  malformed(146)-vs-unknown-name(147) split, the 48-significant-character
  budget, the caret rule and every blame offset below are exactly as MOD-0.6
  measured them, and are still what the CLOSED-GATE half of PC-3's
  differential asserts. What the file gained is a table lookup where it used
  to refuse unconditionally, plus `uprops_produce`.

  **45 CATEGORY NAMES AND 171 SCRIPT VALUES SHIP** from `uprops_tables.inc`
  — GENERATED by `third_party/ucd-16.0.0/generate.py` from the vendored UCD at
  Unicode 16.0.0 (`third_party/README.md` for the vendoring shape; Frank's
  ASK 2 ruling, agreed reluctantly, with the usage condition that none of it
  reaches a generated artifact). The categories are the seven one-letter
  codes, all 30 two-letter ones, `L&`/`Lc`, `Any`, and PCRE2's
  `Xan Xps Xsp Xuc Xwd`. Booleans and `Bidi_Class` are declined by design.

  **[M5.0] STAGE 5 ADDED THE SCRIPTS AND A NAMESPACE COLUMN, and the shape is
  a MEASURED semantic fact rather than table weight.** `utf8_design.md` §3.4
  budgets this family as *"one more UCD file, ~160 names; nothing structural,
  purely table weight"*. It is not: MEASURED against 10.42, 10.46 and 10.48
  and STATED by `man pcre2pattern`, **the bare spelling `\p{Greek}` is
  `Script | Script_Extensions` and only `\p{sc=Greek}` is the Script property
  alone** (U+0342 is the discriminating code point — `Script=Inherited`,
  `Script_Extensions={Greek}`). So a script value carries TWO sets, not one,
  and a row now answers in a set of NAMESPACES: bare and `scx=` share the
  extended set, `sc=` gets the strict one. `uprops_namespace` is the one place
  a body's `=`/`:` prefix is interpreted; there is no `gc=` namespace because
  libpcre2 has none.

  Each value answers to its long name, its four-letter code and any deprecated
  alias (`\p{Greek}`, `\p{Grek}`; `\p{Inherited}`, `\p{Zinh}`, `\p{Qaai}`) —
  717 rows over 312 distinct sets. **One UCD `sc` value is DECLINED**:
  `Katakana_Or_Hiragana`/`Hrkt`, which all three reachable libpcre2 versions
  refuse (error 147). That is `\p{Assigned}` in the other direction — a name
  the UCD has and PCRE2 does not — and D26 makes PCRE2 the source of truth for
  whether a construct is real.

  **THE HAND-WRITTEN SHORT-NAME TABLE RETIRED INTO THE GENERATED ONE, AND THE
  RULING THAT MADE IT HAND-WRITTEN IS INTACT.** That ruling forbade generating
  from *libpcre2* — "one source wearing two hats" — and there is now a THIRD
  source, the UCD, so the independence argument is unchanged: PC-3's 52-letter
  name-axis sweep still asks the LIVE ORACLE, about a table it did not
  produce. Keeping a hand copy beside the generated one would have been the
  D24 drift shape at the scale of a `strchr`.

  **THE PORTS STAY `NO_PORT` AND THAT IS NOW A DECISION.** ext.c's port block
  fires only at post-gate `WANT_RESULT`, while this function must also answer
  at VERDICT, must answer for a MALFORMED body at any level, and must answer
  differently at the two POSITIONS — so a port would leave three of its four
  jobs on a second path. `pcrec_modport_uprops` therefore dropped the
  `ExtPortFn` signature and took `in_class` instead, which selects
  `EXT_MEMBERS` over `EXT_NODE` exactly as ext.c's own `PORT_SET` branch does.

  **THE CASELESS RULE IS A SPAN SUBSTITUTION, NEVER `cls_casefold`** (and
  stage 5 left it unamended: scripts are caseless-INVARIANT, measured
  exhaustively over every code point `CaseFolding.txt` names, 171 values x 3
  spellings, zero differences).
  MEASURED against libpcre2 two ways (a 44-name x 12,290-code-point
  differential and a full-space interval comparison, no exception on either):
  under `-i`, `\p{Lu}`, `\p{Ll}` and `\p{Lt}` are EXACTLY `\p{L&}` and
  every other property is caseless-INVARIANT. It is NOT the general
  close-under-case-mapping rule, and the discriminating cell is the one an
  implementer would get wrong: U+0345 is `Mn` with an uppercase mapping to
  U+0399 (`Lu`), and `(?i)\p{L}` does NOT match it. So each table row carries
  its set TWICE and the D23 ASCII fold is never applied — folding `\p{Lu}`
  would give `A-Z` plus `a-z` plus the non-ASCII uppercase letters, which is
  neither `Lu` nor `L&`.

  **THE REFUSALS SPLIT BY GATE STATE**, three ways, because at an OPEN gate
  "requires module 'unicode-props'" asks the user to do what they have already
  done (K14's shape, ext.c's UNBUILT epilogue's own reason): an unknown
  ONE-LETTER code drops the module clause entirely (no module will ever
  implement it, and pcrec's table is exhaustive for that axis); a name pcrec
  cannot CLASSIFY — real-but-stage-5, real-but-declined, an axis the table
  does not cover, or genuinely unknown — takes the UNBUILT tier. **Every
  CLOSED-gate text is byte-identical to before the producer landed**, which is
  what keeps every pre-stage-3 `tests/reject/` pin green rather than
  re-baselined; the twelve open-gate texts have their own `reject_gated` pins.

  Historically (MOD-0.6 phase 2), NO PRODUCER LANDED HERE (nothing that
  refuses today may start compiling — Frank's ruling). Measured against libpcre2 10.46
