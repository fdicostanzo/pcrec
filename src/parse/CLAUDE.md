...
- **mod_uprops.c** — module `unicode-props`: the `\p`/`\P` doorway's BODY
  ...
  **45 NAMES SHIP** from `uprops_tables.inc` — GENERATED by
  `third_party/ucd-16.0.0/generate.py` from the vendored UCD at Unicode
  16.0.0 (`third_party/README.md` for the vendoring shape; Frank's ASK 2
  ruling, agreed reluctantly, with the usage condition that none of it
  reaches a generated artifact). The seven one-letter general categories,
  all 30 two-letter ones, `L&`/`Lc`, `Any`, and PCRE2's `Xan Xps Xsp Xuc
  Xwd`. Scripts are stage 5; booleans and `Bidi_Class` are declined by
  design.
  ...
  **THE CASELESS RULE IS A SPAN SUBSTITUTION, NEVER `cls_casefold`.**
