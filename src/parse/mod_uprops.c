/* mod_uprops.c — module `unicode-props`: the `\p`/`\P` doorway's BODY SCANNER
 * and, since [M5.0] stage 3, its PRODUCER.
 *
 * ============================================================================
 * WHAT STAGE 3 CHANGED, AND WHAT IT DELIBERATELY DID NOT
 * ============================================================================
 *
 * `\p{L}` COMPILES NOW. The scanner below is unchanged in every byte of its
 * algorithm; what it gained is a table lookup at the point where it used to
 * refuse unconditionally, and `uprops_produce` beneath it. Everything the
 * MOD-0.6 header says about the SHAPE space — the malformed(146)-vs-
 * unknown-name(147) split, the 48-significant-character budget, the caret
 * rule, the blame offsets — is still exactly true and is still what the
 * closed-gate half of `tests/registry/pcre2_check.c`'s differential measures.
 *
 * THREE CLAIMS BELOW HAVE EXPIRED and are corrected where they stand rather
 * than deleted, because each one's REASONING is still the live rule:
 *   1. "NO PRODUCER LANDS HERE" — Frank's 2026-08-12 ruling, whose content is
 *      *nothing that refuses today may start compiling* WITHOUT a milestone
 *      authorising it. [M5.0] stage 3 is that authorisation.
 *   2. "the ExtPortFn signature is kept so wiring aport/cport is a one-line
 *      row edit" — the ports stay NO_PORT and now do so on purpose; the
 *      signature gained `in_class` instead. See internal.h's declaration.
 *   3. "THE SHORT-NAME TABLE is HAND-WRITTEN" — it is generated from the
 *      vendored UCD now, and the ruling that made it hand-written is intact:
 *      see `uprops_lookup`'s own comment for why a THIRD source changes the
 *      answer without changing the rule.
 *
 * THE TABLE (`uprops_tables.inc`, generated by
 * `third_party/ucd-16.0.0/generate.py`) IS COMPILE-TIME DATA IN `libpcrec.a`
 * AND REACHES NO ARTIFACT. A generated matcher carries the LOWERED AUTOMATON
 * — a machine over bytes — and holds no property data at all. That is the
 * usage condition attached to Frank's ASK 2 ruling, discharged at
 * `utf8_design.md` §3.3.1 and restated here because this is the file that
 * would have to break it.
 *
 * ============================================================================
 *
 * Historically (MOD-0.6 phase 2), what this file added was a REFINED refusal:
 * before it, every tail after `\p`/`\P` got the identical generic "\p requires
 * module 'unicode-props'" text from ext.c's fallback, at the BACKSLASH's
 * offset.
 ...
 *
 * THE SEAM. Not an aport/cport — and stage 3 turned that from "not yet" into
 * a decision: the port block ext.c runs fires only at post-gate WANT_RESULT,
 * while this function must also answer at VERDICT, must answer for a
 * MALFORMED body at any level, and must answer differently at the two
 * POSITIONS, so wiring a port would leave three of its four jobs on a second
 * path. Instead: `\p`/`\P` are single-row buckets (no sibling to arbitrate
 ...
 * ext.c). `pcrec_modport_uprops` no longer keeps the `ExtPortFn` signature:
 * stage 3 gave it the one parameter a port could not have carried,
 * `in_class`, which selects EXT_MEMBERS over EXT_NODE exactly as ext.c's own
 * PORT_SET branch does for the byte-set rows.
 *
 ...
 *
 * THE SHORT-NAME TABLE WAS HAND-WRITTEN and is now generated FROM THE UCD —
 * a different source from the one the ruling forbade, which is why the ruling
 * survives intact (`uprops_lookup` carries the argument). The ruling, at the
 * 2026-08-12 phase-2 authorization, overriding the probe_cls_bits.c precedent
 * this file's design note proposed: a table generated from libpcre2 and then
 ...
 * ALL 52 letters — not just the seven — so an eighth short-category letter
 ...
 * significant character, no `=`; the 7-of-52 table is complete, PC-3-
 ...
 * already done (K14's shape). The closed-gate texts are unchanged byte for
 * byte — which is what keeps every pre-stage-3 `tests/reject/` pin green
 * rather than re-baselined — and the open-gate texts have their own
 * `reject_gated` pins beside them. The full three-way argument is at the
 * lookup site below. */
