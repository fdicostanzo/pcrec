...
- **varexp.c** — [VAR] M1 (2026-09-23): THE EXPANSION GRAMMAR,
  ...
  `src/parse/mod_vars.c`) and a variable inside a REPLACEMENT template
  (module `subst-pcrec`, unbuilt). Design: `docs/design/variables_common.md`
  §1. Checked by `tests/core/varexp_check.c`.

  **A PURE FUNCTION, and the seam is what makes it testable below any
  artifact**: `Arena *` in, tree out, refusal RETURNED in a `VarExpErr`
  rather than raised through `pcrec_ctx_fail` — D108's data-in / text-out
  rule, so the doorway that alone knows its own diagnostic conventions
  decides how a refusal reads. Allocation failure needs no code here;
  `pcrec_arena_alloc` already routes through the arena's `.cx`.

  **IT DOES NOT EVALUATE, DELIBERATELY.** Evaluation needs the caller's
  environment, which arrives at MATCH time, so the evaluator is EMITTED C
  generated by walking this tree (`vm_emit_vars_resolve`, `src/gen/
  emit_vm.c`). A second evaluator here would have no production caller and
  would be a parallel mechanism kept alive by its own unit test. The unit
  check therefore covers the GRAMMAR and says so in its own header; the
  emitted evaluator's SEMANTICS are `tests/vars/`'s.

  **The COLON is a `bool` beside the operator, not four more enumerators**:
  `:-` and `-` are one operator under two emptiness rules
  (`variables_common.md` §2.2).
