 ...
 * — is NOT declared here. It is a `limits.def` row (home INTERNAL_H, D90),
 * generated by the X-macro include above, exactly as `PCREC_OFSK_MAX_SET`'s
 * own per-artifact emitter budget is. It has to be one number because the
 * PASS spends it (src/opt/scanedge.c, taking the longest chains first) and
 * the EMITTER's fixed array is sized by it (src/gen/emit_dfa.c) — an emitter
