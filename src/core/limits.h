 ...
 *
 * [LIM-1] (D90, 2026-08-30): every constant this file defines is now
 * GENERATED from `src/core/limits.def` — the single X-macro table
 * `pcrec --list-limits` also dumps and `docs/spec/limits.md` §3 is checked
 * against (tests/registry/limits_check.sh). The PROVENANCE PROSE below each
 * constant's own comment is unchanged and stays the place a reader learns
 * WHY a number is what it is; the number itself is spelled exactly once, in
 * the table, and the `#include` near the end of this file is what turns
 * each row into the `#define` every call site in this tree already used
 * before this lane — no site outside limits.def needed to change. */
 ...
 *
 * M2.8 raised PCREC_MAX_NFA_STATES from 20000. It is a MEMORY backstop
 * (48 B/state, two machines, so ~12.6 MB), not the real ceiling: the DFA
 * caps below are grounded in emitter cost (R1 A-3) and now bind first
 * across the realistic keyword range — 6000-word lists compile,
 * 10000-word lists fail on the DFA cap with its actionable "try
 * --engine=vm". Stack depth is no longer a constraint here: clo_visit's
 * tail edges are iterative (verified at -O0 on a 1,000,000-branch
 * alternation).
 *
 ...
 *
 * [M4.7b] K7's SECOND half (PCREC_MAX_SUBSET_ELEMS): how many NFA-state-list
 * ELEMENTS the priority subset construction may intern, summed over every
 * machine one compile builds (forward and reverse are charged together
 * because both are live at once).
 *
 ...
 *
 * [M4.5b] PCREC_MAX_VM_NODES: The VM emitter's own size backstop, and the
 * reason it needs one separate from the NFA cap above: a bounded repeat
 * REPLICATES its body (docs/design/engine_m4.md §3.3's RULED reading — `X{m,n}`
 * is m mandatory copies plus n-m optional ones, with no counter and no
 * suppression test), so `(a|b){0,10000}` emits ten thousand copies of the
 * body's labels. The NFA cap normally binds first, because the NFA
 * replicates the same way — but `--engine=vm` skips machine construction
 * entirely (§5.6: the prefilter is OFF in that mode, so there is no NFA to
 * cap), which is exactly the invocation where nothing else would stop it.
 *
 ...
 *
 * [M4.5c fix] PCREC_MAX_VM_REPEAT_COPIES: The SECOND VM size bound, and the
 * one D45's consequence 1 asks for: how many times a bounded repeat may
 * REPLICATE a body that contains a choice point.
 *
 ...
 *
 * [K22] PCREC_MAX_VM_REPLICATION_PRODUCT: The THIRD VM size bound, and the
 * one the other two structurally cannot provide: how many times a body may
 * be replicated in TOTAL along a chain of NESTED bounded repeats.
 *
 ...
 *
 * [ENG-BREP] PCREC_DEFAULT_UNROLL_K, the COUNTER rung's unroll factor (D47.2;
 * counter-K design note §4.1). One body copy per K iterations plus an
 * iteration counter, replacing the full replication the two caps above
 * exist to bound.
 *
 * WHY IT BELONGS IN THIS FILE, where VM_DEFAULT_WORK_BUDGET (src/gen/emit_vm.c)
 * deliberately does not: changing K changes what pcrec ACCEPTS. The
 * emitted copy count for a bounded repeat is a function of K, so K and the
 * caps above jointly decide which patterns compile and which are refused —
 * this file's own inclusion rule, met exactly. A runtime give-up budget
 * meets none of it and lives beside its siblings in the emitter.
 *
 ...
 * both by 8, and the first replicates either way. --unroll=N overrides it
 * per compile.
 *
 * [ENG-BREP] PCREC_MAX_POSSESS_POSITIONS: How many byte-consuming POSITIONS
 * a quantifier body may have before src/opt/possessify.c gives up on it. It
 * bounds the position (Glushkov) automaton the §2.2 unique-iteration test
 * is decided on: the analysis carries one byte set and one follow-union
 * per position, and the pairwise-disjointness sweep is quadratic in them.
 *
 ...
 *
 * [ENG-BREP] PCREC_MAX_REVDET_BODY_GROUPS: How many CAPTURING GROUPS a
 * quantifier body may contain before src/opt/revdet.c declines the
 * reverse-deterministic rung.
 *
 ...
 * rung's own `VM_MAX_BODY_CAPS` (src/gen/emit_vm.c — a bare alias of this
 * constant since [LIM-1], not a second independently-chosen number), which
 * bounds the same kind of table for the same failure: a group the table
 * could not hold would never be written and would report UNSET on a match
 * it participated in — a silent wrong span, which is the one outcome D26
 * refuses outright.
 *
 ...
 *
 * [OPT-ALTCLS] stage 2 (src/opt/altcls.c) PCREC_MAX_ALTCLS_FACTOR_DEPTH: how
 * many times prefix factoring may RECURSE into a group that just SPLIT
 * (branches that shared a literal prefix diverging on the next byte),
 * before it stops factoring that subtree further.
 *
 ...
 *
 * PCREC_MAX_REPEAT: `a{65535}` compiles; `a{65536}` is PCRE2 error 105,
 * "number too big in {} quantifier". This is the ceiling FIX-1's K5 was
 * about. Note python `re` does NOT agree — its ceiling is 4294967294 — so
 * the corpus oracle cannot check this one and tests/reject/ owns it.
 *
 * PCREC_MAX_GROUP_DEPTH: 250 nested groups compile; 251 is PCRE2 error 119,
 * "parentheses are too deeply nested", and pcrec agrees exactly. parse.c
 * called this a "PCRE2-like" cap, which undersold it — it is PCRE2's
 * number, measured. It also bounds parser and AST recursion depth (R1
 * review R-1), so it is load-bearing for more than compatibility.
 *
 * PCREC_MAX_GROUP_NAME: [M6.3] a named group's name: 128 bytes compile, 129
 * is PCRE2 error 148, "subpattern name is too long (maximum 128 code
 * units)" — swept 1..2000 bytes of an otherwise-valid name against
 * libpcre2 10.46 (tests/probes/probe_named_groups.c), so this is a
 * measured wall, not an assumed carry-over of PCRE1's older 32-byte
 * MAX_NAME_SIZE. */

/* ---- PCRE2 INTERNALS — minimums we honour, not contracts we owe -------- *
 *
 * Both PCREC_VERB_NAME_MAX and PCREC_VERB_LIMIT_ACC_MAX (and PCREC_UPROP_NAME_MAX
 * below) are artifacts of how libpcre2 10.46 is BUILT, not statements about
 * what a regex means, and both govern constructs pcrec rejects today and
 ...
 * exact again; see D26.
 *
 * PCREC_VERB_NAME_MAX: A verb name of 128 bytes is PCRE2's ordinary "not
 * recognized"; 129 is error 148, "subpattern name is too long (maximum 128
 * code units)". Measured over every length 1..319 in both name tables
 * (R8/C2-9): exactly two transitions in 638 probes, both at 129,
 * table-independent.
 *
 * PCREC_VERB_LIMIT_ACC_MAX: `(*LIMIT_MATCH=N)` and friends. PCRE2 refuses
 * while ACCUMULATING, one digit before its uint32_t counter would
 * overflow, so the boundary is 4294967290 rather than 4294967295 — reject
 * once the running value would exceed UINT32_MAX/10 - 1. Leading zeros
 * never move the accumulator, so `=00000000000000000001` compiles: this is
 * a MAGNITUDE rule, not a length one. Swept to exhaustion over 209 digit
 * strings x 4 names (R8/C2-9).
 *
 ...
 *
 * PCREC_UPROP_NAME_MAX: `\p{...}`/`\P{...}` property names. libpcre2 10.46
 * normalises the body WHILE SCANNING (space/tab/hyphen/underscore
 * insignificant, ASCII case folded) and counts only SIGNIFICANT
 * characters: 48 compiles or refuses as an unknown name (error 147), 49 is
 * "malformed \P or \p sequence" (error 146), blamed at the scan position
 * immediately after the 49th significant character — NOT at the end of
 * the body, which is how tests/probes/probe_uprops.c proved the count is
 * of significant characters rather than of total body bytes (a body
 * padded with insignificant filler past 100,000 bytes still compiles when
 * it has one significant character). R10 disposition 5 named 48 before
 * this was measured; the probe confirmed it exactly, so this is not a
 * corrected number, only a verified one. Not a contract pcrec owes — an
 * artifact of libpcre2's own build, MOD-0.6's `mod_uprops.c`.
 *
 * [DD-14 wave G] PCREC_MAX_SPLICE_NODES / PCREC_MAX_SPLICE_TOTAL: THE
 * SPLICE SIZE BUDGET, design §6.3 condition 3.
 *
 ...
 *
 * [ART-SIZE] PCREC_ANCHORED_MAX_STATES / PCREC_MAX_VM_EMIT_CODE_BYTES /
 * PCREC_DEFAULT_WARN_EMIT_BYTES / PCREC_MAX_EMIT_BYTES /
 * PCREC_SIZE_TERM_THRESHOLD: THE EMITTED-SIZE CAPS (D84 and its addenda;
 ...
 * one, and they are different obligations".
 *
 ...
 * `--max-emit-code-bytes=N` / `--max-emit-bytes=N` raise them per compile
 * (raise-only), and the place a real build does that is the pattern-source
 * file's `config` block, per target, beside the pattern (D84 addendum 3).
 * BUILD-TIME, a `-D` moves the built-in DEFAULT these overrides raise
 * FROM — the ONE non-default consumer today is tests/codegen/
 * run_size_term.sh's reference compiler, driving branches (`cap-rescue`,
 * PCREC_ANCHORED_MAX_STATES's own overflow arm) whose natural population
 * at the shipped defaults is zero.
 *
 ...
 *
 * CODE bytes (PCREC_MAX_VM_EMIT_CODE_BYTES): comment-excluded emitted
 * bytes OUTSIDE table initializers. D45's half. Counted exactly by the
 * emitter as it writes — never modelled, because a refusal must not
 * inherit a fit's error.
 *
 ...
 *
 * TOTAL bytes (PCREC_MAX_EMIT_BYTES): the whole comment-excluded artifact.
 * Frank's Q4 half (D84 ruling 2) — "a large byte count makes the artifact
 * unusable" — a concern in its own right and NOT a proxy for compile time.
 * An EXACT post-emission check, refusing before the file is written, with
 * no model on this axis at all (D84 addendum): a fixed number and a loud
 * refusal, so the outcome is predictable by construction rather than by
 * prediction.
 *
 ...
 *
 * The K SELECTION's trigger (PCREC_SIZE_TERM_THRESHOLD): the emitted size
 * above which the size term evaluates the unroll ladder at all. Same unit
 * as the caps.
 *
 ...
 * §8.2).
 *
 * WHY IT IS ITS OWN NUMBER, added at the r41 close (finding S1). The
 * anchored machine is OPTIONAL — an entry point's form, not an engine's
 * requirement — so a pattern whose anchored machine would be enormous
 * should DECLINE the form rather than pay for it, and under
 * `PCREC_MAX_DFA_STATES_TABLE` it did not: r41 measured **+46 % of pcrec's
 * own COMPILE CPU** on `tests/resource`'s giant-repeat shapes
 * (`[a-z]{0,30000}` 23.4 s -> 37.5 s), taking that suite's 45 s `K7_CPU`
 * headroom from 21.6 s to 7.5 s, and pushed one artifact to 1,984,382 B —
 * over `[ART-SIZE.1b]`'s 1,400,000 B pin, and invisible to the tripwire
 * because those shapes live in a bash array rather than in the size log.
 * Both are the SAME fact: the optional machine was charged the mandatory
 * machines' budget.
 *
 * WHERE 4,096 COMES FROM — measured, not chosen, and RE-derived once when
 * the first measurement turned out to be over the wrong population.
 *
 ...
 * Over the corpus compiled `--no-captures` — the LARGER population,
 * because a capture-bearing pattern is VM-selected captures-on and never
 * builds an anchored machine at all — 1,213 artifacts select it, median
 * 3, p99 196, and **three exceed 4,096**: `((a)|ab){0,4000}c`,
 * `((a)|ab){4000}c` and `((a)|bc){0,4000}d`, all of
 * `tests/counterk/counterk.rxt`'s 4000-count family. `tests/resource`'s
 * DFA-routed shapes are 20,001, 25,001 and 30,001.
 *
 * So 4,096 is **2.05x the captures-on maximum**, above 1,210 of the 1,213
 * `--no-captures` selectors, and **4.9x below the smallest resource
 * shape**. The three it excludes are 4000-count torture shapes whose
 * `<prefix>_match` no caller optimises for, and excluding them is what
 * keeps the ceiling low enough to be worth having — raising it past them
 * would put it within 1.2x of the resource shapes it exists to exclude.
 *
 * **"NO CORPUS ARTIFACT LOSES THE FORM" WAS THE FIRST DERIVATION'S CLAIM
 * AND IT IS FALSE**; three do. The error was measuring one compile mode
 * and generalising to the tree, and it was caught by
 * `tests/anchored/run_anchored_diff.sh`'s compared population moving
 * 1,213 -> 1,210 rather than by re-reading the derivation.
 *
 ...
 * `maxstates` it is handed — so a wide-alphabet anchored machine is
 * bounded by ENTRIES exactly as the mandatory pair is, through the same
 * two lines. That is why this is one more argument to the existing
 ...
 * `docs/spec/limits.md` states that as the second, narrower exception on
 * the three DFA ceilings.
 *
 * IT IS ALSO THE OVERFLOW ARM'S OWN WITNESS SUPPLY. Before this ceiling
 * the arm had ZERO reachable population (the shared caps are hit by the
 * mandatory machines first) and could only be driven by the `-D` override;
 * `tests/codegen/run_anchored_match.sh` §4a now brackets the ceiling from
 * both sides and compiles one of the three IN-CORPUS overflow patterns,
 * and §4b keeps the override as the control that the arm behaves the same
 * way at a cap no shape reaches.
 *
 * OVERRIDABLE FOR EXACTLY ONE CONSUMER. `run_anchored_match.sh` §4 builds
 * a reference compiler with this lowered to 6 so the arm is exercised on
 * small, fast patterns as well as on the four heavy ones — the same shape
 * and the same single consumer as `-DPCREC_NO_TRIE` and
 * `-DPCREC_NO_ENDVAR` (`src/ir/dfa.c`'s header states the rule); never
 * overridden in a shipped build.
 *
 ...
 *
 * WHY OVERRIDABLE AT BUILD TIME (this row and the four size/threshold rows
 * above it), when the CLI overrides on three of them are raise-only.
 ...
 * pattern whose byte ratio exceeds the bar while some ladder K drops its
 * CODE under the cap, and on replication-dominated patterns bytes and code
 * move together, so a ratio above the bar keeps code above the cap. Five
 * candidate shapes were probed and none reached it. Because the CLI
 * overrides are RAISE-ONLY (deliberately: a raise-only flag cannot be used
 * to manufacture a refusal on someone else's build), the path cannot be
 * forced from outside either — which would leave a shipped branch no test
 * can drive.
 *
 * So the structural check builds a REFERENCE COMPILER with a cap lowered
 * at pcrec's own compile time and drives the branch through it. That is
 * not a new surface: it is [ENG-ABS]'s precedent one lane over, whose
 * overflow arm has the same empty natural population and whose check
 * builds a reference compiler with `-DPCREC_ANCHORED_MAX_STATES=6`.
 *
 * A BUILD-TIME `-D`, NEVER A CLI VALUE. The distinction is the whole
 * point: lowering a cap has to be something a person does to a compiler
 * they built for a test, not something a caller can do to a compile, so
 * the raise-only rule stays true for every user of a shipped pcrec.
 *
 * THE ONE NON-DEFAULT CONSUMER of the three D84 macros is
 * `tests/codegen/run_size_term.sh`. If a second appears, it wants a
 * comment here saying why.
 *
 * MEASURED, so the next reader does not have to re-derive it: with
 * `-DPCREC_MAX_VM_EMIT_CODE_BYTES=40000`, `((a)|ab){0,2047}c` reaches
 * `cap-rescue` — its materiality bar declines K=1 (byte ratio 0.922) and
 * the lowered code cap then takes it anyway. That is the witness the
 * check drives.
 *
 * [OPT-4] PCREC_DEFAULT_WARN_EMIT_BYTES: the ADVISORY warning threshold
 * (Frank, 2026-08-29; `lib/pcrec.h`'s `warn_emit_bytes`,
 * docs/spec/limits.md). An order of magnitude under `PCREC_MAX_EMIT_BYTES`
 * on purpose: the point of a warning is to arrive while the pattern can
 * still be changed, not at the moment it is refused. It is a default and
 * not a cap — nothing refuses on it, and 0 turns it off.
 *
 * Units, derivations and the failsafe framing: see
...
#define PCREC_MAX_VM_EMIT_CODE_BYTES PCREC_MAX_VM_EMIT_CODE_BYTES_DEFAULT
#endif
#ifndef PCREC_DEFAULT_WARN_EMIT_BYTES
#define PCREC_DEFAULT_WARN_EMIT_BYTES PCREC_DEFAULT_WARN_EMIT_BYTES_DEFAULT
#endif
#ifndef PCREC_MAX_EMIT_BYTES
#define PCREC_MAX_EMIT_BYTES PCREC_MAX_EMIT_BYTES_DEFAULT
#endif
