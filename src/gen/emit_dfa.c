 ...
 * ESSENTIAL generated-by header line (emit_pattern_comment, just below)
 * and rx_info.abi's own initializer (emit_info_def, far below) -- and both
 * read it from here rather than each carrying its own literal. When the
 ...
 * rule whose own text is the literal SQL-comment-obfuscation bytes that
 * open then close a comment with only "!" between), which reached the DFA
 * emitter's per-state "shortest input" legend (below) UNESCAPED and
 * corrupted every byte the emitter wrote afterward -- a missing terminating
 * string quote cascading into undeclared-identifier errors, a genuine C
 * COMPILE FAILURE rather than a wrong answer.
 *
 * A SECOND, RELATED HAZARD caught while building the regression net: a
 * SLASH THEN STAR embedded in the pattern's bytes LOOKS like an attempt to
 * nest a comment, which gcc flags as `-Wcomment` (`-Wall`) -- and the
 * harness's own GENCFLAGS default is `-Wall -Wextra -Werror`
 * (tests/harness/run.sh), so that warning is every bit as fatal to a real
 * build as the first hazard's missing terminator, reachable on a pattern
 * whose bytes open a comment this way with nothing later in them that
 * would go on to trip the first hazard at all. Same fix, mirrored: a byte
 * that would complete either ordering of the pair with the byte just
 * rendered is escaped.
 *
 * Printable ASCII prints literally UNLESS doing so would complete a
 * STAR-SLASH or SLASH-STAR pair with the byte just rendered, or the
 * caller's own `extra_escape` predicate asks for it (the per-state legend
 * below also hex-escapes `"` and `\` so its printed example reads as an
 * unambiguous quoted string -- a READABILITY choice independent of comment
 * safety, layered on top rather than folded into this function's one job).
 * Everything else becomes a fixed `\xNN` -- the spelling this banner
 * comment already used, now the ONE place that spelling is written.
 *
 * `*prevp` threads the previously rendered byte across calls: the per-state
 * legend assembles one example from several calls (one per reconstructed
 * path byte), so either pair must be caught even when its two halves are
 * rendered by different calls. NOT for a two-slash line comment -- those
 * end at the next newline regardless of content, and running this there
 * would over-escape for no reason. */
static void emit_comment_safe_byte(StrBuf *sb, int *prevp, unsigned char ch,
                                    bool (*extra_escape)(unsigned char))
{
    bool safe = ch >= 32 && ch < 127 &&
                !(*prevp == '*' && ch == '/') &&
                !(*prevp == '/' && ch == '*') &&
                !(extra_escape && extra_escape(ch));
    if (safe) pcrec_sb_putc(sb, (char)ch);
    else      pcrec_sb_printf(sb, "\\x%02x", ch);
    *prevp = ch;
}
 ...
 *
 * [EMIT-VERB rider, D112 item 2] THE LINE ALSO NAMES THE ABI, riding this
 * abi's own bump (26 -> 27) rather than opening a second one (D76/D94's
 * "an emitted-scaffolding change is an abi bump" already covers this
 * change; amending the bump this event already makes, instead of taking a
 * second one, is the D94-addendum shape). D112's essential class was
 * always specified as "the generated-by header line naming pcrec AND THE
 * ABI, and the pattern echo" — the shipped line named only pcrec and the
 * pattern, and this closes that gap. The digit is `PCREC_ARTIFACT_ABI`,
 * never a second literal — see that constant's own comment.
 *
 * [REL-1.4 rider, D115] THE LINE ALSO NAMES THE PCREC VERSION (independent
 * of abi — see `PCREC_VERSION`'s own comment, lib/pcrec.h), riding the
 * abi 27 -> 28 bump this event opens (D76/D94: the change is emitted
 * scaffolding, so it is an abi bump on its own footing regardless of the
 * version rider; putting both on one event rather than two follows the
 * abi-digit rider's own precedent immediately above). This SAME line is
 * why the version stamp survives `-fno-comments`: it is the ESSENTIAL
 * provenance line, D112's class the switch never removes, so an artifact
 * always says which pcrec produced it whether or not `-fcomments` was
 * given. */
static void emit_pattern_comment(StrBuf *sb, const char *pat)
{
    pcrec_sb_cmt_open(sb, PCREC_CMT_ESSENTIAL);
    pcrec_sb_printf(sb, "/* Generated by pcrec %s (abi %d). Pattern: ",
              PCREC_VERSION, PCREC_ARTIFACT_ABI);
    int prev = 0;
 ...
 * engine-appropriate value. It used to be a `pcrec_sb_printf` inside `emit_vm.c`'s
 ...
 * `pcrec_sb_printf`s are two chances for the spelling to drift. The VM's call passes
 ...
 * per-artifact stamp, on both engines, and two independent `pcrec_sb_printf`s are
...
void pcrec_emit_engine_stamp(StrBuf *c, const char *upper, const char *engine,
                             const char *sel)
{
    pcrec_sb_stamp_str(c, upper, "ENGINE",     engine);
...
/* [OPT-REQPOS] tier 2b THE NECESSARY-RUN PRE-CHECK — the `L >= 2` sibling of
 * the one-byte check below, inside the same function so the two engines
 * cannot test the run in two different shapes (that function's own stated
 * reason for existing).
 *
 ...
 * hit implies. No run in the window means no match in the window, so the
 * whole call answers 0 from here.
 *
 * [OPT-LITSCAN] S1 step 6: THE SEARCH IS THE OFFSET-SKIP BLOCK'S, not a loop
 * of its own (litscan_s1.md §7.2 step 6, §1.6 P5). A run `R` of length `L`
 * floating in `[pos, n)` is an `OfsTest` whose scan is `(i, R[i])` and whose
 * one term is the run at offset 0 (`ofs_test_run`): the run is present iff
 * the block returns below `n`. Its loop guard `pos + L - 1 < n` and its
 * `cand + L - 1 >= n` exit are the run's own "lies ENTIRELY inside the
 * window" test, since every byte of every match does, and the guard also
 * closes [K27]'s `memchr(NULL, c, 0)` on an empty window. The block writes
...
/* The emitted name of run pre-check block `i`: `<p>_reqrun` for the window,
 * `<p>_reqrun_whole` for [K66]'s whole run. */
static const char *req_run_fn_name(Ctx *cx, int i)
{
...
{
    pcrec_sb_printf(sb, "extern const struct rx_info %s;\n", infoname);
}

/* [REVW.2] wave 2 stage 3: this file's adapter over `pcrec_sb_fragf`, the emission
 ...
 * exactly one thing over calling `pcrec_sb_fragf` directly: it takes the arena from
...
{
    /* [EMIT-VERB] (D112) ESSENTIAL, for the whole block. Every comment here
     * documents a TYPE a caller touches — `rx_ctx`'s fields, `rx_matchfn`'s
     * return space, the error codes, `rx_info`'s members — which is what
     * docs/spec/match_api.md means by the shipped doc-comments an embedder
     * actually reads. One region covers the block because the block is one
                          ...
                          const char *upper, const InfoStamp *st)
{
    ...
    if (cx->n_named_groups > 0) {
        ...
        pcrec_sb_printf(c, "static const rx_group_entry %s[] = {\n", groups_name);
     ...
     *
     * `abi` is the version of the artifact's EMITTED SCAFFOLDING AS A WHOLE
     * (D76) — not of `struct rx_info` alone. Any change to an emitted byte
     * is a bump: a new `#define`, a moved struct offset, a comment, layout.
     * The ritual is one commit carrying the bump, the identity-gate re-pin
     ...
     * time. A bump's own D76/D94 ritual already maintains §6; nothing
     * maintained this copy, which is the argument for not keeping one.
     *
     * The current value is `PCREC_ARTIFACT_ABI` (this file). The bump this
     * emitter last took is `27 -> 28`, [REL-1.4]'s version stamp (D115):
    ...
    pcrec_sb_printf(c, "    .encoding = %d,\n", cx->opt->encoding);
    pcrec_sb_printf(c, "    .ncaps = %s_NCAPS,\n", upper);
    ...
    pcrec_sb_printf(c, "    .ngroups = %d,\n", (int)cx->ncap_primary);
    pcrec_sb_printf(c, "    .nnames = %u,\n", ng_count_caller_scope(cx));
    /* [ABI-NS] (D60 addendum): the comment names the emitted PCREC_ENGINE_*
     * constants (emit_rx_abi_types), not the internal ENGM_* enum. */
    pcrec_sb_printf(c, "    .engine = %d, /* %s */\n", st->engine,
              st->engine == 2 ? "PCREC_ENGINE_VM" : "PCREC_ENGINE_DFA");
    pcrec_sb_printf(c, "    .step_budget = %lld,\n", st->step_budget);
    pcrec_sb_printf(c, "    .work_budget = %lld,\n", st->work_budget);
    pcrec_sb_printf(c, "    .frame_capacity = %lld,\n", st->frame_capacity);
    pcrec_sb_printf(c, "    .subject_ceiling = %lld,\n", st->subject_ceiling);
    pcrec_sb_printf(c, "    .resume_frames = %lld,\n", st->bufs->resume_frames);
    pcrec_sb_printf(c, "    .trail_frames = %lld,\n", st->bufs->trail_frames);
    pcrec_sb_printf(c, "    .resume_frame_size = %d,\n", st->bufs->resume_frame_size);
    pcrec_sb_printf(c, "    .trail_frame_size = %d,\n", st->bufs->trail_frame_size);
    pcrec_sb_puts(c,   "    .pattern = ");
    emit_c_string_literal(c, cx->pat, cx->patlen);
    pcrec_sb_puts(c,   ",\n");
    pcrec_sb_printf(c, "    .pattern_len = %zu,\n", cx->patlen);
    if (groups_name)
        pcrec_sb_printf(c, "    .groups = %s,\n", groups_name);
    else
    ...
    if (pcrec_artifact_has_dfa_scan(cx)) {
        pcrec_sb_printf(c, "    .scan = \"%s\",\n", dfa_scan_name(cx));
        pcrec_sb_printf(c, "    .prefilter = \"%s\",\n", dfa_prefilter_name(cx));
    } else {
...
{
    pcrec_sb_printf(c, "    static const unsigned char %s_%s[%d] = {", p, tag, n);
    for (int i = 0; i < n; i++) {
        if (i % 16 == 0) pcrec_sb_puts(c, "\n       ");
        pcrec_sb_printf(c, " %3d,", v[i]);
    }
    pcrec_sb_puts(c, "\n    };\n");
}
...
static void emit_eol_table(StrBuf *c, const char *p, const char *tag,
                           const Dfa *d, const DfaRepr *r)
{
    pcrec_sb_printf(c, "    static const %s %s_%s[%d] = {", r->cell_type, p, tag, d->n);
    for (int i = 0; i < d->n; i++) {
        if (i % 16 == 0) pcrec_sb_puts(c, "\n       ");
        int v = d->st[i].eolvar;
        pcrec_sb_printf(c, " %d,", v < 0 ? r->dead_cell : r->cell_of(v, d));
    }
    pcrec_sb_puts(c, "\n    };\n");
}
                           ...
                           const Dfa *d, const DfaRepr *r)
{
    ...
    pcrec_sb_printf(c, "    static const unsigned char %s_%s[%d] = {", p, tag,
              d->n * rep_n);
    int k = 0;
    for (int i = 0; i < d->n; i++) {
                               ...
                               const Dfa *d)
{
    ...
    for (int i = 0; i < d->n; i++) {
        for (int cl = 0; cl < d->ncls; cl++, k++) {
            if (k % 16 == 0) pcrec_sb_puts(c, "\n       ");
            pcrec_sb_printf(c, " %d,", accw_cell(d, i, cl));
        }
    }
    pcrec_sb_puts(c, "\n    };\n");
}
...
static void legend_byte(StrBuf *c, int b)
{
    ...
    else if (b >= 0x20 && b < 0x7f) pcrec_sb_printf(c, "'%c'", b);
    else                pcrec_sb_printf(c, "%d", b);
}
...
{
    pcrec_sb_printf(c, "     * Class legend (%d classes; every byte not listed is class 0):\n",
              d->ncls);
    pcrec_sb_puts(c, "     *   0  every other byte\n");
    for (int cl = 1; cl < d->ncls; cl++) {
        pcrec_sb_printf(c, "     *   %d  ", cl);
        int shown = 0, b = 0;
        while (b < 256 && shown < 8) {
...
/* The per-state legend's own `extra_escape` for emit_comment_safe_byte: `"`
 * and `\` are not a comment hazard on their own, but this example is shown
 * as a quoted string and an unescaped one of either would make it read as
...
static void emit_state_legend(Ctx *cx, StrBuf *c, const Dfa *d, bool reverse)
{
    ...
    if (brief) {
        ...
        pcrec_sb_printf(c, "     * State legend -- %d states, %d of them accepting: too many\n"
                     "     * to name individually. The start state is %d",
                  d->n, nacc, d->s0);
        if (shortest >= 0)
            pcrec_sb_printf(c, ", and the\n"
    ...
    for (int i = 0; i < d->n; i++) {
        ...
        if (dist[i] < 0) {
            pcrec_sb_printf(c, "     *  %3d  (no path through the transitions above;\n"
                         "     *       a position view can still select it)\n", i);
            continue;
        }
        pcrec_sb_printf(c, "     *  %3d  ", i);
        if (dist[i] == 0) pcrec_sb_puts(c, "(start) nothing consumed yet");
             ...
             * byte is judged on its own merits, exactly as before this
             * hazard was handled. */
            int prev = '"';
            for (k = 0; k < shown; k++) {
                int b = d->rep[path[k]];
            ...
            pcrec_sb_puts(c, "\"");
            if (shown < len) pcrec_sb_printf(c, "... (%d bytes)", len);
        }
                       ...
                       const char *index_expr)
{
    ...
    pcrec_sb_printf(c, "static inline %s_%s_state %s_%s_step(const %s *transitions, %s_%s_state s, unsigned cl)\n"
...
static void token_accepts(StrBuf *c, const DfaForm *f, const char *cls_index)
{
    ...
    if (f->acc_fold.folded) {
        ...
        pcrec_sb_printf(c, "static inline int %s_%s_accepts(const unsigned char *accepting, %s_%s_state s)\n"
                     "{ return accepting[s]; }\n", p, m, p, m);
    if (!f->acc->wide_table) return;
    pcrec_sb_printf(c, "static inline int %s_%s_accepts_class(const unsigned char *accepting, %s_%s_state s, unsigned cl)\n"
                 "{ return %s; }\n", p, m, p, m, cls_index);
}
...
static void token_premul(StrBuf *c, const DfaForm *f)
{
    ...
    pcrec_sb_cmt_close(c);
    pcrec_sb_printf(c, "typedef unsigned %s_%s_state;\n", p, m);
    token_step(c, f, "unsigned short", "transitions[s + cl]");
    pcrec_sb_printf(c, "static inline int %s_%s_is_dead(%s_%s_state s) { return s == %d; }\n",
              p, m, p, m, PREMUL_DEAD);
    token_stop(c, f);
    ...
    if (f->viewsel) {
        ...
        pcrec_sb_printf(c, "static inline unsigned %s_%s_row(%s_%s_state s) { return s / %d; }\n",
                  p, m, p, m, nc);
        pcrec_sb_printf(c, "static inline int %s_%s_view_live(const unsigned short *view, unsigned row)\n"
                     "{ return view[row] != %d; }\n", p, m, PREMUL_DEAD);
        pcrec_sb_printf(c, "static inline %s_%s_state %s_%s_view_take(const unsigned short *view, unsigned row)\n"
                     "{ return view[row]; }\n", p, m, p, m);
    }
    pcrec_sb_puts(c, "\n");
}
...
static void token_indexed(StrBuf *c, const DfaForm *f)
{
    ...
    pcrec_sb_printf(c,
        ...
        " */\n", f->dir->label, p, m, nc);
    pcrec_sb_cmt_close(c);
    pcrec_sb_printf(c, "typedef int %s_%s_state;\n", p, m);
    ...
    pcrec_sb_printf(c, "static inline int %s_%s_is_dead(%s_%s_state s) { return s < 0; }\n",
              p, m, p, m);
    token_stop(c, f);
    token_accepts(c, f, acc_ix);
    if (f->viewsel) {
        pcrec_sb_printf(c, "static inline unsigned %s_%s_row(%s_%s_state s) { return (unsigned)s; }\n",
                  p, m, p, m);
        pcrec_sb_printf(c, "static inline int %s_%s_view_live(const short *view, unsigned row)\n"
                     "{ return view[row] >= 0; }\n", p, m);
        pcrec_sb_printf(c, "static inline %s_%s_state %s_%s_view_take(const short *view, unsigned row)\n"
                     "{ return view[row]; }\n", p, m, p, m);
    }
    pcrec_sb_puts(c, "\n");
}
...
static void view_emit_eol_only(StrBuf *c, const DfaForm *f)
{
    ...
    view_decl(c, f);
    pcrec_sb_printf(c, "%sif (__builtin_expect(%s + 1 >= subject_length, 0) &&\n"
...
static void view_emit_end_only(StrBuf *c, const DfaForm *f)
{
    ...
    pcrec_sb_printf(c, "%sif (__builtin_expect(%s == subject_length, 0) &&\n"
...
static void view_emit_end_and_eol(StrBuf *c, const DfaForm *f)
{
    ...
    view_decl(c, f);
    pcrec_sb_printf(c, "%sif (__builtin_expect(%s + 1 >= subject_length, 0)) {\n", ind, pos);
    pcrec_sb_printf(c, "%s    if (%s == subject_length) {\n", ind, pos);
    pcrec_sb_printf(c, "%s        if (%s_%s_view_live(%s_%s_end_view, %s_%s_row(%s)))\n"
                 "%s            %s = %s_%s_view_take(%s_%s_end_view, %s_%s_row(%s));\n"
              ...
              ind, VTBL(f), VTBL(f), VROW(f),
              ind, f->dir->viewv, VTBL(f), VTBL(f), VROW(f));
    pcrec_sb_printf(c, "%s    } else if (subject[%s] == '\\n' &&\n"
                 "%s               %s_%s_view_live(%s_%s_eol_view, %s_%s_row(%s))) {\n",
              ind, pos,
              ind, VTBL(f), VTBL(f), VROW(f));
    pcrec_sb_printf(c, "%s        %s = %s_%s_view_take(%s_%s_eol_view, %s_%s_row(%s));\n",
              ind, f->dir->viewv, VTBL(f), VTBL(f), VROW(f));
    pcrec_sb_printf(c, "%s    }\n%s}\n", ind, ind);
}
...
static void acc_emit_tail_scalar(StrBuf *c, const DfaForm *f)
{
    ...
    pcrec_sb_printf(c, "%s%s = %s_%s_step(%s%s, %s_%s_byte_class[%s]);\n",
...
static void acc_emit_tail_by_class(StrBuf *c, const DfaForm *f)
{
    ...
    pcrec_sb_printf(c, "%s    break;\n%s}\n", ind, ind);
    pcrec_sb_printf(c, "%s{\n", ind);
    pcrec_sb_printf(c, "%s    unsigned %s_class = %s_%s_byte_class[%s];\n",
              ind, m, p, m, f->dir->peek);
    pcrec_sb_printf(c, "%s    if (%s_%s_accepts_class(%s_%s_is_accepting_by_class, %s, %s_class))"
                 " %s = %s;\n",
              ind, p, m, p, m, f->src, m, f->dir->recv, f->dir->posv);
    pcrec_sb_printf(c, "%s    %s = %s_%s_step(%s%s, %s_class);\n",
...
static void pf_emit_ofs_bounded(StrBuf *c, const DfaForm *f)
{
    ...
    pcrec_sb_printf(c, "%s    } else if (scan_position + 1 < subject_length) {\n", ind);
    pcrec_sb_printf(c, "%s        scan_position = subject_length - 1;\n", ind);
    {
        pf_emit_ofs_reseed(c, f, dfa_fragf(f->cx, "%s        ", ind));
    }
    pcrec_sb_printf(c, "%s    }\n%s}\n", ind, ind);
 ...
 *
 * `pcrec --list-axes` (src/dump/axes_dump.c) needs the SAME name+deny data
...
static void emit_scan_edge(StrBuf *c, const DfaForm *f, int head)
{
    ...
    pcrec_sb_cmt_open(c, PCREC_CMT_NONESSENTIAL);
    if (span < 0)
        pcrec_sb_printf(c, "%s// [OPT-5] SCAN EDGE: every state a run of class %d\n"
                     "%s// would pass IS this state, so count them here rather\n"
                     "%s// than stepping the table once per byte.\n",
                  ind, st->scan_cls, ind, ind);
    else
        pcrec_sb_printf(c, "%s// [OPT-5] SCAN EDGE: the states between here and state\n"
    ...
    pcrec_sb_printf(c, "%s        %s = %d;\n", ind, f->dir->statev,
              nx < 0 ? f->repr->dead_cell : f->repr->cell_of(nx, f->d));
    if (facc)
        pcrec_sb_printf(c, "%s        %s = %s;\n", ind, f->dir->recv, f->dir->posv);
    pcrec_sb_printf(c, "%s    }", ind);
    if (acc) {
        pcrec_sb_printf(c, " else {\n");
        pcrec_sb_printf(c, "%s        %s = %s;   // the run stopped inside the edge\n",
                  ind, f->dir->recv, f->dir->posv);
        pcrec_sb_printf(c, "%s    }\n", ind);
    } else {
        pcrec_sb_puts(c, "\n");
...
static void emit_scan_loop(StrBuf *c, const DfaForm *f)
{
    ...
    if (seedhead) {
        if (f->seed->table)
            pcrec_sb_printf(c, "%sif (%s_%s_is_stop(%s) && !%s_%s_is_dead(%s))"
                         " goto %s;   // seeded straight onto a scan-edge head\n",
                      ind, f->p, f->dir->c.name, f->dir->statev,
                      f->p, f->dir->c.name, f->dir->statev, le);
        else
            pcrec_sb_printf(c, "%sgoto %s;   // the start state carries a scan edge\n",
                      ind, le);
    }
    pcrec_sb_printf(c, "%sfor (;;) {\n", ind);
    ...
    if (edges) pcrec_sb_printf(c, "%s  %s:;\n", ind, lv);
    if (f->view->emit) f->view->emit(c, f);
    if (f->acc->emit_after_view) f->acc->emit_after_view(c, f);
    f->acc->emit_tail(c, f);
    if (!edges) {
        pcrec_sb_printf(c, "%s    if (%s_%s_is_dead(%s)) break;   // dead: no match can continue\n",
                  ind, f->p, f->dir->c.name, f->dir->statev);
        pcrec_sb_printf(c, "%s}\n", ind);
        return;
    }
    ...
    pcrec_sb_printf(c, "%s    if (!%s_%s_is_stop(%s)) continue;   // the generic path\n",
              ind, f->p, f->dir->c.name, f->dir->statev);
    pcrec_sb_printf(c, "%s    if (%s_%s_is_dead(%s)) break;   // dead: no match can continue\n",
              ind, f->p, f->dir->c.name, f->dir->statev);
    ...
    if (seedhead) pcrec_sb_printf(c, "%s  %s:;\n", ind, le);
...
{
    if (tgt < 0) pcrec_sb_printf(c, "&&%s_dead", p);
...
static void emit_attempt(Ctx *cx, const char *fn, const char *storage)
{
    ...
    pcrec_sb_cmt_open(c, PCREC_CMT_NONESSENTIAL);
    pcrec_sb_printf(c, "    /* Jump targets, one row per state, indexed by byte class.\n"
    ...
    pcrec_sb_cmt_close(c);
    for (int i = 0; i < d->n; i++) {
        pcrec_sb_printf(c, "    static const void *const %s_targets_%d[%d] = { ",
                  p, i, d->ncls);
        for (int cl = 0; cl < d->ncls; cl++) {
    ...
    if (seed) {
        pcrec_sb_printf(c, "    static const void *const %s_seed_state[%d] = { ",
                  p, d->ncls);
        for (int cl = 0; cl < d->ncls; cl++) {
            if (cl) pcrec_sb_puts(c, ", ");
    ...
    if (gtbl) {
        pcrec_sb_printf(c, "    static const void *const %s_gstart_seed_state[%d] = { ",
                  p, d->ncls);
        for (int cl = 0; cl < d->ncls; cl++) {
            if (cl) pcrec_sb_puts(c, ", ");
    ...
    if (gseed) {
        ...
        pcrec_sb_puts(c, "\n               : (start == search_from) ? ");
        if (gtbl) pcrec_sb_printf(c, "%s_gstart_seed_state[%s_byte_class[subject[start - 1]]]", p, p);
        else      emit_target(c, p, d->s1g[UPC_PLAIN]);
        pcrec_sb_puts(c, "\n               : ");
        if (seed) pcrec_sb_printf(c, "%s_seed_state[%s_byte_class[subject[start - 1]]]", p, p);
        else      emit_target(c, p, d->s1u[UPC_PLAIN]);
        pcrec_sb_puts(c, ");\n");
    } else if (seed) {
        pcrec_sb_puts(c, "        goto *((start == 0) ? ");
        emit_target(c, p, d->s0);
        pcrec_sb_printf(c, " : %s_seed_state[%s_byte_class[subject[start - 1]]]);\n", p, p);
    } else if (d->s0 == d->s1u[UPC_PLAIN]) {
        pcrec_sb_puts(c, "        goto ");
        if (d->s0 < 0) pcrec_sb_printf(c, "*&&%s_dead;\n", p);
        else           pcrec_sb_printf(c, "*&&%s_s%d;\n", p, d->s0);
    } else {
        pcrec_sb_puts(c, "        goto *((start == 0) ? ");
        emit_target(c, p, d->s0);
        pcrec_sb_puts(c, " : ");
    ...
    for (int i = 0; i < d->n; i++) {
        ...
        pcrec_sb_printf(c, "%s_s%d: __attribute__((unused));\n", p, i);
        ...
        if (wsplit) {
            ...
            if (st->eolvar >= 0) {
                ...
                pcrec_sb_printf(c, "            goto *%s_targets_%d[%s_byte_class[subject[scan_position++]]];\n",
                          p, st->eolvar, p);
                pcrec_sb_puts(c, "        }\n");
            }
            pcrec_sb_printf(c, "        {\n"
            ...
            if (a_end == a_nl) {
                ...
                if (a_end)
                    pcrec_sb_puts(c, "            last_accept_position = scan_position;\n");
                pcrec_sb_printf(c, "            if (scan_position >= subject_length) goto %s_done;\n", p);
                pcrec_sb_printf(c, "            goto *%s_targets_%d[%s_byte_class[subject[scan_position++]]];\n",
                          p, st->eolvar, p);
                pcrec_sb_puts(c, "        }\n");
            } else {
...
static void emit_orientation_block(Ctx *cx, StrBuf *c, const GenNames *g)
{
    ...
    pcrec_sb_puts(c, "/* =====================================================================\n"
               ...
               " *\n *     ");
    /* [O-31 F1] the pattern itself, rendered safe for a block comment.
     * DELIBERATELY NOT routed through the shared emit_comment_safe_byte
     * above: that function's contract is printable-ASCII-or-hex-escape,
     * which is right for the state legend's quoted example (comment
     * safety AND readability both matter there) but is a STRICTER policy
     * than this one-line map paragraph ever had or needs -- every byte
     * that is not one of the two comment-hazard pairs below, INCLUDING
     * a raw UTF-8 continuation byte or a literal tab, has always printed
     * here exactly as it appears in the pattern. Routing this site
     * through the shared escaper was tried and MEASURED WRONG: over the
     * whole corpus it moved 115 currently-compiling artifacts' bytes
     * (every `-e utf8` multi-byte-literal and `\t`-in-a-modifier-run
     * pattern in the tree), an identity-population violation this row's
     * own ABI verdict exists to rule out -- caught by the corpus-wide
     * byte-diff this row's report carries, not by any test in the tree.
     * So this site keeps its OWN loop, corrected to consume each source
     * byte EXACTLY ONCE (the prior version's bug, below) and extended to
     * the same TWO hazard pairs emit_comment_safe_byte guards, with
     * every other byte -- non-printable or not -- passed straight
     * through exactly as it always was. */
    {
        int prev = 0;
        for (const char *q = cx->pat; *q; q++) {
             ...
             * STAR alone while still unconditionally advancing by one --
             * which leaves the SLASH ahead to be read and printed a
             * SECOND time by the following iteration. A class holding
             * both of the pair's bytes came out with the slash
             * duplicated in the printed pattern: harmless to the comment
             * itself (the inserted backslash still keeps it from closing
             * early) but wrong as a rendering of the pattern text.
             * Looking BEHIND at the byte just rendered instead, as
             * emit_comment_safe_byte does, cannot make that mistake --
             * each source byte is consumed by exactly one iteration. */
            if ((prev == '*' && ch == '/') || (prev == '/' && ch == '*'))
                pcrec_sb_printf(c, "\\x%02x", ch);
            else
    ...
    pcrec_sb_printf(c, " * %s does the work. %s and %s are\n"
...
void pcrec_emit_dfa_scan_stamps(Ctx *cx, StrBuf *c, const char *upper)
{
    ...
    pcrec_sb_stamp_str(c, upper, "DFA_PREFILTER", dfa_prefilter_name(cx));
     ...
     * caller to hand to `pcrec_sb_stamp_str` — routing it through the primitive
    ...
    pcrec_sb_printf(c, "#define %s_DFA_PREFILTER_OFFSETS \"", upper);
    ...
    pcrec_sb_stamp_str(c, upper, "DFA_SCAN_EDGE", dfa_scan_edge_name(cx));
...
static void emit_dfa_stamps(Ctx *cx, StrBuf *c, const char *upper)
{
    ...
    pcrec_sb_stamp_str(c, upper, "DFA_MATCH", dfa_match_name(cx));
