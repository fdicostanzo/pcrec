...
{
    sb_cmt_open(sb, PCREC_CMT_ESSENTIAL);
                          ...
                          const char *upper, const InfoStamp *st)
{
    ...
    /* [EMIT-VERB] abi 26 -> 27 (D76/D94; docs/dev/lanes/emitverb_report.md):
     * THE EMITTED-COMMENT AXIS. Every default artifact drops its
     * NON-ESSENTIAL prose (`-fcomments` restores it); no answer moves, no
     * struct offset moves and no `rx_info` member is added or changed.
     *
     * [EMIT-VERB rider, D112 item 2, same-day amendment] THIS BUMP'S LOG
     * ENTRY IS AMENDED, not re-opened: the ESSENTIAL generated-by header
     * line (`emit_pattern_comment`, above) now ALSO names the abi digit —
     * D112's essential class was specified as "pcrec AND THE ABI, and the
     * pattern echo" from the start, and the shipped line only carried
     * pcrec and the pattern. Riding the still-open 26 -> 27 bump rather
     * than taking a second one is D94's addendum shape exactly: the
     * comment TEXT changes (one line gains seven bytes, `" (abi 27)"`) but
     * `rx_info.abi`'s own VALUE, every struct offset and every answer are
     * unchanged, so this is scaffolding riding scaffolding. Comparison (A)
     * still does not move: the header line sits above `goto <prefix>_L0;`.
     * Comparison (B) is re-pinned in the same change this rider lands in. */
    sb_printf(c, "    .abi = %d,\n", PCREC_ARTIFACT_ABI);
