...
| write or change C under `src/`, `cli/`, `lib/` | `docs/dev/coding_guide.md` FIRST — the house disciplines, the taught primitives in their CURRENT state, the emitted-text/anchor-column rules, the altitude rubric and the do-nots, from the 2026-09-17 code review |
...
| change emitted scaffolding (comments, declarations, layout) | it IS an `abi` bump + identity-gate re-pin in the same change — D76/D94. The site list is EVERY READER OF THE NUMBER, FOUND BY GREP (grep the tree for the current abi number's readers: the `.abi` stamp, test expectations, every spec sentence, the gate's (B) pin) — a hand-enumerated "four sites" list missed a fifth reader in match_api.md (D94, 2026-09-01); run `make test-codegen` before delivering; then RUN THE SUITES THAT COUNT (registry, codegen, rxtsource) — a reader that never cites the number still moves with it (D94 addendum, 2026-09-19) |
