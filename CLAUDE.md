...
| change emitted scaffolding (comments, declarations, layout) | it IS an `abi` bump + identity-gate re-pin in the same change — D76. An abi bump has FOUR sites, all in one change: `src/gen/emit_dfa.c`'s `.abi`, `tests/codegen/run_codegen_tests.sh`'s [DD-14.FB] §10.4 expectation, `docs/spec/match_api.md` §6's "abi is N" sentence, and the gate's (B) pin — two lanes in one night missed the second and third; run `make test-codegen` before delivering |
