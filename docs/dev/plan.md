...
- [OPT-3] STATE:started (STEP 1 measurement lane launched 2026-08-26 ~12:0x; the report's per-subject rows now exist — reporter v4 — and half-answer it: on 1 MB of `a` the skip loop never runs, so the 3.26 ns/byte ≈ 11 cycles/byte IS the transition loop; inbox I-7 item 2) — THE LOOP'S THIRD OUTLIER, with Frank's
