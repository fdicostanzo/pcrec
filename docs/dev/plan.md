...
- [M6.0] STATE:started — milestone (EXPANDED 2026-08-18, thirty-third session, on Frank's standing ruling). Scope per the ruled list: assertions (\b \B, \A \z, (?m) multiline, \G, \K — module `assertions`), lookaround, backrefs, atomic groups + possessive-quantifier SPELLINGS (module `atomic-groups`; the possessify OPTIMIZATION already exists internally — this is the surface syntax, which is SEMANTICS, not an optimization), named groups (module `named-groups`). PREMISES RE-VERIFIED AT EXPANSION (constraint (a); probe on HEAD b95fbe6, 2026-08-18): \b \B \A \z \Z (?m) \G \K all refuse with module `assertions`; (?= (?! (?<= (?<! with `lookaround`; \1 \k (?P= with `backrefs`; (?> and *+ ++ ?+ with `atomic-groups`; (?<n> (?'n' (?P<n> declarations with `named-groups`; (?( with `conditionals`, (?R with `recursion`, (?| with `branch-reset` — those three are NOT in Frank's ruled M6 list, their refusals stand; \d, [[:alpha:]], (?i), (?s), and mid-pattern $ all COMPILE (the 2026-08-16 row correction holds). ABI groundwork verified shipped: `rx_info.groups` (`const rx_group_entry *`, "NULL until named-groups"), `nnames`, and the `rx_group_entry` ABI type all exist in the frozen M4 ABI — named-groups POPULATES an anticipated slot, no ABI change; the groups SORT KEY is the one deliberately-unfixed spec point (match_api.md §6) and is fixed by [M6.3]. INHERITED OBLIGATIONS travelling on substeps: D47.5 possessification-gate test rides whichever wave lands (?m) — see [M6.2]; D58 seam routing — every encoding-sensitive residue (lookbehind back-step, \G advance, caseless backref compare, word-character classification for \b) routes through src/gen/enc/ residual entries FROM BIRTH, never raw byte arithmetic in shared emitter code
- [M6.1] STATE:started — module `assertions` DESIGN GATE (design before code — this is the engine-touching substep). Produce docs/design/assertions_design.md answering, per construct (\b \B \A \z \Z (?m)^$ \G \K): (i) ENGINE SPLIT — which are DFA-representable as state context (DD-6's territory: last-byte-class context states for \b/\B, multiline ^/$ variants; cost vs the state budget, measured not asserted), which force the VM, which both engines carry; (ii) DD-4 — \G vs startpos semantics (note engine_m4.md §7.3: `nfa_wrap_unanchored` bakes in the self-loop with no toggle, confirmed STRUCTURAL); (iii) DD-11 — which newline convention (?m) and \Z bind to, and where that axis is declared; (iv) \K vs the match-start reporting contract and the DFA reverse pass; (v) D58 RESIDUE ENUMERATION — name each construct's encoding-sensitive residue and the seam entry it routes through (word-char classification is a CHARACTER question; \G's advance; nothing lands as raw byte arithmetic); (vi) the D47.5 gate design for (?m); (vii) module gating — all constructs under module `assertions`, partial-enable behavior. D6 adversarial panel on the design BEFORE implementation starts. **DESIGN DELIVERED 2026-08-18 (asrtdesign lane): docs/design/assertions_design.md + docs/design/assertions_measurements/ (4 probes, outputs archived). The PANEL is what closes this row.** Headline answers: every construct is an absolute POSITION TEST, a NEXT-BYTE VIEW (folds into the transition/accept tables by byte class), or a PREVIOUS-BYTE CONTEXT BIT (folds into the DFA state identity) — the forward and reverse machines swap which is which; \K is none of them and stays VM-only (the registry already ships that row VM_ONLY). \A and \Z are EXACT ALIASES of the shipped A_BOL/A_EOL nodes — parser rows, no engine work; \z needs a third closure view, interned only when it differs, so a \z-free artifact stays byte-identical BY CONSTRUCTION rather than by a flag. DD-4 answered WITHOUT engine_m4.md §7.3's wrap toggle: ENG_ATTEMPT already emits the un-self-looped shape and \G is `start_max = startpos`, a third value for a string the emitter already picks between. (?m)^ routes to ENG_ATTEMPT because it needs strictly MORE than plain ^ (D8's deferred reverse BOT variant PLUS a byte-selected view) — [DD-7] gains a second consumer, still parked. TWO PREMISES RE-MEASURED ON HEAD AND ONE REFUTED: (?m) already refuses with module `assertions` (src/parse/mod_modifiers.c:280, not `modifiers`), so question (vii) needs NO re-attribution work; and python3 `re` is the WRONG ORACLE FOR \Z — python's \Z is PCRE2's \z, measured 1 of 7 cells disagreeing in the silent direction (python no-match where PCRE2 matches), so \Z expectations must ride the libpcre2 differential and the wave brief must say so. **LIVE LANDMINE FOUND: D47.5's gate is BUILT AND SHIPPING AND IS SCOPE-BLIND** — src/opt/possessify.c:579 captures cx->mods.multiline ONCE, and pcrec_possessify runs from select_engine.c:145 AFTER the parse, so the value is the END-OF-PATTERN state; `(?m:a{0,4}$)` and `(?m)a{0,4}$(?-m)` would EXEMPT a multiline `$` the day the `m` letter is accepted, and D47.5's own recorded test obligation exercises the ONE row the shipped code gets right. MEASURED as cells (probe_d475_scope.py, both oracles): 2 of 5 miscompile, and the failure is a LOST MATCH not a shifted span -- `(?m:[^c]{1,3}$)` and `(?m)[^c]{1,3}$(?-m)` on "a\nc" are (0,1) correct and NO MATCH possessified; the one shape D47.5's own obligation names is the one the shipped code gets right. THE REQUIREMENT (stated separately from the spelling): scoped modifier state is resolved AT PARSE TIME onto the node; no post-parse pass reads cx->mods. SPELLING recommended: a distinct node kind over a flag, decided on a MEASUREMENT rather than taste -- a new AKind enumerator produces 15 -Wswitch warnings across 6 files (probe enumerator added, tree rebuilt, warnings counted, edit reverted; 15 of 19 switch sites carry no default:) where a new struct field produces none, so a flag's failure mode IS the silent bug being fixed while a node kind's is a build diagnostic. AND the lane then found this is NOT a new convention but pcrec's OWN NAMED RULE: src/opt/mrl.c:18-24 (R26 V7) already mandates default:-less exhaustive switches so "a node kind added after this file is written must be a COMPILE ERROR here... exactly the alarm the analysis cannot otherwise raise", with src/opt/altcls.c:405 citing it as "mrl.c's rule" -- and that sentence describes possessify.c's situation word for word. The manager leans FLAG (house pattern: r->greedy at parse.c:908) and the doc records the disagreement for the panel. FRANK'S (b) ANSWERED NO, with my own grep: `mods.` appears in exactly TWO files in all of src/ -- parse.c (10 reads, every one AT PARSE POSITION) and possessify.c (the one post-parse read); every other modifier already resolves onto the AST as it is built and is scope-correct by construction. STRUCTURAL ENFORCEMENT PROPOSED so the answer is durable: after the fix cx->mods has zero legitimate consumers outside src/parse/, so ModState leaves Ctx and becomes parse-driver-owned, turning the invariant from a discipline rule into a compile error (blast radius: the two compile.c seed sites at :117/:324, the Ctx member, the src/parse/ save/restore sites -- nothing in ir/gen/opt/cli). Schedulable in wave A while it is all provably a no-op. REPORTED, NOT OWNED: ENG_ATTEMPT's `for (start = startpos; start <= start_max; start++)` (src/gen/emit_dfa.c:1108) is an external byte-arithmetic advance loop in shared emitter code — D58's "the hot path has NO external advance loop" rationale is true of ENG_UNANCH only — and this module makes it more prominent because (?m)^ and \G both route patterns onto that engine; belongs to [DD-12]/M5, not [M6.2]. STATE BUDGET MEASURED TWO WAYS as the row requires: EXACTLY in pcrec (the word-set alphabet refinement, \b's prerequisite, costs at most +1 equivalence class on 38 realistic patterns and +2 on 574 .rxt ones; word+newline at most +3; largest states*ncls after both = 48,012 against PCREC_MAX_TABLE_ENTRIES 2,000,000) and by a CALIBRATED PROTOTYPE for the context bit's state cost (minimised ratio 1.00x/1.11x/4.75x, reproducing pcrec's own state count on 29 of 33 assertion-free arms, its one fidelity gap — no priority pruning — stated in the file). The states*ncls accept table those views force MEASURES AT ZERO hot-path cost on D11's own shape (197.5 vs 202.8 MB/s, answer-preserving variant, identical match counts). D11's SKIP HAZARD is the module's most dangerous item and is designed for: a class-indexed accept makes the existing skips unsound in a way the n-1 bound does NOT fix, cured by intersecting the skip bitmap with "and this state does not accept on this byte" at compile time. Proposes a FIVE-WAVE [M6.2] structure (A \A/\z/\Z + the gate refactor; B \b/\B; C (?m); D \G; E \K), five BELIEVED claims each with its refutation experiment, seven things NOT measured headed by the REVERSE machine's state cost, and EIGHT open questions for Frank headed by whether DD-11's NEWLINE CONVENTION axis is declared now on --encoding's per-pattern refuse-by-name precedent (recommendation: yes — it is a semantic namespace, not a D18 optimisation axis). Also flags that eng_brep_design.md §2.5's cited 0/720 and 180/720 are STALE: re-running that section's own probe gives 0 of 168 and 12 of 168 on the greedy population, same qualitative result, both oracles agreeing
- [M6.2] STATE:not-started — module `assertions` IMPLEMENTATION, in the wave structure [M6.1]'s design fixes (blocked: [M6.1] design + panel first). D47.5's possessification-gate test lands WITH the multiline wave — a `(?m)` pattern whose `$`-follow bounded quantifier must NOT possessify (eng_brep_design.md §2.5's exemption is safe only under a LIVE !multiline check; 180/720 cells diverge under (?m)). D27 blinded corpus for the module rides the close
- [M6.3] STATE:started — module `named-groups`: the three declaring spellings (?<n>...) (?'n'...) (?P<n>...) parse and capture as their group number; duplicate-name policy decided against the PCRE2 default and oracle-verified (DUPNAMES/(?J) is out of scope — clean refusal); `rx_info.groups` populated (sorted, bsearch-able — FIX the sort key and resolve spec §6's "not yet fixed" paragraph in the same change, re-quoting under the verification ledger) and `nnames` counts it; NO ABI change (the slot is anticipated — see [M6.0] premise note). Backref-BY-NAME spellings (\k<n>, (?P=n)) STAY in module `backrefs` and must still refuse cleanly with named-groups enabled. Oracle note for the lane: python `re` speaks only (?P<n>...) — spelling-translate for the oracle or route through the libpcre2 differential per docs/testing.md. D27 blinded test author (cell) writes the corpus from the PCRE2 goal
...
- [M6.5] STATE:not-started — module `backrefs`: VM-forcing (a backref is not DFA-representable); numeric \1..\99 with the octal disambiguation the parser's refusal already hints at, \k spellings, (?P=n) once named-groups is in; CASELESS BACKREF COMPARE is D58-named residue — routes through a seam entry from birth
