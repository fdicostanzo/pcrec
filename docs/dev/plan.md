...
- [ENG-ABS] STATE:started (OPENED 2026-08-28 ~22:3x, forty-fourth session, on Frank's "Go 1&2" — the SECOND MECHANISM only: anchored match-here via the UNWRAPPED forward DFA, lane engabs (opus, worktree lane/engabs); [OPT-2] closes into it. The FIRST mechanism, `^`-absorption into ENG_UNANCH, stays gated on [BENCH-1]'s `^`-on-some-branches case and is NOT opened) — formerly STATE:not-started — ENG_UNANCH absorbs `^` (the DD-7
