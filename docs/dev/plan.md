...
- [K38-FIX] STATE:not-started — the VM emitter's fixed-size name buffers truncate at long prefixes (known_issues K38, 2026-08-26): a 60-char prefix — the documented maximum — yields uncompilable C. General fix: one `PCREC_MAX_PREFIX_LEN`-derived buffer size for every emitted name; a tests/cli witness that emits and COMPILES a max-length-prefix artifact on both engines. Sonnet; the identity gates must stay byte-identical for `rx` (the buffers only grow).
...
- [SPEC-1.4] STATE:not-started — match_api.md patches (survey: D3 pointer, C2 — NOW the DFA stamps' statement, shipped in §6.3 by srStamp: verify and drop the caveat, C4 D76 abi paragraph, F6 byte-only encoding lead, F9 whole-subject subsection) with a dated revision-log entry. Careful lane; after 1.1.
...
- [OPT-3] STATE:not-started — THE LOOP'S THIRD OUTLIER, with Frank's
