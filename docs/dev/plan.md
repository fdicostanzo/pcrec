...
- [K38-FIX] STATE:started (lane srK38, sonnet, 2026-08-26 ~12:0x) — the VM emitter's fixed-size name buffers truncate at long prefixes (known_issues K38, 2026-08-26): a 60-char prefix — the documented maximum — yields uncompilable C. General fix: one `PCREC_MAX_PREFIX_LEN`-derived buffer size for every emitted name; a tests/cli witness that emits and COMPILES a max-length-prefix artifact on both engines. Sonnet; the identity gates must stay byte-identical for `rx` (the buffers only grow).
...
- [SPEC-1.4] STATE:started (lane srK38 after K38-FIX, sonnet, 2026-08-26 ~12:0x) — match_api.md patches (survey: D3 pointer, C2 — NOW the DFA stamps' statement, shipped in §6.3 by srStamp: verify and drop the caveat, C4 D76 abi paragraph, F6 byte-only encoding lead, F9 whole-subject subsection) with a dated revision-log entry. Careful lane; after 1.1.
...
- [OPT-4] STATE:not-started — the VM hybrid's inlined DFA PREFILTER scales with a bounded-repeat count (K39, found 2026-08-26 when [ENG-BREP]'s size ceiling went red by 6 lines of slack): `((a)|b){0,4000}c` 1,994 lines vs 869 for {0,400} at the default, 573 at any count with the prefilter off. Candidate general fix: build the candidate-start DFA from a count-INDEPENDENT language (the first-byte class / a count-collapsed pattern); answers unchanged (D46: the prefilter axis is answer-identity-preserving), the identity gates the control. Build when a bench row shows the cost (D77); the size checks now print the auto sizes so the number stays visible.
- [OPT-5] STATE:not-started — THE LOOP'S FOURTH OUTLIER, from the bench's
