...
- [DD-13c] STATE:started (2026-08-25 ~20:4x, lane srStamp2, opus) — THE STAMPS' SCOPE (r37 A5/A6) + the RUNTIME MIRRORS (Frank, D40 addendum): `RX_DFA_SCAN "empty"` for the four no-loop artifacts; VM HYBRIDS (1,263 of 1,488 VM artifacts) stamp their inlined DFA scan's `RX_DFA_SCAN`/`RX_DFA_PREFILTER` from the same derivation, the check's leak assertion becomes an iff; `struct rx_info` gains `scan` and `prefilter` string fields (appended; NULL where n/a; one derivation feeds macro and field); abi 4→6 (srTier takes 5 first), (B) re-pinned, (A) unchanged. Interim (536180c, before the rx_info addition): run_dfa_stamps.sh 26/0, codegen 105/0, gate 15/0 + refusal on the old pin, the DFA-only prefilter distribution unchanged at 380/327/176/61/51.
