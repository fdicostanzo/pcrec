...
- `form_char_step0.md` — [FORM-CHAR] STEP 0 + [OPT-CLSPACK] STEP 0 (2026-09-04, lane form0, measurement only, nothing under `src/`/`tests/`/`docs/spec/`): is a caseless CLASS test faster than a folded COMPARE, and is a 256-byte table load faster than the 32-byte bit array's load+shift+and, on both engines, cache footprint counted. Four hand-twin families, each a mechanical transform of an emitted base artifact's own class-bitmap or scan-edge tables (byte sets parsed off the compiler's OWN emitted text, never re-derived from the pattern), correctness-checked per twin against its base and sized (`.text`/`.rodata`, `gcc -O2 -c`) under the evening box hold — **all size, no timing** (owed post-`.lift`). (A) VM literal chain `abcdef` caseless, 6 sites: `fold` wins both axes (-38% `.text`, `.rodata` deleted entirely), `table` loses both (+31% `.text`, 8× `.rodata`), `atom` is between. (B) a general (`[a-zA-Z0-9_]`, 4 runs) and sparse (`[aeiou]`, 5 singletons) class, 1 site each: `table` loses `.text` at both; `rangecmp` is a near-wash on general (+2.8%) and a clear win on sparse (-5.6%). (C) the DFA scan edge — a small witness (`(?i)a{2,40}Z`, 4 sites) and `ci-256` (pcrec-bench's own bitmap-edge witness, read-only, 8 sites, every site confirmed a case-fold pair): the "bitmap" 256-byte body costs `.text` at both scales (+24-25% small, +0.8% at ci-256's scale) and all the `.rodata` `range`/`fold` need none of; `range` and `fold` are within noise of each other because every measured site happens to be a fold pair. (D) N=16 distinct classes (`abcdefghijklmnop` caseless VM, above the plan row's ~10-class estimate): the shared atom table is SMALLER than both alternatives on `.text` (-41%/-54%) as well as `.rodata` (2×/16× smaller) — not a space-for-time trade on this axis, a plausible CSE mechanism named but not disassembly-confirmed. Recommends AGAINST the 256-byte table form everywhere measured (loses `.text` in all four families) pending timing; names `fold` and `atom`@N≥16 as the two candidates worth a real build if timing agrees. Owes: the timing run itself, a disassembly read of family D, and a family-C witness whose scan-edge set is not a case-fold pair.
