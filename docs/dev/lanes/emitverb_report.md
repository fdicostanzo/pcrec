...
## 7. Rider: the abi in the generated-by line (lanes emitverb2 → emitverb3 → the manager's chain, 2026-09-19)

D112 item 2 names the essential header line as "the generated-by header
naming pcrec and the abi"; the shipped line named pcrec and the pattern
only (the abi lived solely in `rx_info.abi`). The manager ruled the open
abi-27 bump is the event (no second bump). The line is now

    /* Generated by pcrec (abi 27). Pattern: <pattern> */

with the number from `PCREC_ARTIFACT_ABI` in `src/gen/emit_dfa.c` — ONE
constant shared by the two sites that spell it (this line and
`rx_info.abi`'s initializer), so the header and the field cannot drift
(WIP 6db9e552). The D107 detector caught the new name-invisible constant
on its first run; it is allowlisted as a schema version (030acf40).

**Readers re-aimed** (by grep for the old wording, then by the suites that
count): `run_codegen_tests.sh` (two header-comment sites; `ag_strip`'s
...
**Byte delta** (`scripts/emit_sweep.py --ref d672757f`, self-check passed
at full reach 3,517 / 3,518 / 3,518 / 32-96): c-default 3,517 movers,
c-vm 3,518, composition 96 — every artifact, by exactly the 9 bytes of
` (abi 27)`; emit-ir-vm 0, dumps 0. `.o` identity: not re-sampled by the
manager — the change is inside the ESSENTIAL comment region, emitted
identically under both settings, and `run_comments_axis.sh` (65/0) asserts
the comment-excluded size identity per case; a comment cannot reach the
object file, and the event-2 proof (3,517/3,517) stands for the region it
lives in.

**Counting suites on the branch** (the manager's chain, log
`rider_chain.log`): registry 225/0, PC-3 209/0, 108/0, 24/0, 54/0; cli
284/0; rxtsource 212/0/1 recorded; `make test-codegen` every script green
bar the standing darwin `nm` probe (comments_axis 65/0, comment_escape
9/0, 109/0, 31/0, 22/0, 31/0, 7/0, 14/0, 13/0); strict clean.
`run_specimen_identity.sh` reads 5/10 on THIS branch because the branch
pre-dates evtriage's fix (8d513b4d, on main) — it reads 13/0 after the
merge. `run_recursion_identity.sh`'s (B) file pin is re-pinned by the
manager at the merge commit (D76).
