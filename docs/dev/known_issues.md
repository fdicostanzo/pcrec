...
**Two causes, measured:**
1. **The size-cap retry ladder rebuilds everything, 3×.** `compile_driver`'s retry loop (src/core/compile.c:826) re-runs parse→NFA→forward+reverse(+anchored) DFA→emit from scratch per rung (compile.c:1213 drop_eligible / K53-SELRETRY; compile.c:1269 premul_eligible / K59-PREMUL). Neither rung changes forward/reverse construction. `--max-emit-bytes` raised (no retry, `selected`) = 26.32 s; default (2 rungs) = 77.49 s. GENERAL (any pattern that takes the ladder), so filed separately as plan row **[OPT-RETRY-REUSE]**; K67 is its measured witness.
2. **The 26 s floor is subset construction** — 99.3% of samples in `make_state` (src/ir/dfa.c:1225) → `closure()`/`clo_walk()` (dfa.c:864/643). `\p{L}` is 677 UCD intervals (src/parse/uprops_tables.inc:3099); under `+` the 677-branch fan-out is re-walked at every loop-boundary DFA state (per-cell BFS calls 75,200→104,900, 1.4×; time 700×). Instrumented (lane plwhy): 92% of closure visits take the loop-context hash-memo path (~466 ns vs ~6 ns) — GENERAL, filed as plan row **[OPT-CLOSURE-CTX]**; the class-specific share (flat 600+-branch utf8 lowering) stays with [CLS-TREE].
...
## K43 — INFRASTRUCTURE (2026-09-01, forty-ninth session, found by battery 8's `make test LINTGEN=1` stage): the LINTGEN axis is RED on this box's gcc 15 — `-fanalyzer` false positives (CWE-457) on VM artifacts whose driver calls the anchored entry, plus the traced-artifact and rung-boundary build cases **DIRECTION (b) REFUTED BY MEASUREMENT (2026-09-03 22:0x EDT, lane edge1, gcc 15.2.0 -O1 -Wall -Wextra -Werror -fanalyzer, 30 patterns, two arms): the designated-range initializer fixes K43's own site (7 artifacts error → clean) and CREATES a second, distinct CWE-457 at the TRAIL REWIND (`slot_values[run->trail[run->trail_depth].slot_index] = …`; 2 artifacts clean → error: `((a)|b)*`, `(?:(a)|(b))*c`) — making the slot array provably initialized lets the analyzer push into the rewind loop, where nothing proves entries below trail_depth were written; the same trick would be the O(cap) per-call page-touch [OPT-1] removed. main and the branch are exact inverses on `(a*)*` vs `((a)|b)*`; LINTGEN is red either way, and (b) costs 578 B on every VM artifact (moving [ART-SIZE]'s bar). RULED (manager, pending Frank): (b) DROPPED from lane/edge1 (the branch is [OPT-EDGE] STEP 1 alone); K43 returns to Frank with the live options (a) per-site suppressions / a documented exclusion list, (c) a newer gcc.** **RULED (a) 2026-09-04 08:4x (Frank; D95): a documented exclusion list in the LINTGEN runner naming the false-positive shapes with the diagnosis beside each; the accepted risk recorded in D95; re-measured whenever gcc moves; K48 lands in the same pass.**
...
## K46 — INFRASTRUCTURE (2026-09-03, fifty-first session, found by lane isl1 during r53's landing pass): the Makefile does not list src/core/limits.def as a dependency, so adding a limits row and rebuilding can ship a binary whose `--list-limits` dump disagrees with its own table. WITNESS: after adding two rows (manifest 45 → 47) the first rebuild's dump still printed 45; caught only because the lane checked the dump rather than trusting the build. **STATUS: FIXED on branch `lane/ccd2` ([CC-DIFF] STEP 2, 2026-09-04); CLOSES AT MERGE.** `src/core/limits.def` joined the `$(BUILD_DIR)/obj/%.o` pattern rule's prerequisites in `Makefile`, beside `src/core/limits.h` and `src/parse/cls_bits.inc`, with the history in the comment there. RISK, as filed: a stale build's registry dump lies silently — the registry check (tests/registry) reads the dump, so a stale build could pass a check it should fail.

**IT WAS RE-DISCOVERED INDEPENDENTLY THE NEXT DAY, BY A DIFFERENT WITNESS, AND THAT IS THE PART WORTH KEEPING.** isl1's witness is a COUNT (the manifest read 45 after two rows were added, so the dump was short). ccd2's is a VALUE: it moved `VM_INLINE_CHAIN_MAX_BYTES` from 20,000 to 4,096, the EMITTER honoured 4,096 — `src/gen/emit_vm.c` had changed in the same edit, so that object was recompiled — while `pcrec --list-limits` still printed 20,000. **One binary carrying two values of one constant**, with the artifact and the registry dump disagreeing about the number that chose the artifact's shape. The count witness needs a row to be ADDED; the value witness fires on any edit to an existing row, which is the commoner operation. Neither lane could see the other's.

**AND IT IS THE SAME DEFECT THE RULE'S OWN COMMENT ALREADY WARNED ABOUT, ONE FILE LATER.** The comment immediately above the pattern rule records `src/parse/cls_bits.inc` joining the prerequisites at MOD-0.3e, *"found the hard way: a PC-4 bitmap sabotage produced ZERO disagreements because the edited .inc never entered the binary — hand-maintained header deps must grow with every new include"*. `[LIM-1]` then introduced `limits.def` and did not add it. The general fix — automatic dependency generation (`-MMD -MP`), which retires the hand-maintained list entirely and is portable GNU make, so D2's plain-makefile rule is no objection — is NOT done here and is filed for whoever owns the build: it is the only change that stops this recurring a third time.
