...
- `2026-09-23-r1-var-design.md` — three-critic panel (mech/spec/test, opus) on the [VAR] design (variables_common.md, variables_pattern.md, replace_design.md, variables_roadmap.md; lane vardesign). 2 blockers (one OPEN to Frank — a var-bearing artifact's `<prefix>_match` is no longer an `rx_matchfn` and the note contradicts itself about it), 12 major, 4 medium, 6 minor, 6 note. The central claim (a pattern variable is a backreference whose span comes from the caller) held; what fell was the note's own byte-identity and "no code in select_engine.c" claims (the PREFILTER decline needs a third whole-tree predicate the note never named), citation accuracy (a stale corpus count, a mis-attributed D41.1 ruling, a retired function name), and MVP test-measurability (no mechanism existed to bind a `.rxt` case's variable to a compiled artifact by name — the names table promoted from an open question into the MVP; no oracle design existed for pattern-side match semantics — QUOTEMETA-SPLICE + a labeled second-order backref-equivalence differential added). Fix round applied in the same change (lane varfix), verified by grep, including one stale count the compile itself caught mid-delivery.
