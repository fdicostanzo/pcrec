...
|---|------|----------|---------|-------------|
| E1 | engine | BLOCKING (refusal regression) | Two computations of a splice's \|W\| disagree: `spl_nw[i]` (emit_vm.c:7305-7316, capture half only) sizes the reservation at vm_count_slots (:2436); `u.call.nsave = rgn_nw[i]` (:7448-7500) unions in reached LINKED targets' seven per-copy families; vm_splice allocates from the latter (:5845) and :5852 fires. `(?:(a{2,5}(?1)?b)((?1)c)){0}(?2)` → "splice save block overflowed (7 of 6 slots)"; compiles with `-fno-splice-calls`. Corpus: 113 artifacts SPLICED>0, 37 LINKED>0, **0 with both** — every bar structurally blind. | FIXED 99eecd5 — decided from the semantics: a reached LINKED region's own RX_RETURN restores its W on EVERY activation, so a splice's save block is the capture half (pairs + PENDING by group number); `rgn_grp[i]` is the one set, `spl_nw` its count and `rgn_w` its indices, asserted equal at the W build. Cells: tests/recursion/bothlinkage.rxt 4 blocks/22 cells, both arms 22/0; codegen rule 2b asserts the BOTH-linkage population ≥ 4 read off artifact stamps. RE-VERIFIED by critG-engine from an archive build: resume-frame retreat into the linked callee HELD (`(?:(a{1,2}(?1)?b\|ab.)((?1)c)){0}(?2)` on `abzc` → [0,4]); quantified site, link→splice→link chain, PENDING coverage read off the artifact, assertion fires under sabotage, 120+100 biased-fuzz pairs all both-linkage, 0 divergences. Residual nits sent 02:0x: same-source assertion comment; widen the cells to the three fuzz-only shapes. |
...
## Verdict
Both critics: merge-with-fixes. E1 discharged at 99eecd5 (re-verified). Merge on the sweep count + the final make test.
