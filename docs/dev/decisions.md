...
### D111 addendum — the header ship shape as DELIVERED (lane w4, 2026-09-19): the bit is spelled ONCE, in `lib/pcrec.h`; `axes.def` names it as a TOKEN

The lean above (a generated block in the public header regenerated by
`make gen-tables`, plus a drift check) was priced by the lane at 650-700
lines against the brief's ~150-line stop threshold — the flags enum is
578 lines for 26 enumerators, ~552 of them the per-bit prose that IS the
header's documentation — and declined per that stop condition. What
ships: a `PCREC_AXIS(deny_macro, deny_flag, force_macro, force_flag,
default_state)` row names the bit by its enumerator TOKEN and the X-macro
stringifies it where a name is wanted, so the value lives only where it
is a contract (the public header, byte-identical across the wave) and the
table holds no copy to drift from. The reference is compiler-checked
(an unknown token fails the build); the seam the lean worried about is
already covered by `axes_registry_check.sh`'s pre-existing arm (every
`1u << N` bit in the header must be named by `--list-axes` on some row),
driven red by deleting a row. Accepted by the manager: smaller in every
column, and stricter — no generator, no marker, no second check. The
`default_state` column ships with no reader on purpose: [EMIT-VERB]'s
`-fcomments` row (D112) is its first customer.

### D94 addendum — after the grep, RUN THE SUITES THAT COUNT (lane w4, 2026-09-19; second recorded instance)

The re-pin ritual finds a moved number's readers by grepping for the
number. It is structurally blind to a reader that never CITES the number
but moves with it: `run_registry_tests.sh` guards `limits_check.sh`'s
PASS-LINE COUNT as the literal `22`, and wave 4's item 2 took that count
to 24 without touching any text the grep could find — item 4's registry
run caught it, one commit late (95537791). First instance:
battriage_report.md's EMITTED_BYTES manifests after the abi-25 label fix.
The ritual gains a line: after the grep, run every suite that COUNTS
