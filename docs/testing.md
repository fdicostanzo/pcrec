...
| `cli/main.c` | `test-cli`, `test-examples`; also `test-reject`/`test-registry` if the change touches how errors or `--list-syntax` are surfaced | cli/'s own suite is the CLI-surface test; `test-examples` is the one section that builds `examples/makefile/` with a real `make` — a parser change to the operand/`-o` grammar shows up there even when no `.rxt`-corpus answer moves; the other two invoke `build/pcrec` as a subprocess and would show a broken diagnostic path |
...
LOW bound is the one doing real work: bits below 4 are unrelated `1ull << N`
...
from `lib/pcrec.h`'s `1ull << N` constants and `cli/main.c`'s flag-parsing
