...
events but never stated the general rule; `rx_info.abi` is `14`
([DD-13b.W1.2], `rx_info.name` + `nentries`; it read `6` when this note
...
- **`rx_info.abi` is `14` on every artifact today ([DD-13b.W1.2] bumped
  it from 13 by APPENDING `name` and `nentries` to this struct — two
  initializer lines on every artifact of both engines, no struct offset
  moved, no emitted program byte moved and no stamp VALUE changed; `13`
  was [OPT-5]'s DFA scan edge, which added `<PREFIX>_DFA_SCAN_EDGE` (§6.3) to every artifact and,
