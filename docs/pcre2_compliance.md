...
| `\b \B \G` | `REJECTED` | — | module `assertions`, waves B and D. `\b`/`\B` need a one-byte lookbehind, which is a state-context change (same machinery as DD-6), plus runtime start-state seeding from a byte outside the search window at `startpos > 0` (assertions_design.md §3.8). `\G` additionally collides with `nfa_wrap_unanchored`'s baked-in start self-loop — that is DD-4. With the module ENABLED each refuses BY ITS OWN NAME rather than claiming the module is missing (§9.2), which would be a lie once it is enabled |
