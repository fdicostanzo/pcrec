...
**BUILT** (`tests/counterk/counterk.rxt`, generated by
`../counterk_impl/make_corpus.py`): 31 patterns, 1,634 cases, 0 failed, riding
`test-corpus`. Oracle plumbing IMPORTED from `../possessify_impl/gen_rxt.py`
rather than copied, so it inherits that file's ovector instrument note.

**AND THE HIGH-COUNT BLOCKS FOUND SOMETHING: libpcre2 CANNOT COMPILE THE
ENDGAME SHAPES AT ALL.** Bisected, the boundary for `((a)|ab){0,N}c` is exactly
**N = 2047**; at 2048 it answers error 120, *"regular expression is too large"*.
The cause is the one this rung exists to remove — **PCRE2 replicates a
capture-bearing quantified group exactly as pcrec's frames rung does**, so its
compiled-pattern limit and pcrec's `PCREC_MAX_VM_REPEAT_COPIES` are two
spellings of one constraint. The counter rung lifts pcrec's; PCRE2's stands.

So at the endgame counts **pcrec compiles a pattern PCRE2 refuses**, which is a
capability difference rather than an oracle disagreement, and D26 is the frame:
PCRE2 is the source of truth for what a pattern MATCHES, and here it has no
opinion because it declined to have one. Those blocks fall back to python alone
with the reason ASSERTED per block — the generator re-checks libpcre2 and exits
FATAL if the pattern compiles after all, or if it declines for any code other
than 120, because a semantic refusal would be a finding and not a fallback.

**It produced a VACUOUS BLOCK first, which is why this is code and not a
comment.** `gen_rxt.emit` treats UNCOMPILABLE-versus-an-answer as a
disagreement and DROPS the cell — right when the oracles contradict each other,
wrong here — so the first run emitted the endgame blocks with a `pattern` line
and **no cells at all**: blocks that read as coverage in a listing and assert
nothing. §3.6's rule is INVESTIGATE, not filter, and the investigation is what
turned a dropped cell into a recorded capability difference.
