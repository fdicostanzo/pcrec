...
| **V-6** | §11's A2 budget covered the `-Wswitch`-visible arms and not the FOUR kind switches carrying a `default:` | tabulated and **each inspected**: `revdet.c:370` widens to all bytes (sound), `emit_vm.c:1132` declines (sound), `:1176` returns −1 (sound, gated), `:3132` is a hard `pcrec_ctx_fail` (sound and loud) |
...
| `src/gen/emit_vm.c:3132` | `vm_rev_emit` (`:3011`) | falls to `pcrec_ctx_fail("internal error: bad AST node in the backward walk")` | **SOUND AND LOUD** — a hard compile error, not a silent accept. It is reachable if a lookaround ever reaches the backward walk, which is why revdet's arms must DECLINE rather than descend |

**AMENDED 2026-08-23 AT WAVE A2's LANDING (lane/waveA 549e2e2, merged 1187973)** — three corrections from the landed code: (1) the emit_vm.c row's five arms are `vm_emit`, `vm_nullable`, `vm_cost`, **`vm_rev_caps`** and **`vm_count_slots`** — `vm_cuts` is not a switch on HEAD (`return a->u.rep.possessive || under_atomic;`) and `--emit-ir` renders the VEvent stream, not the AST, so a lookaround's listing is whatever `vm_look` records as events; the COUNT of 23 was exact, file for file. (2) The four `default:` line numbers above are pre-D70 and drifted again with this wave — find the sites by function name. (3) `int look_widths[]` is confirmed unbuildable inside the union; landed as `const int *widths` (sizeof(Ast) 72 unchanged, sizeof(u.look) 24). Also landed: `PCREC_W_UNBOUNDED == PCREC_MINW_MAX` so it composes with the saturating arithmetic (`sat_mul(UNBOUNDED, 0) == 0`); A_CLASS's maxw arm of 1 byte is EXACT only while the UTF-8 encoding refuses by name — a cross-milestone obligation recorded at both functions. Wave B+C owes, at the site: `vm_emit`'s pcrec_ctx_fail → `vm_look`; `vm_count_slots`' A_LOOK arm gains the lookaround's slot families IN THE SAME EDIT (under-counting = an out-of-bounds write in emitted code, K27's class); `vm_cost`'s +1 frame / +2 trail re-checked against `vm_look` as landed.
...
to `built` while every accepted pattern still hits the emitter's `pcrec_ctx_fail` —
