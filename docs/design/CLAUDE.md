...
- `tt4m_harness_batching.md` — [TT-4M] STEP 2 (2b) (2026-09-08, lane
  ...
  arm to check, not assumed; ROLLOUT recommends opt-in
  (`HARNESS_BATCH=N`) first, converging to default-on (not
  opt-in-forever like `CCACHE=1`) once a full `make test` is
...
- `oracle_interface.md` — **PROPOSED, NOT PANELED** (2026-09-10, lane
  ...
  **WIRED 2026-09-10 (lane `orwire`)**: `tests/uprops/uprops_compare.py`'s
  ...
  as a counted coverage split, never silently dropped. PC-3/PC-4 remain
  unwired (§9 Step 3, a later rung); the C3 python tier's Step 2 landed
  the SAME NIGHT via lane `pyrole` — see `c3_three_way.md` immediately
  below. See `docs/dev/lanes/orwire_report.md`.
