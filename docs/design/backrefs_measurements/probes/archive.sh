#!/bin/sh
# [M6.5.1] Run one probe and archive its output with FULL PROVENANCE.
#
...
#   e.g. archive.sh z_oracle.txt docs/design/backrefs_measurements/probes/probe_z_oracle.py
set -e
OUT=$1; PROBE=$2; shift 2

REPO=$(git rev-parse --show-toplevel)
DEST="$REPO/docs/design/backrefs_measurements/out/$OUT"
cd "$REPO"
...

{
  ...
  echo "# host       : $(uname -sr)"
  echo "# ------------------------------------------------------------"
  echo "# Evidence for the [M6.5.1] panel, never an oracle: no check reads this"
