#!/bin/sh
# [M6.1] Run one probe and archive its output with FULL PROVENANCE.
#
...
#   e.g. archive.sh z_oracle.txt docs/design/assertions_measurements/probes/probe_z_oracle.py
set -e
OUT=$1; PROBE=$2; shift 2

REPO=$(git rev-parse --show-toplevel)
DEST="$REPO/docs/design/assertions_measurements/out/$OUT"
cd "$REPO"
...
{
  echo "# ============================================================"
  echo "# ARCHIVED PROBE OUTPUT — [M6.1] module \`assertions\` design lane"
  echo "# ------------------------------------------------------------"
  echo "# PROBE      : $PROBE"
  echo "# ARGS       : ${*:-(none)}"
  ...
  echo "# host       : $(uname -sr)"
  echo "# ------------------------------------------------------------"
  echo "# Evidence for the [M6.1] panel, never an oracle: no check reads this"
