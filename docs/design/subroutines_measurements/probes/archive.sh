#!/bin/sh
# [DD-14] Run one probe and archive its output with FULL PROVENANCE.
#
...
#   e.g. archive.sh spellings.txt docs/design/subroutines_measurements/probes/probe_spellings.py
set -e
OUT=$1; PROBE=$2; shift 2

REPO=$(git rev-parse --show-toplevel)
DEST="$REPO/docs/design/subroutines_measurements/out/$OUT"
cd "$REPO"
...
{
  echo "# ============================================================"
  echo "# ARCHIVED PROBE OUTPUT — [DD-14] module \`recursion\` design lane"
  echo "# ------------------------------------------------------------"
  echo "# PROBE      : $PROBE"
  echo "# ARGS       : ${*:-(none)}"
  ...
  echo "# host       : $(uname -sr)"
  echo "# ------------------------------------------------------------"
  echo "# Evidence for the [DD-14] panel, never an oracle: no check reads this"
