...
print()
print("# W4 -- WITHDRAWN AS A REASON (R34 V-6). `^q(?>(a|ab))?z(?1)c$` was")
print("#       offered as \"the wrapper never ran lexically\". It is a real")
print("#       match but a NON-REASON: the optional group IS EMITTED, merely")
print("#       not executed on that subject, so there is code to reach. The")
print("#       shape with NO LEXICAL EMISSION is `X{0}` -- axis Z0 below.")
cell(r"^q(?>(a|ab))?z(?1)c$", "qzabc", "matches, but proves nothing new")
print()

print("=== AXIS Z0: A CALLEE DEFINED INSIDE `X{0}` =========================")
print("# The classic pre-DEFINE idiom: a group that exists ONLY to be called,")
print("# parked under a {0} repeat so it never runs lexically. pcrec emits")
print("# NO CODE and allocates NO SLOTS for X{0} (probe_slotcount.py axis C),")
print("# so the callee region has no slot instances counted for it -- the")
print("# out-of-bounds write vm_count_slots\' own header names (K27\'s class).")
print("# These are the cells that make that a REAL construct family and not a")
print("# hypothetical.")
cell(r"^(?:(?<g>a|ab)){0}(?&g)c$", "abc", "and the callee RETRIES into ab")
cell(r"^(?:(?<g>a|ab)){0}(?&g)c$", "ac", "control: first alternative suffices")
cell(r"^(x)?(?:(a(?2)?b)){0}(?2)$", "aabb", "a RECURSIVE callee in {0}")
cell(r"^(x)?(?:(a(?2)?b)){0}(?2)$", "ab", "control: depth 1")
cell(r"^(?:((?>a|ab))){0}(?1)z$", "az", "an ATOMIC callee in {0}")
cell(r"^(?:((?>a|ab))){0}(?1)z$", "abz",
     "and the atomicity travels WITH the callee: nomatch, because the (?> is "
     "INSIDE the group and is part of the body, unlike axis W3 where it "
     "WRAPS the group")
cell(r"^(?:(a?)){0}(?1)*b$", "b", "a rung-bearing callee in {0}")
cell(r"^(?:((a)(?1)?(b))){0}(?1)$", "aabb", "three groups, recursive, in {0}")
print("# CONTROLS -- the same callees WITHOUT the {0} wrapper:")
cell(r"^(?:(?<g>a|ab))?(?&g)c$", "abc", "optional instead of {0}")
cell(r"^(?(DEFINE)(?<g>a|ab))(?&g)c$", "abc", "DEFINE instead of {0}")
print()
