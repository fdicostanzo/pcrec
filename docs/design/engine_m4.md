...
|---|---|
| **1. Exact islands** — fragments auto-possessification proves atomic in context (`[^"]*` before `"`) | **DESIGNED AND SCHEDULED (M4.6).** DFA and backtracking semantics coincide, so the island emits a plain table loop with one answer and pushes no frame. This is the hot case (`.*`, class repeats, alternation tries) and it is also what keeps the trail and frame arrays small. **RE-HOMED 2026-08-17 (D50, Frank): deferred out of M4.6 to plan row [ENG-ISL], evidence-gated** — the [ENG-BREP] ladder (possessify/revdet), built after this table was written, delivers much of the same frameless win VM-internally for loop shapes; the emitter build waits for [BENCH-1]/[ENG-PGO] evidence that capture-free VM-fallback FRAGMENTS are hot. The design in this section stands; the atomicity proof it needs is eng_brep_design.md §2.2's corrected rule. |
...
| Trie-factored alternation emitting a first-byte switch with no pushes | **MEASURED ([M4.6e], 2026-08-17), DEFERRED — not built.** Candidate fraction: 22/1146 (1.92%) of ALL corpus patterns, 22/347 (6.34%) of CAPTURE-BEARING (VM-forced) corpus patterns, are heuristically first-byte-disjoint alternations that could take the path — and NEITHER shipped capture-bearing bench shape (case j; case c's alternation is pinned `--no-captures` and never reaches `vm_alt`) is a hit. Measured payoff on two real disjoint-alternation shapes (branch-position sweep, isolating `vm_alt`'s N-1-push chain, src/gen/emit_vm.c:1783): a 5-way word alternation's worst branch position costs +18% over its best (~23 ns of ~130 ns/call); a 3-way HTTP-method dispatch costs +3-4%. Real but narrow (D18: "an axis must earn itself") — declined for now rather than building a new emitter analysis (with its own D46 stamp+force pair and sabotage-matrix obligation, `src/opt/CLAUDE.md`'s established cost for a selection axis) against a shape that is 6% of even the adversarial correctness corpus and absent from the bench suite's own capture-bearing floors. Revisit if [BENCH-1]/[ENG-PGO]-class evidence surfaces a real customer, the same evidence-gate D50 used to re-home exact islands (§6.3, above). Data: `m46e_impl/out/trie_switch_sweep.txt`; probe: `m46e_impl/probes/trie_switch.py`. **BUILT 2026-09-03 as `[ENG-ISL]` STEP 1, and what shipped is STRICTLY MORE than this row proposed.** The row described a first-byte switch chosen by a heuristic over first-byte-disjoint alternations; what shipped is a FULL TRIE over a predicate about the LANGUAGE — an alternation qualifies when its whole subtree matches a finite set of literal byte strings, so a partially factored tree and the flat alternation it was factored from produce the same island (`docs/spec/tuning.md` §2.20). The evidence gate this row named was met from the other direction: not a bench customer for the first-byte case, but `docs/design/alt_dispatch_study.md`'s measurement of five dispatch algorithms plus bench O-15's ×8.87/×20.1 branch-ORDER penalty. The narrowness argument above still reads correctly for what it measured: this row's 6% candidate fraction is the FIRST-BYTE-DISJOINT population, and the shipped predicate's is larger. |

**"ISLAND" NAMES TWO DIFFERENT THINGS IN THIS DOCUMENT, and §6.3/§6.4's usage
is the older one.** Everywhere above, an ISLAND is an EXACT-MATCH DFA region
spliced into the VM's program — determinized, entered from the VM, charged one
step at its entry (§4.2), and deferred by D50's evidence gate. `[ENG-ISL]`
STEP 1's ALTERNATION ISLAND is a different construct that borrows the word: a
trie dispatch over an alternation's literal alternatives, emitted by the VM's
own emitter, carrying no DFA and no transition table at all. The two share the
idea of a determinized region inside the VM program and nothing else; the plan
row's own bidirectional framing ("islands of VM in DFA as well as DFA in VM")
is what put them under one name. `src/gen/CLAUDE.md`'s `[ENG-ISL]` section
draws the same line from the emitter's side.
