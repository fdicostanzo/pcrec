#!/bin/sh
# [M6.6.1] Run one probe and archive its output with FULL PROVENANCE.
#
...
#   e.g. archive.sh spellings.txt docs/design/lookaround_measurements/probes/probe_spellings.py
set -e
OUT=$1; PROBE=$2; shift 2

REPO=$(git rev-parse --show-toplevel)
DEST="$REPO/docs/design/lookaround_measurements/out/$OUT"
cd "$REPO"
...
{
  echo "# ============================================================"
  echo "# ARCHIVED PROBE OUTPUT — [M6.6.1] module \`lookaround\` design lane"
  echo "# ------------------------------------------------------------"
  echo "# PROBE      : $PROBE"
  echo "# ARGS       : ${*:-(none)}"
  ...
  echo "# host       : $(uname -sr)"
  echo "# ------------------------------------------------------------"
  echo "# Evidence for the [M6.6.1] panel, never an oracle: no check reads this"
