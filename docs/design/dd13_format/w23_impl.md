...
| `indent_under_m.rxtin` | W23.2 | an indented line under an `m` case line, mid-block | all three refuse, class **`structure-attachment`** (CORRECTED at the W23.2 merge, 2026-09-15 — this row said `schema-constraint`; MEASURED, `rxt_source.c`'s S1 block files every "indented line continues nothing" refusal under RXTD_STRUCTURE regardless of whether there is no parent at all or the parent's `children` column is NONE, three call sites `:1169`/`:1173`/`:1184`; legs B/C were matched to the delivered code per the lane's must-not-touch boundary — w232_report.md §3.2) | S-R1's detector: flipping `m`'s `children` makes leg A accept while B and C refuse, so only the DIFFERENTIAL sees it |
