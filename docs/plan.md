...
- [R3.2] STATE:not-started — R3 had NO adversarial critic coverage (all three critics failed to report). Before M3 work begins, run the panel over M2.8/M2.10/M2.12: the trie's soundness guards (D9) vs nested alternations/quantified groups/startpos!=0, the EOL vs non-EOL ordering asymmetry (D11), the iterative clo_visit rewrite's equivalence to the recursive original, and whether D13's dispatch micro-benchmark represents the real emitter shape
