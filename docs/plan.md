...
- [M2.10] STATE:completed — dense/counting-pattern codegen (R2-A5, D13): case (f)'s "no skip-eligible states" was a wrong CRITERION, not an absent opportunity — eligibility required self-looping on >=192 of 256 bytes, which rejects `[01]` (2 live bytes, 100% stay). Now stay/live >= 75%: case (f) 83.7 -> 116.8 MB/s (+40%), every other probe pattern byte-identical. D7's promised computed-goto-vs-table arbitration RESOLVED by micro-benchmark: computed goto is 2.59x SLOWER at 6 states, so there is no crossover and the unconditional table emitter stands
- [M2.11] STATE:started — process ratchets (R2-PR7/PR8). DONE: tests/known_fail/run_known_fail.sh inverts the verdict on deferred-bug regressions (still-failing = expected, now-PASSING = flagged + exit 1 with promotion instructions), wired into `make test`, validated in all three states (empty dir, now-passing, still-failing). REMAINING: the pass/fail gate over tests/bench/compare — blocked on M2.9's compare.sh rework landing first
