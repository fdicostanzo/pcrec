...
- [R3.2] STATE:started — R3 had NO adversarial critic coverage (all three R3 critics failed to report). Partially closed by self-run differential sweeps after the fact: the trie's guards held over 35,280 cases (nested alternations, quantified groups, overlapping classes, `$`, mixed eligible/ineligible branches) and startpos!=0 held over 12,480 cases. STILL UNPROBED, in priority order: (1) the iterative clo_visit rewrite's equivalence to the recursive original — highest risk, it touches the loop-entry/reent empty-iteration rule behind two past wrong-answer bugs; (2) the EOL vs non-EOL ordering asymmetry in emit_unanchored (D11); (3) whether D13's dispatch micro-benchmark represents the real emitter shape
