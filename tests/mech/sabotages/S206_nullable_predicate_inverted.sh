# S206 (S-OPT41-2) — [OPT-4.1] THE NULLABILITY PREDICATE INVERTED: THE RESCUE
# IS REFUSED ON EVERY PATTERN IT WAS BUILT FOR, AND KEPT ON EVERY PATTERN IT
# WAS BUILT TO AVOID.
#
# WHY THE PAIR EXISTS. S205 removes the predicate; this one turns it round.
# They are not two spellings of one plant, because the two failures have
# OPPOSITE costs and only one instrument sees each. S205 is a SLOWDOWN with
...
#
# **THE CORPUS CANNOT SEE EITHER HALF**, for the reason S205's header states
...
# in both directions — but they are S205's detectors, not this row's, and a
# reader must not take their redness as evidence that the twin was checked.
SAB_ID="S206-nullable-predicate-inverted"
...
SAB_AFTER='        fit.prefilter_lang_nullable = pcrec_minw(root) != 0;   /* SABOTAGE S206 */'
