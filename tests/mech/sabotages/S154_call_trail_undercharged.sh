...
SAB_DOC_FIGURE="PREDICTED (design 9.3 S-SR7): NO ANSWER CHANGES until the trail is exhausted, then PCREC_ERR_FRAMES on a matching subject. The detector is therefore a DEEP-CALL cell rather than a corpus answer, which is what run_recursion_diff.sh's 2 bisection provides: it finds the artifact's own ceiling and asserts that ONE STEP BEYOND IT the answer is a typed give-up rather than a wrong nomatch. The corpus arm is assigned too because a halved charge shrinks the ceiling far enough that leftrec.rxt's 200-deep cell can cross it. LIVE WITNESS (verified 2026-09-21, lane adm0921, ADMIN-0921 item 3, closing tour2_report.md's owed item): pattern '(a)(?1)' over subject 'aa' -- a single NON-RECURSIVE, non-cyclic subroutine call (cost.unbounded stays false at this call site, so trail_frames is COMPUTED from the real charge rather than defaulted to the fixed 3072 bring-up ceiling every attempt at tour2's own '^(a(?1)?b)$' over a^n b^n hit first, because that pattern's call target reaches itself). Two scratch git-archive-HEAD builds (tests/mech/lib/replace.py applying this row's own BEFORE/AFTER), clean vs sabotaged: the CLEAN compiler stamps RX_TRAIL_FRAMES 13 and the emitted matcher answers 'match 0 2' on 'aa' (agrees with libpcre2 10.48-Homebrew: '0: aa' / '1: a'); the SABOTAGED compiler, same pattern, stamps RX_TRAIL_FRAMES 10 and the emitted matcher answers PCREC_ERR_FRAMES ('frames') on the IDENTICAL subject the clean build matches -- the row's documented failure mode reproduced exactly, with a witness two atoms long. run_recursion_diff.sh SS2's bisection witness is UNCHANGED and remains the automated detector; this pattern/subject pair is recorded here so the next re-verification does not have to rediscover a discriminating witness by search."
SAB_COUNT=1
