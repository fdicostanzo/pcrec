# S112 (design row S-BR9) — §8.3's RESOLUTION TAKES THE FIRST BY NUMBER RATHER
...
#
# S112 AND S113 ARE SEPARATE ROWS ON PURPOSE (design §11.4's closing note):
...
SAB_ID="S112-resolution-first-by-number"
...
SAB_DOC_FIGURE="PREDICTED: dupnamesdiff RED; the corpus RED on exactly the \"yy\" cell of dupnames.rxt's resolution block. Canonical figure owed from run_sabotage_matrix.sh S112."
SAB_COUNT=1
SAB_BEFORE='        for (int i = 0; i < a->nrefs; i++) {
            char ns[144], ne[144];'
SAB_AFTER='        for (int i = 0; i < 1 && i < a->nrefs; i++) {   /* SABOTAGE S112 */
            char ns[144], ne[144];'
