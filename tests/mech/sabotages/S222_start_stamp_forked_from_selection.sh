...
# certifies nothing. So the fork is not an arbitrary second predicate — it is
# the WIDENED `state_acc_any` read, and §8 of
# `tests/codegen/run_search_pinned.sh` already names a population on which
# widened and narrowed DISAGREE BY CONSTRUCTION: the `(?m)…$` family, floored
# at 12 members and measured at 16. On every one of those the fork stamps
# `"pinned"` while the body still emits `rewind_position`, which is precisely
# what check §2's stamp-vs-body third term reports.
#
...
SAB_DOC_FIGURE="PREDICTED (the canonical DETECTED figure is owed from the manager's own matrix run): searchpinned RED in §2's stamp-vs-body third term and in its mirror leg, on the >= 12 artifacts §8's manifest floors. NON-VACUITY: the fork is the WIDENED state_acc_any read, on which the manifest population disagrees with the narrowed one by construction, so the row cannot pass by the two predicates happening to agree. The corpus arm is not assigned -- no answer moves, and assigning it would invite a reader to score its green as a half-detection."
 ...
 * the body was emitted through. It reads the WIDENED accept bit, so it
 * disagrees with the real selection on exactly the (?m)...$ family. */
