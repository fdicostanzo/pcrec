...
# `ctx_nomem`-routed allocation failure propagates immediately instead of
...
SAB_SUITES="resource"
SAB_DESC="compile_driver's setjmp handler never tests cx.failed_nomem (if (0 && ...) instead of if (...)), so a genuine ctx_nomem-routed allocation failure falls through to the [ART-SIZE] ladder's blanket catch and can be silently absorbed into a later attempt's success -- K60's ladder class, D109's own fix undone"
