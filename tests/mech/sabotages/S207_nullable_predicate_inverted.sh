...
SAB_DOC_FIGURE="CANONICAL RUN 2026-08-30 (run_sabotage_matrix.sh S207 at db670dc): reach:ok(2/2), pfcollapse:17fail/20pass, resource:2fail/27pass, corpus:0fail/51pass -- DETECTED, all three SAB_REACH_POP floors met. READ IT AGAINST S206 RATHER THAN ALONE: an INVERSION is loud in both directions (17 pfcollapse fails against S206s 5) because it breaks the winners AND the losers at once, and resource shows BOTH size-rung cells red where S206 shows ONE. That 2-vs-1 is the pairs whole point -- S206 removes the predicate so only the nullable cell moves; S207 inverts it so the non-nullable twin moves too. A single row could not have expressed the difference. corpus:0fail/51pass on BOTH: the prefilter is a FILTER, so neither its loss nor its uselessness moves one cell of the corpus, which is why this arm exists at all."
SAB_REACH='"$PCREC" --features all -p rx -o - --pattern "\b(?:ERROR|FATAL|CRIT)\b.{0,200}?\b(?:timeout|timed out|refused|denied|unreachable)\b"'
...
# still INVERTS `lang_nullable` at its one derivation, so both declines
