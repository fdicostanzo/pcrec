...
SAB_DOC_FIGURE="Reach probe verified 2026-09-17 against the current tree's build/pcrec: the witness pattern (a{10,20}){10,50}, --engine=vm, reads RX_UNROLL_K_WHY size-model at --tune=min-size (threshold 40,000 crossed, K drops to 1) and default at --tune=balanced (threshold 120,000 not crossed, K stays 8) -- both directions in one probe. SOLO MECH RUN 2026-09-17 (bash tests/mech/run_sabotage_matrix.sh S251, tree cb4f55eba0c0f42eaf55343fc932fd0109f372ad): edit APPLIES, sabotaged tree BUILDS cleanly, pop:tests/base/d27_nested_min_boundary.rxt:/^pattern (a{10,20}){10,50}$/=1(want>=1), reach:ok(2/2), suites run: tunedial -> UNKNOWN-SUITE:tunedial -> verdict ANOMALY (no suite ran), unexpected: 0. tests/codegen/run_tune_dial.sh did not exist in this worktree at authoring time (a sibling agent is building it concurrently per the lane brief), so tunedial is not yet registered in run_sabotage_matrix.sh's dispatch (R31 C11: unregistered scores UNKNOWN-SUITE, never UNDETECTED) -- everything up to the cross-check's own arm is verified; DETECTED/UNDETECTED is OWED once it is registered. Exact re-run command: bash tests/mech/run_sabotage_matrix.sh S251."
