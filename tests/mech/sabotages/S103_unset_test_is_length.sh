# S103 (design row S-BR1) — THE UNSET TEST BECOMES `ref_end > ref_start`.
#
...
#
# It is also the mirror of S-BR3's mistake one level down: "is the pair set"
...
SAB_ID="S103-unset-test-is-length"
...
SAB_DOC_FIGURE="PREDICTED: the corpus RED on numeric.rxt's EMPTY block; brefdiff RED. Canonical figure owed from run_sabotage_matrix.sh S103."
SAB_COUNT=1
SAB_BEFORE='                "        %sif (slot_values[%s] != PCREC_UNSET) {\n"'
SAB_AFTER='                "        %sif (slot_values[%s] != slot_values[%s + 1]) {   /* SABOTAGE S103 */\n"'
