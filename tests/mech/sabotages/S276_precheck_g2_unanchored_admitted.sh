...
SAB_DOC_FIGURE="MEASURED 2026-09-25 (lane chkgaps, re-anchored at the chkgaps/k64fix merge against the post-fix-A source; solo single-row scratch run via tests/mech/run_sabotage_matrix.sh S276, tree 4106b318): DETECTED, reach:ok(1/1), prechecks:10fail/250pass. Section 5.7's own positive control flips from RX_REQ_WHY \"emitted\" to \"one-attempt\" and its no-'@' subject flips from exit 1 (nomatch, bounded) to a step give-up; the other failures are the same missing conjuncts reached through §5's other witnesses. S274 (main's own k64fix row, same file, disjoint anchor text after re-derivation) re-verified unaffected in the same tree: prechecks:2fail/260pass, corpus:4fail/5pass, DETECTED."
