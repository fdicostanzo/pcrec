# S102 (design row S-BR15b) — ONLY ONE MEMBER OF A DUP-NAME RUN IS MARKED.
#
...
SAB_ID="S102-mark-one-run-member"
...
SAB_DOC_FIGURE="PREDICTED: dupnamesdiff RED; the corpus RED on dupnames.rxt's re-entry block. Canonical figure owed from run_sabotage_matrix.sh S102."
SAB_COUNT=1
SAB_BEFORE='            for (int i = 0; i < a->nrefs; i++)
                if (a->refs[i] > 0 && a->refs[i] < nmark) mark[a->refs[i]] = true;'
SAB_AFTER='            /* SABOTAGE S102: only the first member of the run */
