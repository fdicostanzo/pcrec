# S273 — [K64] G2's VM LINEARITY CONJUNCT REMOVED (src/gen/emit_dfa.c,
...
SAB_ID="S273-precheck-g2-vm-linearity-removed"
...
SAB_BEFORE='        return cx->job->start_anchor != PCREC_SANCH_NONE &&
               ((cx->job->fit.prefilter && !cx->job->fit.prefilter_collapsed) ||
                cx->job->vm_frameless);'
SAB_AFTER='        return cx->job->start_anchor != PCREC_SANCH_NONE;   /* SABOTAGE S273: K64 linearity conjunct removed */'
