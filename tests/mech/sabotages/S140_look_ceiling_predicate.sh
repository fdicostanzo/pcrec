...
SAB_DOC_FIGURE="PREDICTED: tests/lookaround/prefilter.rxt RED on its five CLAMPING blocks (17 answer cells across them) and GREEN on the erasure control and on §5.4's two non-clamping shapes, which stamp \"none\" and never had a ceiling; the lookaround arm's §1 pcre2-only sweep RED on the (?*! block. [M6.6-LOOKAROUND rule 1] in tests/codegen would also fire, but codegen is NOT assigned here -- design §9.3 puts this row on the behavioural pair, and S141 is the codegen row. Canonical figure owed from run_sabotage_matrix.sh S140."
SAB_COUNT=1
