...
SAB_DOC_FIGURE="PREDICTED: limits:1fail (part 3: 'enum { VM_MAX_BODY_CAPS = 64 }; -- VM_MAX_BODY_CAPS is neither a limits.def row nor on the cited allowlist'). harness and vmidentity expected 0fail -- the value is unchanged, so every answer and every emitted byte is identical. CANONICAL RUN 2026-09-19 ([REVW.4] wave 4, solo, at d0c8b4c2): limits:1fail/23pass, vmid:0fail/10pass, corpus:0fail/51pass -- DETECTED, unexpected: 0, anomalies: 0. The one red cell is part 3 arm [code], and its WORDING changed with D107's inversion while its verdict did not: \"src/gen/emit_vm.c:232:enum member VM_MAX_BODY_CAPS = 64 -- 'VM_MAX_BODY_CAPS' is a numeric constant outside limits.def and is on NEITHER allowlist\". Worth recording that this plant survives the inversion for a reason rather than by luck: the plant is a SINGLE-MEMBER enum whose one explicit value is 64 at ordinal position 0, so it is not an ORDINAL enum and the new scan sees every member of it."
SAB_COUNT=1
