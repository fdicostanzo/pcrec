# S109 (design row S-BR7) — THE MODULE GATE IS DROPPED FROM THE NEW ATOM PORT.
#
...
SAB_ID="S109-gate-check-dropped"
...
SAB_DOC_FIGURE="PREDICTED: reject RED broadly; the corpus RED on gated.rxt's refusal cells. Canonical figure owed from run_sabotage_matrix.sh S109."
SAB_COUNT=1
SAB_BEFORE='    want = pcrec_ext_gate(r, want);

    if (!r) {'
SAB_AFTER='    /* SABOTAGE S109: the gate no longer demotes */
