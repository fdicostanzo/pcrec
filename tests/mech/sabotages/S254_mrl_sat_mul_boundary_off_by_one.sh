...
#
# `pcrec_mrl_sat_mul`'s overflow guard changes from `a > MRL_MINW_MAX / b` to
...
# equality) — `pcrec_mrl_sat_mul`'s boundary pair now disagrees with
