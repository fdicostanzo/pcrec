...
SAB_DOC_FIGURE="Reach probes verified 2026-09-17 against the current tree's build/pcrec (both denied axes, plus the tiered-entry flag's OWN direct-denial value as a positive control that the mechanism it wrongly triggers really exists -- see header). SOLO MECH RUN 2026-09-17 (bash tests/mech/run_sabotage_matrix.sh S250, tree cb4f55eba0c0f42eaf55343fc932fd0109f372ad): edit APPLIES, sabotaged tree BUILDS cleanly, reach:ok(3/3) against the clean reference tree, suites run: tunedial -> UNKNOWN-SUITE:tunedial -> verdict ANOMALY (no suite ran), unexpected: 0. tests/codegen/run_tune_dial.sh did not exist in this worktree at authoring time (a sibling agent is building it concurrently per the lane brief), so tunedial is not yet registered in run_sabotage_matrix.sh's dispatch (R31 C11: unregistered scores UNKNOWN-SUITE, never UNDETECTED) -- everything up to the cross-check's own arm is verified; DETECTED/UNDETECTED is OWED once it is registered. Exact re-run command: bash tests/mech/run_sabotage_matrix.sh S250."
SAB_REACH='"$PCREC" -p rx --no-captures --tune=min-size -o "$REACH_TMP/premul.c" -- "a(b|c)+d" && grep -h "RX_DFA_TABLE" "$REACH_TMP/premul.c" && "$PCREC" -p rx --engine=vm --tune=min-size -o "$REACH_TMP/tiered_ms.c" -- "((a)|(aa))+b" && grep -h "RX_FAST_FRAMES" "$REACH_TMP/tiered_ms.c" && "$PCREC" -p rx --engine=vm -fno-tiered-entry -o "$REACH_TMP/tiered_denied.c" -- "((a)|(aa))+b" && grep -h "RX_FAST_FRAMES" "$REACH_TMP/tiered_denied.c"'
