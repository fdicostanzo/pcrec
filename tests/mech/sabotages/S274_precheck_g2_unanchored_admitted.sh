# S274 ([chkgaps] check-design closure) — G2 ADMITTED FOR AN UNANCHORED VM
...
# not per-attempt). `tests/codegen/run_prechecks.sh` §5.6 measured this
...
# so this plant is that control's own failing direction.
SAB_ID="S274-precheck-g2-unanchored-admitted"
...
{
    /* SABOTAGE S274: the VM arm admits EVERY VM route as one-attempt,
