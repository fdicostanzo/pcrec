...
# answer moved. `vm_rev_emit`'s `default:` is a `pcrec_ctx_fail` reading "internal
...
# `rd_reverse`'s own `case A_CALL:` `pcrec_ctx_fail` firing before the tail
