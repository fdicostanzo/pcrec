# S256 — [REVW.U L8-F6(c)] `sb_grow`'s `ctx_nomem` ROUTE IS NEUTERED, SO
...
#
# `if (sb->cx) ctx_nomem(sb->cx);` becomes `if (0) ctx_nomem(sb->cx);` —
...
SAB_SUITES="resource"
SAB_DESC="sb_grow's ctx_nomem route is neutered (if (0) instead of if (sb->cx)), so EVERY StrBuf realloc failure falls to the unconditional abort() below it, restoring K7's worst case on every compile's csb/hsb/vmsb/irsb/scr_test/scr_desc buffer, not only the two F1 missed"
...
SAB_AFTER='    if (!np) {
        if (0) ctx_nomem(sb->cx);   /* SABOTAGE S256: the error channel is never consulted */
        abort();   /* a detached buffer (syntax_dump.c) has no error channel */
    }'
