# S113 (design row S-BR8b) — AN 8/9-LED DIGIT RUN IS ROUTED THROUGH THE OCTAL
# BRANCH.
#
...
SAB_ID="S113-rule3prime-via-octal"
...
SAB_DOC_FIGURE="PREDICTED: the corpus RED on octal.rxt's rule-3' block only. Canonical figure owed from run_sabotage_matrix.sh S113."
SAB_COUNT=1
SAB_BEFORE='    if (runlen == 1 || d0 >= 8) {'
SAB_AFTER='    if (runlen == 1) {   /* SABOTAGE S113: 8/9-led runs fall to rule 3 */'
