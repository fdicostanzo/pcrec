...
SAB_DOC_FIGURE="PREDICTED ([DD-14.LB]): the seven call-bearing-lookbehind MATCH blocks in tests/recursion/inlookaround.rxt fail as PATTERN-COMPILE FAILURES, not as wrong spans -- vm_look_behind ctx_fails on the pending shape. The file's four perr blocks still pass (a perr cell asserts a nonzero exit, not a message), which is why the row is scoped to a file that has match cells at all. MEASURED at [DD-14.LB], FINAL corpus: harness corpus:27fail/23pass, DETECTED -- 27 cases across exactly the call-bearing-lookbehind match blocks, every failure a pattern-compile failure. (An earlier run against the 16-block corpus read 14fail/23pass; the file grew to 21 blocks in the same wave.)"
SAB_COUNT=1
