...
SAB_DOC_FIGURE="PREDICTED ([DD-14.LB]): the seven call-bearing-lookbehind MATCH blocks in tests/recursion/inlookaround.rxt fail as PATTERN-COMPILE FAILURES, not as wrong spans -- vm_look_behind ctx_fails on the pending shape. The file's four perr blocks still pass (a perr cell asserts a nonzero exit, not a message), which is why the row is scoped to a file that has match cells at all. MEASURED at [DD-14.LB]: harness corpus:14fail/23pass, DETECTED -- 14 cases across exactly the seven match blocks, every failure a pattern-compile failure."
SAB_COUNT=1
