...
SAB_DOC_FIGURE="tests/codegen/run_prechecks.sh is the detector: §1.1b goes RED on every bounded witness (^abc, \\Aabc, ^(a|b)+\$, \\Gabc — 4 of the 7 §1.1 rows), reporting 'stamps \"anchored\" but the emitted loop is not bounded by attempt_max'. The harness arm is expected to score 0fail: the bound removes only failing attempts, so no answer moves. Exact re-run command: bash tests/mech/run_sabotage_matrix.sh S263."
# [MECH-REACH] THE PROBE says the SITE still answers: on the clean tree a
# `^`-anchored capture-bearing pattern compiles to a VM program, stamps
# RX_VM_START "anchored" and emits the bound this row plants on — so the
# emission block the plant edits really does execute on this witness.
