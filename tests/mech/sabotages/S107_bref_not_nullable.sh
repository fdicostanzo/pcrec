...
SAB_DOC_FIGURE="MEASURED 2026-08-22, run_sabotage_matrix.sh S107 at a1cd705: DETECTED, corpus 9fail/79pass + brefdiff 4fail/10pass (1 rows, undetected: 0, anomalies: 0). Every corpus failure is a PCREC_ERR_FRAMES give-up on a cell whose referenced group captured EMPTY (^(a*)\\1*\$ on \"\", ^(a?)\\1*b\$ on \"b\", ^(a?)\\1{2,}\$ on \"\" and \"aa\", ^()\\1+\$ on \"\"); brefdiff's own is the new structural section 10, whose 4 guard-bearing fixtures emit the marker 0 times. Clean tree: numeric.rxt 88/0, brefdiff 11/0."
SAB_COUNT=1
