...
SAB_DOC_FIGURE="tests/base/k65_precheck_whole_set.rxt is the answer-level detector: the three '...Zb' n cells of block 1 (byte), the three '...@b' n cells of block 2 (utf8) and the three '...@#' n cells of block 3 (the run form) fail as a steps give-up — corpus:9fail/15pass, the same 9 the pre-fix compiler (5a2094e7) fails. tests/codegen/run_prechecks.sh section 5.8 reports the four rows expecting an rq_set array finding none, and section 5.9 its two rows expecting one — prechecks:6fail/268pass (re-measured on lane k66fix, 2026-09-25; 4fail/263pass before K66 added §5.9). Exact re-run command: bash tests/mech/run_sabotage_matrix.sh S277."
