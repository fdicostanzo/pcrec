...
SAB_DOC_FIGURE="PREDICTED (the canonical DETECTED figure is owed from the manager's own matrix run): vmframeless RED in section3's corpus sweep wherever a VM artifact's real goto* dispatch disagrees with the npush-derived stamp -- expected on the population containing a LINKED call with no other push site (npush == 0, has_push true). corpus:0fail expected on every axis -- no answer moves anywhere in the tree."
