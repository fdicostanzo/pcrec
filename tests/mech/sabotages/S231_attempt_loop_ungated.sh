# S231 — [K50] SITE 2: `ENG_ATTEMPT`'s START LOOP STEPS ONE BYTE AGAIN.
#
...
# That discrimination is the row's value: a matrix in which S230 and S231 fire
# the same cells would not be telling two mechanisms apart.
SAB_ID="S231-attempt-loop-ungated"
...
SAB_COUNT=1
SAB_REACH='$PCREC -p rx -e utf8 --features assertions,modifiers -o probe.c "(?m)^a|\\B" && grep -c "start_max = subject_length" probe.c'
SAB_REACH_EXPECT='start_max = subject_length'
SAB_BEFORE='                "        if (start > search_from && !(%s)) continue;\n", sbnd);'
SAB_AFTER='                "        (void)search_from; /* SABOTAGE S231 */%s\n", "");'
