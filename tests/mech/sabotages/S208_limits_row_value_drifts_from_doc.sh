...
SAB_COUNT=1
SAB_BEFORE='PCREC_LIMIT(PCREC_MAX_VM_EMIT_CODE_BYTES, 500000, "bytes", "size cap", FLAG_D, "8", "[ART-SIZE]/D84: emitted C bytes OUTSIDE table initializers; --max-emit-code-bytes ALSO raises it per-compile (raise-only); -D moves the built-in default (run_size_term.sh'"'"'s reference-compiler consumer) -- [LIM-OVR] 2026-09-22: override was BUILD_D ('"'"'never a caller lever'"'"'), false given --max-emit-code-bytes; FLAG_D names the two-lever shape honestly", LIMITS_H)'
SAB_AFTER='PCREC_LIMIT(PCREC_MAX_VM_EMIT_CODE_BYTES, 400000, "bytes", "size cap", FLAG_D, "8", "[ART-SIZE]/D84: emitted C bytes OUTSIDE table initializers; --max-emit-code-bytes ALSO raises it per-compile (raise-only); -D moves the built-in default (run_size_term.sh'"'"'s reference-compiler consumer) -- [LIM-OVR] 2026-09-22: override was BUILD_D ('"'"'never a caller lever'"'"'), false given --max-emit-code-bytes; FLAG_D names the two-lever shape honestly", LIMITS_H)  /* SABOTAGE S208 */'
