...
ref_out="$(pcrec_run "$PCREC" --features assertions --engine=dfa -p rx -o "$WORKDIR/ref.c" --pattern 'a\Kb' 2>&1)"
ref_rc=$?
if [ "$ref_rc" -eq 0 ]; then
...
elif ! pcrec_run "$PCREC" --features assertions -p rx -o "$WORKDIR/ok.c" --pattern 'a\Kb' >/dev/null 2>&1; then
