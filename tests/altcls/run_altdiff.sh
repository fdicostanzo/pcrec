...
one_pattern() {
    ...
    if ! pcrec_run "$PCREC" -p pa -o "$d/pa.c" --pattern "$pat" >/dev/null 2>"$d/err_a"; then
        skipped=$((skipped + 1))
        return 0
    fi
    if ! pcrec_run "$PCREC" -p pb -fno-altcls-merge -fno-altcls-factor -o "$d/pb.c" --pattern "$pat" \
...
if [ "${1:-}" = "--corpus" ]; then
    ...
    while IFS= read -r cp; do
        ...
        pd="$WORKDIR/probe"
        rm -rf "$pd"; mkdir -p "$pd"
        if pcrec_run "$PCREC" -p pp -o "$pd/pp.c" --pattern "$cp" >/dev/null 2>&1; then
