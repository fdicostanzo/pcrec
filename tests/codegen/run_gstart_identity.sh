...
# were two levels down at src/gen/enc/ (run_mlinectx_identity.sh's own note;
# they are src/enc/ since [REVW.3] wave 3, and the tree has now changed shape
# twice under such a glob). A reference compiler quietly built from a
# different source set than the subject is the differential going vacuous.
