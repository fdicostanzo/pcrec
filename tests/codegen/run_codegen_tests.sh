...
if [ -f "$WORKDIR/abi_rx.c" ]; then
    ...
    if [ -n "$new_missing" ]; then
        ...
        ok "[ABI-NS]: all 17 universal constants (give-up codes incl. PCREC_ERR_RECURSE, UNSET, ENGINE_DFA/VM, nine D46 stamp bits) are emitted exactly once in a VM artifact"
    fi
...
if "$PCREC" -p rx --no-captures -o - -- 'a(b|c)+d' > "$WORKDIR/abins_dfa.c" 2>/dev/null; then
    dfa_missing=""
    for n in PCREC_ERR_STEPS PCREC_ERR_FRAMES PCREC_ERR_WORK PCREC_ERR_RECURSE PCREC_ERR_FLOOR PCREC_UNSET PCREC_ENGINE_DFA PCREC_ENGINE_VM PCREC_VM_RUNG_CURSOR PCREC_VM_STRAT_POSSESSIVE PCREC_VM_PRUNE_CLAMPED; do
