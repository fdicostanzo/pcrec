...
gen() { # gen <name> <pattern> [extra pcrec args...] -> <name>.c plus <name>.body
    local name="$1" pat="$2"
    shift 2
    pcrec_run "$PCREC" -p rx "$@" -o "$WORKDIR/$name.c" --pattern "$pat" >/dev/null 2>&1 \
...
# nothing else does.
if pcrec_run "$PCREC" -p rx -o "$WORKDIR/minimvm.c" --pattern '(get|post|put|delete|patch)' >/dev/null 2>&1; then
...
# is the `/* Generated by pcrec (abi N). Pattern: ... */` comment, which differs by
...
gensc() { # gensc <name> <pattern> [extra pcrec args...] -> <name>.body
    local name="$1" pat="$2"
    shift 2
    pcrec_run "$PCREC" -p rx "$@" -o - --pattern "$pat" 2>/dev/null > "$WORKDIR/$name.sc"
    [ -s "$WORKDIR/$name.sc" ] \
...
elif pcrec_run "$PCREC" -p rx -o - --pattern '.*=.*' > "$WORKDIR/multi.c" 2>/dev/null \
  && pcrec_run "$PCREC" -p rx -o - --pattern '^a|b'  > "$WORKDIR/engb.c"  2>/dev/null \
...
if pcrec_run "$PCREC" -p rx -o "$WORKDIR/dprx.c" --pattern 'a(b|c)+d' >/dev/null 2>&1 \
   && pcrec_run "$PCREC" -p qq -o "$WORKDIR/dpqq.c" --pattern 'x(y|z)+w' >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" -p rx -o "$WORKDIR/eng.c" --pattern 'a(b|c)+d' >/dev/null 2>&1; then
...
# docs/dev/decisions.md D37 / [STD1] phase A: every emitted file stamps the
# enabled set's own NAME and its EXPANDED module list, rendered from the mask
# by src/parse/enabled.c so it can never say something the compile didn't
# actually do. Whole-file checks (not `body()` — the stamp is outside any
# engine function, at the very top).
#
# [EMIT-VERB] CONVERTED 2026-09-19 (D112 item 4: read the STAMP where the fact
# has one). D37 shipped the same fact TWICE, as a `/` `* Feature set: … *` `/`
# comment and as two `#define`s, and the comment is NON-ESSENTIAL prose a
# default artifact no longer carries. The macros are the stamp and are
# emitted unconditionally, so the `.c` arms read them; the third arm below —
# "the .h must not carry the MACROS" — is unchanged and still meaningful.
#
# WHAT THIS CONVERSION COSTS, recorded rather than glossed: the `.h` arm can
# no longer assert the feature set at all by default, because the macros are
# `.c`-only by `emit_feature_macros`' own design and the comment was the `.h`'s
# only carrier. It is re-aimed at `-fcomments`, where the comment still ships,
# so the `.h`'s own half of D37 keeps a detector instead of being deleted.
if pcrec_run "$PCREC" -p rx --features std1 -o "$WORKDIR/stamp.c" --pattern 'a' >/dev/null 2>&1 \
   && pcrec_run "$PCREC" -p rx --features std1 -fcomments -o "$WORKDIR/stampc.c" --pattern 'a' >/dev/null 2>&1; then
    if grep -qF '#define PCREC_FEATURE_SET "std1"' "$WORKDIR/stamp.c" \
       && grep -qF '#define PCREC_FEATURE_MODULES "classes,modifiers"' "$WORKDIR/stamp.c"; then
        ...
        bad "D37: --features std1 did not stamp the expected macros" \
            "$(head -4 "$WORKDIR/stamp.c")"
    fi
    if grep -qF '/* Feature set: std1 (modules: classes,modifiers) */' "$WORKDIR/stampc.h" \
       2>/dev/null; then
        ok "D37: under -fcomments the paired .h carries the same stamp COMMENT"
    else
        bad "D37: under -fcomments the paired .h is missing the stamp comment"
    fi
...
if pcrec_run "$PCREC" -p rx -o - --pattern 'a' 2>/dev/null | grep -qF '#define PCREC_FEATURE_SET "std1"'; then
...
if pcrec_run "$PCREC" -p rx --features none -o - --pattern 'a' 2>/dev/null | grep -qF '#define PCREC_FEATURE_SET "none"'; then
...
# `/* Generated by pcrec (abi N). Pattern: ... */` comment, which echoes the user's
...
ts1_one() { # ts1_one <label> <pattern> [extra pcrec args...]
    local lbl="$1" pat="$2"
    shift 2
    pcrec_run "$PCREC" -p rx "$@" -o "$WORKDIR/ts1.c" --pattern "$pat" >/dev/null 2>&1 \
...
if pcrec_run "$PCREC" -p rx -o - --pattern 'a(b|c)+d' > "$WORKDIR/m44info.c" 2>/dev/null; then
...
if pcrec_run "$PCREC" -p rx -o - --pattern 'abc' > "$WORKDIR/patlen1.c" 2>/dev/null; then
...
if pcrec_run "$PCREC" -p rx -o - --pattern 'a\nb' > "$WORKDIR/patlen2.c" 2>/dev/null; then
...
elif pcrec_run "$PCREC" -p rx --no-captures -o - --pattern '(alpha|beta|gamma|delta|epsilon)' > "$WORKDIR/k24.c" 2>/dev/null; then
...
for pfx in rx foo z_9 verylongprefixname; do
    if ! pcrec_run "$PCREC" -p "$pfx" -o - --pattern 'a(b|c)+d' > "$WORKDIR/abi_$pfx.c" 2>/dev/null; then
...
if pcrec_run "$PCREC" -p rx --no-captures -o - --pattern 'a(b|c)+d' > "$WORKDIR/abins_dfa.c" 2>/dev/null; then
    dfa_missing=""
...
#
# [EMIT-VERB] CONVERTED 2026-09-19, and it is a STRENGTHENING rather than a
# substitution. It used to key on the phrase "ENCODING RESIDUAL entry" in the
# entry's own doc-COMMENT, which a default artifact no longer carries (D112) —
# but the deeper point is that it never should have: the fact under test is
# which entries the artifact DECLARES, and a comment about a declaration is
# one remove from it. A check reading prose about a fact goes blind the day
# the prose moves, which is what this one did the moment the prose could be
# turned off.
#
# It now reads the DECLARATION HEADS at column 0 and SUBTRACTS the fixed
# surface — the five per-artifact entry points, their three `_in` siblings and
# `rx_info` (docs/spec/match_api.md §1/§10.4), a set unchanged since [M4.4]
# and stated in the spec rather than derived from this build. Whatever else
# an artifact declares at file scope IS a residual entry. `typedef` lines are
# excluded by name; the seam's entries are ordinary function declarations.
#
# FAILING DIRECTION, verified: the caller below asserts the extracted set
# against a per-fixture literal, and a fixture whose artifact declares nothing
# reads `[]` — which is what this check printed, 34 times, on the first run
# after the default flipped, before this conversion.
residual_names() {
    awk '
        # A residual entry is EXPORTED (the seam refuses `static`: an unused
        # static fails the harness -Werror build, which is src/enc/enc.h`s
        # own recorded reason next_pos is exported) and is a plain function
        # declaration at file scope. Everything the artifact defines for its
        # own use is `static`; `main` and the attribute line are named.
        /^typedef/ || /^static/ || /^__attribute__/ || /^int main/ { next }
        /^[A-Za-z_][A-Za-z0-9_ *]*[ *][a-z_][A-Za-z0-9_]*\(/ {
            ...
            # prefixed throughout (the `want_set` above builds `rx_<name>`),
            # so the exclusion is spelled the same way.
            if (line == "rx_search" || line == "rx_match" || line == "rx_match_caps" ||
                line == "rx_search_in" || line == "rx_match_in" ||
                line == "rx_match_caps_in" || line == "rx_info") next
            print line
        }
    ' "$1" | LC_ALL=C sort -u
}
...
while IFS=$'\t' read -r nm pat extra decl; do
    ...
    if ! pcrec_run "$PCREC" -p rx $extra -o "$WORKDIR/$nm.c" --pattern "$pat" >/dev/null 2>&1 \
       || ! pcrec_run "$PCREC" -p rx $extra -o - --pattern "$pat" > "$WORKDIR/${nm}_sc.c" 2>/dev/null; then
...
while IFS=$'\t' read -r nm feats pat; do
    [ -n "$nm" ] || continue
    if ! pcrec_run "$PCREC" -p rx --features "$feats" -o - --pattern "$pat" \
...
if pcrec_run "$PCREC" -p k27 --no-captures -o - --pattern 'abc' > "$WORKDIR/k27null.c" 2>/dev/null; then
...
if pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/wordset.c" \
        --pattern '(\b\w+\b)' >/dev/null 2>&1; then
    ...
    # question — is this byte a word character — in three places, so a correct
    # artifact has exactly ONE distinct test. (The word set can never BE a
    # fold pair, so the fold alternative below is enumeration-completeness,
    # not a live arm on this fixture.)
    ndistinct=$(grep -oE '\(rx_class_bitmap[0-9]+\[\(subject\[[^]]*\]\) >> 3\] >> \(\(subject\[[^]]*\]\) & 7\)\) & 1|\(unsigned\)\(subject\[[^]]*\] - [0-9]+\) <= [0-9]+u|\(subject\[[^]]*\] \| 0x20\) == [0-9]+|subject\[[^]]*\] == [0-9]+' \
...
if pcrec_run "$PCREC" -p rx --features assertions -o "$WORKDIR/kres.c" --pattern 'a\Kb' >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" -p rx --engine=vm -o "$WORKDIR/nok.c" --pattern '(a)(b)' >/dev/null 2>&1; then
...
# that spelling.
if pcrec_run "$PCREC" -p rx --features assertions -o "$WORKDIR/kent.c" --pattern 'a\Kb' >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" -p rx --no-captures -o "$WORKDIR/dent.c" --pattern 'a(b|c)d' >/dev/null 2>&1; then
...
ceil_drop() {
    ...
    if ! pcrec_run "$PCREC" $feats -p rx -o "$c" --pattern "$pat" >/dev/null 2>&1; then
        bad "[$tag rule 1]: pcrec failed to compile the fixture '$pat'"
        return
    fi
    pcrec_run "$PCREC" $feats -p rx --emit-ir --pattern "$pat" > "$ir" 2>&1
...
if pcrec_run "$PCREC" $AG -p rx --engine=vm --no-captures -fno-possessify \
        -o "$WORKDIR/ag_np.c" --pattern '(?:a|ab){1,3}+c' >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" $AG -p rx --engine=vm --no-captures \
        -o "$WORKDIR/ag_mark.c" --pattern '(?>a|ab)c' >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" $AG -p rx --no-captures -o "$WORKDIR/ag_dis.c" --pattern 'a*+b' >/dev/null 2>&1; then
...
ag_path() {  # ag_path <label> <pattern> <expect-rungs-mask> <min-cut> <min-2nd> <max-push>
    ...
    if ! pcrec_run "$PCREC" $AG -p rx --engine=vm --no-captures -o "$f" --pattern "$pat" >/dev/null 2>&1; then
...
while IFS=$'\t' read -r ag_poss ag_grp; do
    [ -n "$ag_poss" ] || continue
    pcrec_run "$PCREC" $AG -p rx --engine=vm --no-captures -o "$WORKDIR/ag_s.c" --pattern "$ag_poss" >/dev/null 2>&1 || continue
    pcrec_run "$PCREC" $AG -p rx --engine=vm --no-captures -o "$WORKDIR/ag_g.c" --pattern "$ag_grp"  >/dev/null 2>&1 || continue
    ag_eq=$((ag_eq + 1))
...
for agf in '' '-fno-possessify' '-fno-revdet' '-fno-counter'; do
    for agp in '(?>a*)b' '(?>(?:a|bc){2})d' '(?>(?:ab|b){8,})c' '(?>(?:a|bc)*)d' \
        ...
        pcrec_run "$PCREC" $AG -p rx --engine=vm --no-captures $agf -o "$WORKDIR/ag_sl.c" \
            --pattern "$agp" >/dev/null 2>&1 || continue
        ag_slots=$((ag_slots + 1))
...
for dd14_row in \
    ...
    dd14_want="${dd14_row%%|*}"
    dd14_pat="${dd14_row#*|}"
    if pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/dd14.c" --pattern "$dd14_pat" >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/dd14_free.c" --pattern '(a)(b)+c' >/dev/null 2>&1 \
   && pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/dd14_call.c" --pattern '(a(?1)?b)' >/dev/null 2>&1 \
   && pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/dd14_spl.c" --pattern '(a)(?1)' >/dev/null 2>&1; then
    dd14_leak=0
...
while IFS= read -r dd14_bpat; do
    ...
    pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/dd14_b.c" --pattern "$dd14_bpat" >/dev/null 2>&1 || continue
...
if pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/dd14_rb.c" --pattern '((?:a|b))(?1)' >/dev/null 2>&1; then
...
dd14_ctl='(?:(a)){1}(?1)b'
if pcrec_run "$PCREC" -p rx --features all -o "$WORKDIR/dd14_z.c" --pattern "$dd14_zero" >/dev/null 2>&1 &&
   pcrec_run "$PCREC" -p rx --features all -o "$WORKDIR/dd14_d.c" --pattern "$dd14_def"  >/dev/null 2>&1 &&
   pcrec_run "$PCREC" -p rx --features all -o "$WORKDIR/dd14_c.c" --pattern "$dd14_ctl"  >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" -p rx --features all --engine=vm -o "$WORKDIR/fb_vm.c" --pattern '^(a(?1)?b)$' >/dev/null 2>&1 &&
   pcrec_run "$PCREC" -p rx --no-captures -o "$WORKDIR/fb_dfa.c" --pattern 'a(b|c)+d' >/dev/null 2>&1; then
    ...
    for fb_tr in "recursion:^(a(?1)?b)\$:48" "none:(a|aa)+b:32"; do
        ...
        if ! pcrec_run "$PCREC" -p rx "${fb_tr_flags[@]}" -o "$WORKDIR/fb_tr.c" --pattern "$fb_tr_pat" >/dev/null 2>&1; then
    ...
    # never authored here. It had drifted by exactly the amount that predicts:
    # `20->21` and `21->22` were missing (each later wave appended its own
    # transition instead of backfilling), and both were backfilled from §6 in
    # this change.
    ABI_EXPECT=37
    if [ "$fb_abi_vm" != "$ABI_EXPECT" ] || [ "$fb_abi_dfa" != "$ABI_EXPECT" ]; then
        bad "[DD-14.FB] (§10.4): rx_info.abi is $fb_abi_vm (VM) / $fb_abi_dfa (DFA), expected $ABI_EXPECT on both — the emitted scaffolding's version (D76), bumped by [DD-14.FB]'s four sizing fields (2->3), by [DD-13]'s DFA selection stamps (3->4), by [OPT-1]'s two-tier entry (4->5), by [DD-13c]'s empty-scan value + hybrid scan stamps + the two rx_info mirrors (5->6), and by [OPT-3]'s pre-multiplied DFA transition table (6->7 — the FIRST bump that moves emitted PROGRAM bytes and not scaffolding only: the tables, the state variables, the dead test and the transition line, plus the new <PREFIX>_DFA_TABLE stamp), and by [ENG-FORM]'s opaque DFA state token (7->8 — the largest emitted-text event so far: a file-scope block of static inline state accessors per machine, and a scan loop rewritten against them, with no struct offset moved and no stamp VALUE changed), and by [OPT-K]'s offset-k candidate-start skip (8->9 — a <PREFIX>_DFA_PREFILTER_OFFSETS stamp line on EVERY DFA artifact, plus, on an artifact that selects the form, a file-scope <prefix>_ofsskip block, up to three candidate tables and a changed prefilter body with a reseed in it), and by [ENG-ABS]'s anchored match-here form (9->10 — a <PREFIX>_DFA_MATCH stamp line and an rx_info.match_form field on EVERY artifact, plus, on a DFA artifact that selects the form, a file-scope <prefix>_anchored_state accessor block, a THIRD machine's tables inside <prefix>_match, and rewritten _match/_match_caps bodies), and by [ART-SIZE]'s emitted-size term and caps (10->11), and by [OPT-4]'s prefilter-language stamp (11->12 — a <PREFIX>_VM_PREFILTER_LANG line AND its <PREFIX>_VM_PREFILTER_LANG_WHY companion on every VM HYBRID and on no other artifact; on the few hybrids above PCREC_PREFILTER_EXACT_NFA_STATES the two lines also come with a smaller inlined prefilter's tables and a dropped prefilter-window MRL ceiling; the SAME bump also adds <PREFIX>_ENGINE_SEL, unconditional on EVERY artifact and both engines, which is why a DFA artifact's scaffolding moves at abi 12 as well), and by [OPT-5]'s DFA SCAN EDGE (12->13 -- a <PREFIX>_DFA_SCAN_EDGE stamp line on EVERY artifact, plus, on any DFA scan whose machine carries a counted class run, one in-loop scan block per edge, a membership table per non-range edge, and the run's interior states DELETED from every per-state table: this is the first bump that moves the MACHINE and not only the emitted text), and by [CC-CLANG]'s clang-compatibility pair (13->14 -- an __has_attribute guard around emit_search_head's noclone line on EVERY DFA and VM-hybrid artifact, plus the fail label's pop-and-resume goto* dispatch omitted entirely on a FRAMELESS VM artifact with no RX_PUSH and no RX_CALL site: no answer moves on either half), and by [DD-13b.W1.2]'s rx_info.name + nentries (14->15 -- two APPENDED struct members and two initializer lines on EVERY artifact of both engines, no struct offset moved, no emitted program byte moved and no stamp VALUE changed: the first scaffolding-only bump since [OPT-1]), and by [OPT-5] STEP 2's START-PINNED SEARCH (15->16 -- the mirror image of the bump before it: EVERY artifact of both engines gains an APPENDED rx_info.search_form member and its initializer line (no struct offset moves; it is appended after `nentries`, the [DD-13c] discipline's substance rather than its dated \"after match_form\" wording), every artifact CONTAINING a DFA scan (a VM HYBRID included) gains a <PREFIX>_DFA_START stamp line, and on any artifact whose forward machine's start state accepts unconditionally the whole REVERSE machine leaves the file -- its tables, its accessor block, its scan-edge membership tables and the reverse scan loop -- with <PREFIX>_DFA_TABLE and <PREFIX>_DFA_SCAN_EDGE's folds no longer reading a machine the artifact does not contain; no answer moves either way, which is what -fno-start-pinned sweeps; RIDING the same bump by manager ruling, [OPT-VMFL]'s <PREFIX>_VM_FRAMELESS joins it -- one more stamp line on EVERY VM artifact and every hybrid, 1 iff the program emits no RX_PUSH site and no linked call so the fail label has no pop-and-resume dispatch, held to the artifact's own scoped goto* count by tests/codegen/run_vm_frameless.sh), and by [CC-DIFF] STEP 1's two emitted-code spellings (16->17 -- ONE event for both, since both are emitter changes on the same landing: (a) the eight VM entry-chain statics (<prefix>_run_state_bind/_run_state_init/_reset_for_next_attempt/_match_anchored/_report_captures and the three <prefix>_{search,match,match_caps}_run) gain 'static inline __attribute__((always_inline))' IFF <PREFIX>_VM_FRAMELESS is 1, so a FRAMED artifact is byte-identical to abi 16 and a frameless one loses its out-of-line <prefix>_search_run / <prefix>_match_anchored copies, its 152-byte entry frame and its stack-protector canary; and (b) a DFA transition or accept table whose cells are ALL EQUAL is NOT EMITTED and its accessor returns the constant, dropping the table parameter but keeping the state and class parameters so a call site's subject[pos++] is still evaluated, with a new <PREFIX>_DFA_UNIFORM_FOLDS integer stamp on every artifact containing a DFA scan counting how many folded (0..6; <PREFIX>_DFA_TABLE keeps naming the ENCODING that was selected, which still fixes the folded constant, so a 'premultiplied' artifact can read 4 here and carry no table) -- no answer moves on either half), and by [ENG-ISL] STEP 1's VM ALTERNATION ISLAND (17->18 -- a flat alternation whose whole subtree matches a FINITE SET OF LITERAL BYTE STRINGS is lowered as a TRIE over those strings (a byte compare at a node with one child, a switch at a node with several, one try site per node where an alternative ends, and the candidate list at each end node written out in ascending original index) instead of vm_alt's chain of one resume frame per untried branch; EVERY VM artifact gains a <PREFIX>_VM_ALT_ISLANDS line whatever its value, and on any artifact that takes an island the emitted PROGRAM changes shape -- which makes this the FIRST bump in this list whose change reaches the VM program REGION itself rather than the scaffolding, tables, entry chain or prefilter above it, so run_recursion_identity.sh's comparison (A) carries an IFF for it (a moved region is excused only where the artifact's own RX_VM_ALT_ISLANDS reads > 0, and an artifact that stamps one whose region did NOT move fails); no answer moves either way, which is what -fno-alt-island sweeps), and by [OPT-EDGE] STEP 1's SHARED-SENTINEL SCAN-EDGE DISPATCH (18->19 -- this event is [OPT-EDGE] alone after K43 (b) was dropped from it on measurement. On any DFA machine carrying a scan edge the edge HEADS are renumbered to the machine's TOP rows, the loop emits one extra accessor <prefix>_<m>_is_stop (folded to the constant 1 where every state is a head) and its ONE per-iteration state test now answers dead-or-head, and the per-edge 'if (state == HEAD && ...)' blocks move off the generic path onto an edge path reached only from that test -- so the emitted state NUMBERS of an edge-bearing machine move, while an artifact with NO scan edge (which includes every -fno-scan-edge build) is byte-identical to abi 17. A new precondition (8) in src/opt/scanedge.c additionally refuses a head that any SEED family names, which measured 11 of 2,539 corpus artifacts losing an edge -- 10 of them all of theirs, every one a \\b/\\B pattern stamping RX_DFA_SCAN_EDGE \"none\" where abi 17 stamped a body form. No answer moves: test-axes is answer-identical on all 21 axes), and by [DD-13b.W1.3]'s COMPOSITION (19->20): on a COMPOSED artifact (pcrec --source) rx_info.groups[] gains rows the target pattern did not declare, each carrying a non-NULL .ref naming the DEFINITION it came from -- the column D61 reserved and every artifact has emitted as a literal NULL until now; the array's sort key gains a LEADING SCOPE term (caller-scope rows, which are the pattern's own names AND any flat imports, before site-scoped `site.group` rows), so .nnames counts the caller-scope prefix and match_api section 6's shipped bsearch is correct unchanged while .nentries counts the whole array; .ngroups reads the PRIMARY pattern's own count so a definition's slots sit above it; and a DELIVERING call site retains what its callee matched, which adds trailed copies at that site's return and forces it to CALL_SPLICE. EVERY ONE OF THESE IS INVISIBLE ON A NON-COMPOSED COMPILE by construction -- ncap_primary equals ncap, no injected rows, .ref still NULL -- which is what makes identity gate (A) a real control on this change: MEASURED 176 artifacts / 0 differing against a compiler built from main), and by [OPT-EDGE] STEP 1.1's SCAN-EDGE ENTRY DISPATCH (20->21 -- the entry-seed check is generalised to 'is_stop && !is_dead' and precondition (8) now requires the seed AND a prefilter reseed, so the ENTRY BLOCK's shape moves on every edge-bearing artifact and on nothing else; no answer moves), and by [CC-DIFF] STEP 2's VM ENTRY SHAPE (21->22 -- two stamps on every VM artifact, <PREFIX>_VM_ENTRY_SHAPE and <PREFIX>_VM_PROGRAM_BYTES, plus the entry chain's shape at rungs 'shared'/'forward'; the VM program region is unmoved, and _VM_PROGRAM_BYTES deliberately costs STEP 1's \"framed artifacts are byte-identical\" property), and by [FORM-CHAR] STEP 1's ASCII-FOLD CLASS TEST (22->23 -- a two-member VM pool class that is an ASCII fold pair (differing only in bit 0x20, both letters, what D23's parse-time caseless folding produces) tests as '(byte | 0x20) == lower' with NO 32-byte bitmap table emitted, every VM artifact gains a <PREFIX>_VM_CLS_FOLDS activity-count line whatever its value, and the VM program REGION moves on exactly the fold-bearing population, so run_recursion_identity.sh's comparison (A) carries a SECOND deny-axis IFF (-fno-cls-fold must restore the pinned region, the island's shape one axis over); no answer moves either way, which is what -fno-cls-fold sweeps) , and by [K50]'s CHARACTER-BOUNDARY CANDIDATE STARTS (23->24 -- EVERY artifact of both engines gains TWO lines -- '#define PCREC_ERR_STARTPOS (-7)' in the shared PCREC_RX_ABI_H block, a below-the-floor CALLER-ARGUMENT REFUSAL rather than a give-up, and a <PREFIX>_STARTPOS_GUARD selection stamp ('guarded'/'permissive') in the shared prologue; no byte artifact moves in any other way (the utf8 backend supplies the character-start set and the byte backend supplies none, so the IR gate is not built, the class axis never produces UPC_NOSTART, ENG_ATTEMPT's loop gains no boundary continue and the entries emit no guard); a utf8 UNANCHORED DFA artifact's MACHINE moves, since the gate is a real NFA state the subset construction sees; a utf8 ENG_ATTEMPT artifact gains the loop's continue; and every utf8 artifact gains the caller-startpos guard at its entries. This is the FIRST bump in this list on a change that moves an ANSWER: \\B over 61 CE B1 from startpos 0 answered (2,2) and answers (3,3), which is libpcre2 10.46's answer under both UTF option words, so -fno-startpos-guard sweeps the CALLER-side half only and test-axes carries that half as a documented floored divergence class rather than an identity claim), and by [PORTFIX]'s CLANG-COMPATIBILITY LABEL FIX (24->25 -- every scan-edge-bearing DFA machine (or VM-hybrid inlined prefilter) of either direction gains a trailing ';' on its scan_views label and its scan_edge label ('<p>_<m>_scan_views:' -> '<p>_<m>_scan_views:;', same for scan_edge), closing a label-immediately-followed-by-a-declaration shape (view_decl's <dir>_view_state assignment) that gcc accepts as a -std=gnu11 extension and clang 21 rejects as -Wc23-extensions under -Werror; an artifact with no scan edge at all gains nothing, exactly as -fno-scan-edge already excuses it from every earlier scan-edge bump; no answer moves on either compiler), and by [OPT-DIAL]'s SPEED-VS-SIZE DIAL (25->26 -- every artifact of both engines gains exactly ONE line, \`#define <PREFIX>_TUNE \"<token>\"\`, a closed five-token selection stamp in the shared prologue, 23-28 bytes by token; nothing else moves at the default position \`balanced\`, which is STRUCTURAL rather than measured because every cell of that row in src/core/tune.c is the em-dash sentinel and its deny mask is empty; a non-default position may move far more, which is the dial's point, but no struct offset moves, no rx_info member is added or changed, and no answer moves at any position, which is the dial's own acceptance criterion), and by [EMIT-VERB]'s EMITTED-COMMENT AXIS (26->27 -- the artifact's NON-ESSENTIAL prose is no longer emitted by default (D112): every comment LINE leaves every artifact of both engines except the generated-by/pattern-echo provenance line and the shared PCREC_RX_ABI_H type block's doc-comments, which are the ESSENTIAL class; -fcomments restores the rest, and BOTH bits (PCREC_NO_COMMENTS, PCREC_FORCE_COMMENTS) are masked out of rx_info.flags. Measured 44.2% of a default artifact's source bytes over the 3,517 compiling corpus patterns (DFA 45.6%, VM 43.1%). This is the FIRST bump in this list that REMOVES emitted text rather than adding it, and the only one whose whole population is bytes the C compiler discards: the OBJECT FILE is byte-identical under both settings on 3,517 of 3,517 corpus artifacts, the comment-EXCLUDED source size is identical, and every emitted #define is identical -- so no cap, no refusal and no answer moves), and by [REL-1.4]'s VERSION STAMP (27->28 -- D115: the ESSENTIAL generated-by line, already the one line D112 keeps under every comment setting, now also names PCREC_VERSION (lib/pcrec.h) beside the abi digit -- '/* Generated by pcrec 0.1.0-beta (abi 28). Pattern: ... */' -- no struct offset moves, no rx_info member is added or changed, and no answer moves at all: the comment TEXT is the only thing that changed, on every artifact of both engines regardless of -fcomments), and by [OPTLOOP.1] batch 1's THREE WHOLE-WINDOW PRE-CHECKS (28->29 -- D119, ONE bump for the three because they are one emitted-scaffolding event on one landing: every artifact of BOTH engines gains TWO shared-prologue stamp lines, <PREFIX>_END_WINDOW and <PREFIX>_REQ_BYTE (each a string carrying a number or the token 'none', because 0 is a legal value of both facts and no number is free to mean 'declined'), and every VM artifact gains a THIRD, <PREFIX>_VM_START ('anchored'/'gstart'/'unanchored'); on the populations the three analyses actually reach the artifact additionally gains emitted PROGRAM text -- [OPT-ANCHOR-VM] a 'const size_t attempt_max = search_from;' declaration and its existing attempt-loop test rewritten to read it, [OPT-ENDWIN] a two-line clamp raising search_from to subject_length - W at both engines' search entries, [OPT-REQBYTE] a three-line memchr pre-check at those same two places plus a #include <string.h> on a VM artifact that had none. No struct offset moves, no rx_info member is added or changed, and no answer moves on any of the three: two of them remove only attempts that would have run and FAILED, which is why their sabotage rows S263/S265 are a STAMP row and an inverted-sense row rather than two of a kind, and the third moves a search's start position only within a window it has proved no match can begin before (tests/assertions/end_window.rxt is its 66-case answer-level net). -fno-vm-anchor-bound / -fno-end-window / -fno-req-byte each restore the pre-mechanism text on their own population exactly, which is what makes test-axes' sweep of the three new bits a control rather than a comparison of two new shapes), and by [OPTLOOP.2] batch 2's NECESSARY LITERAL RUN AND ITS SCAN PICK (29->30 -- D119, ONE bump for two mechanisms because their populations overlap and two bumps would re-pin the same manifests twice: every artifact of BOTH engines gains ONE shared-prologue stamp line, <PREFIX>_REQ_RUN (the run's bytes as lowercase hex, then '@', then the scanned member's index within them, or the token 'none' at L < 2 -- its two siblings' own 'none'-member shape and for their reason). On the run analysis's own population (18.6% of the corpus) the three-line memchr pre-check abi 29 added is REPLACED by a scan loop of about ten lines -- one memchr for the run's rarest member, one constant-length memcmp of the run at each hit, and a window guard of one or two conjuncts depending on whether the scanned member is the run's first byte. On the DISJOINT population with a necessary byte and no run, the emitted text is that same three-line check with, on 13.60% of the corpus, a DIFFERENT decimal in it: [OPT-FREQPICK] chooses the member of the necessary set a subject is least likely to contain rather than PCRE2's rightmost one, with PCRE2's rule surviving as the tiebreak and as the whole answer under every encoding the prior is not keyed to. No struct offset moves, no rx_info member is added or changed, and NO ANSWER MOVES: every byte of every run is a byte every match must contain, so both the wider check and the different pick can move a speed and nothing else. -fno-req-run restores abi 29's emitted text on the run population exactly and -fno-req-byte restores the pre-batch-1 text on both; the flags enum in lib/pcrec.h is respelled '1ull << N' in the same change, since PCREC_NO_REQ_RUN is bit 31 and '1u << 32' is undefined behaviour, with no value moved), and by [OPT-PRECHECK-ADMIT]'s ADMISSION OF THE WHOLE-WINDOW PRE-CHECK (30->31 -- D119/[OPTLOOP.1] ledger reading section 6, ratified G1+G2: every artifact of BOTH engines gains ONE shared-prologue stamp line, <PREFIX>_REQ_WHY, a closed four-token set ('emitted'/'none'/'one-attempt'/'dominated') saying why the artifact does or does not carry a [OPT-REQBYTE]/[OPT-REQPOS] pre-check -- <PREFIX>_ENGINE / <PREFIX>_ENGINE_WHY's own split, so that <PREFIX>_REQ_BYTE and <PREFIX>_REQ_RUN keep naming ONLY what the emitted check tests and no existing reader's value space moves. On the two declined populations the artifact LOSES emitted text: the three-line memchr pre-check (or tier 2b's scan loop) and, where nothing else in the body calls memchr, the #include <string.h> with it -- G2 where the route runs ONE attempt (the DFA's start_max row that is the literal 0 or search_from, i.e. dfa_interior_dead(d->s1u), which is the same predicate attempt_cand already declines the candidate-start prefilter on; the VM's RX_VM_START 'anchored'/'gstart'), and G1 where the artifact's own candidate-start memchr already scans a byte at least as rare by pcrec_byte_freq_ppm (identity alone under any encoding but byte). No struct offset moves, no rx_info member is added or changed, and NO ANSWER MOVES: a declined pre-check is a check not run, and the check could only ever return the answer the engine below it then returns anyway. -fno-req-byte still removes the whole mechanism, so its sweep is the control -- one bump per change, D76), and by [VAR]'s CALLER-VARIABLE SURFACE (31->32 -- module `vars` gives a pattern ${name}, whose bytes the CALLER supplies per call, and the whole surface lands in one event. On EVERY artifact of BOTH engines: a new fixed-literal ABI type rx_var {name,p,len} in the shared PCREC_RX_ABI_H block, TWO fields APPENDED to rx_ctx (const rx_var *vars; size_t nvars;) so no existing member's offset moves and -- the point of putting them there rather than on the entries -- rx_matchfn's signature is UNTOUCHED and a var-bearing artifact's <prefix>_match is still an rx_matchfn byte for byte, a #define PCREC_ERR_UNSET_VAR (-8) as the THIRD below-the-floor code in PCREC_ERR_STARTPOS's class (a caller REFUSAL: nothing attempted, caps untouched), and TWO members APPENDED to rx_info (const char *const *vars; int nvars;) carrying the variable-NAME table on rx_info.groups' model. A VAR-FREE artifact's whole change is the abi digit (a same-length substitution) plus those two rx_info initializer lines -- MEASURED at exactly +34 bytes on four witnesses spanning 13 KB to 762 KB written to the SAME -o basename, six changed lines each and no emitted program byte among them, which is stronger than the design's own \"unchanged except for the abi digit\" wording and is why tests/resource's a{5,25000} rescue pin moves 762367 -> 762401. On a VAR-BEARING artifact only: <PREFIX>_NVARS and one internal <PREFIX>_VAR_<NAME> index macro per name (the ARTIFACT's own index, never something a caller writes -- variables are passed BY NAME, because an index is meaningless across separately compiled artifacts), a <prefix>_var_names[] table rx_info.vars points at, a <prefix>_vars_resolve static that scans the caller's array ONCE PER CALL and evaluates each distinct expansion, the encoding seam's fifth and sixth residual entries $_var_match/$_var_match_caseless plus (under a multi-byte encoding) a seventh $_var_valid, and a trailing const rx_var *vars, size_t nvars pair on <prefix>_search and its _in siblings -- which are not rx_ctx-shaped and so cannot read the ctx, <prefix>_search_in's own trailing-descriptor precedent with the descriptor staying LAST. NO ANSWER MOVES on any artifact that mentions no variable, and a var-bearing pattern could not be compiled at all before this bump, so there is no answer to move on the other side either, and by [K64] fix A's LINEARITY CONJUNCT ON G2 (32->33 -- [OPT-PRECHECK-ADMIT]'s G2 rule declined the necessary-byte pre-check wherever the VM route tries at most one start position, arguing the attempt reads no more than the check would scan -- true of a DFA and false of a backtracking VM program, where the pre-check is also the NO-MATCH PROOF bounding the call: a framed, unguarded, ^-anchored forced-VM one-attempt program gave up on its step budget where PCRE2 answers NOMATCH (K64). req_route_one_attempt now additionally requires the one attempt to be LINEAR -- an EXACT-language hybrid DFA in front, or a frameless VM program, which cannot backtrack at all -- via a new Job.vm_frameless field published beside emit_vm's own has_push derivation. No new stamp, no declaration and no layout change: the bump moves a STAMP VALUE (<PREFIX>_REQ_WHY \"one-attempt\" -> \"emitted\") and the emitted PROGRAM TEXT it names, on 176 of 6,634 measured artifact-configs (256 bench + 6,378 corpus over 3,189 patterns x --features all/+--engine=vm), every one VM_FRAMELESS 0 / VM_PREFILTER \"none\" / VM_START anchored-or-gstart, gaining back either the 5-line byte check or the 16-line run check plus #include <string.h> -- including 41 on the AUTO route (backreference and linked-call VM artifacts, which decline the hybrid outright and so have the K64 hazard too). This is the SECOND bump in this list (after [K50]'s 23->24) whose change moves an ANSWER, and only in the direction of REPAIR: a give-up becoming a NOMATCH within the same step budget, never the reverse. -fno-req-byte still removes the whole mechanism on both routes), and by [K65]'s WHOLE-SET PRE-CHECK (33->34 -- on a VM artifact with no DFA scan in front (RX_VM_PREFILTER \"none\") an emitted necessary-byte pre-check no longer rests on the one member the pick chose: after the byte check or the run check, a 6-line block memchr's every remaining member of the necessary set (a static const rq_set[] array and a loop), any absent one answering NOMATCH, so a hostile subject's NOMATCH-vs-give-up no longer follows a SPEED choice (K65). No new stamp, no stamp value, no declaration and no layout change; MEASURED on 452 of 6,642 census artifact-configs (30 bench + 422 corpus), every one a pure 6-line insertion and none a DFA artifact or a hybrid. The THIRD bump in this list whose change moves an ANSWER, again only in the direction of REPAIR: a give-up becoming a NOMATCH, never the reverse. -fno-req-byte still removes the whole mechanism), and by [K66]'s WHOLE-RUN COMPARE (34->35 -- on the same no-DFA-scan VM route, a necessary run longer than the 8-byte window RX_REQ_RUN names is ALSO compared whole, by a second scan loop of the window's own shape, so the no-match proof no longer rests on the prior's window pick (K66); [K65]'s rq_set block drops the whole run's bytes. No new stamp, no stamp value, no declaration and no layout change; MEASURED on 12 of 6,642 census artifact-configs (2 bench + 10 corpus), each +13/-6 lines. The FOURTH bump in this list whose change moves an ANSWER, again only in the direction of REPAIR), and by [OPT-LITSCAN] S1's RUN-PINNED PREFILTER ROWS AND G1's WIDENING (35->36 -- two new RX_DFA_PREFILTER values, 'run-pinned'/'run-pinned-bounded', whose offset-skip block compares a pinned necessary run as one constant-length memcmp, and a run pre-check the selected prefilter's test dominates is not emitted; new axis bit 32, -fno-run-prefilter; no answer moves), and by [OPT-LITSCAN] S1 step 6's RUN PRE-CHECK AS A CALL OF THE ONE SEARCH BLOCK (36->37 -- the run pre-check's scan loop, and [K66]'s whole-run loop where one is emitted, each become a file-scope static inline <prefix>_reqrun / <prefix>_reqrun_whole written by the offset-skip block's own emitter and one call line at the search entry; the empty-window line goes, its job done by the block's loop guard. No stamp, stamp value, declaration or layout moves, and no answer moves)"
...
while IFS= read -r w12_pat; do
    w12_out="$("$TIMEOUT_BIN" 20 "$PCREC" -p rx -o - --pattern "$w12_pat" 2>/dev/null)" || continue
...
for w12_px in rx r myprefix a_very_long_prefix_name; do
    w12_got="$("$TIMEOUT_BIN" 20 "$PCREC" -p "$w12_px" -o - --pattern 'a(b|c)+d' 2>/dev/null \
               | grep -m1 '^    \.name = ' || true)"
    [ "$w12_got" = "    .name = \"$w12_px\"," ] || \
...
for w12_pair in "(?<g>a)(?<h>b):2" "a(b)c:0" "(?<z>x)|(?<a>y):2"; do
    ...
    w12_art="$("$TIMEOUT_BIN" 20 "$PCREC" -p rx --features named-groups -o - --pattern "$w12_p" 2>/dev/null)" || {
