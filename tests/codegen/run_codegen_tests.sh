...
gen() { # gen <name> <pattern> -> $WORKDIR/<name>.c plus <name>.body
    "$PCREC" -p rx -o "$WORKDIR/$1.c" -- "$2" >/dev/null 2>&1 \
        || { bad "$1: pcrec failed to compile pattern '$2'"; return 1; }
    body "$WORKDIR/$1.c" rx_search "$WORKDIR/$1.body" \
        || { bad "$1: could not extract the rx_search engine body from the generated C"; return 1; }
}
