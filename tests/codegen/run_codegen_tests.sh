...
if pcrec_run "$PCREC" -p rx --features std1 -o "$WORKDIR/stamp.c" -- 'a' >/dev/null 2>&1 \
   && pcrec_run "$PCREC" -p rx --features std1 -fcomments -o "$WORKDIR/stampc.c" -- 'a' >/dev/null 2>&1; then
...
if pcrec_run "$PCREC" -p rx -o - -- 'a' 2>/dev/null | grep -qF '#define PCREC_FEATURE_SET "std1"'; then
...
if pcrec_run "$PCREC" -p rx --features none -o - -- 'a' 2>/dev/null | grep -qF '#define PCREC_FEATURE_SET "none"'; then
