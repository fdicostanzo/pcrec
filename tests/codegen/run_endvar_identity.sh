...
# were two levels down at src/gen/enc/ (run_trie_identity.sh's own [M5-SEAM]
# note; they are src/enc/ since [REVW.3] wave 3, and the tree has now changed
# shape TWICE under a glob that encodes an assumption about its depth). A
