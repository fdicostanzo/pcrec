...
LOADED=0
if num_gt "$LOAD_1MIN" "$LOAD_LIMIT"; then LOADED=1; fi
...
fi
{
    if [ "$LOADED" = "1" ]; then
        ...
        echo "(1-minute load $LOAD_1MIN exceeds this report's busy-box threshold of"
        ...
        echo "Generated by \`tests/bench/compare/compare.sh\` on $DATE_ISO."
        echo
        echo "Box was quiet at generation time (1-minute load $LOAD_1MIN, at or below"
        echo "this report's busy-box threshold of $LOAD_LIMIT -- same threshold"
        echo "tests/bench/run_bench.sh's LOADED/INCONCLUSIVE gate uses, cores/2 clamped"
    ...
    echo "| busy-box verdict | $([ "$LOADED" = "1" ] && echo "LOADED (1-min $LOAD_1MIN > $LOAD_LIMIT)" || echo "quiet (1-min $LOAD_1MIN <= $LOAD_LIMIT)") |"
    echo "| pinning | $PIN_NOTE (cpu $BENCH_CPU) |"
