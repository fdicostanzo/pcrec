...
- **reject/** — the "unsupported constructs fail cleanly, never miscompile" mandate, asserted per construct (137 hand-written rejections + 66 reached by iterating `pcrec --list-syntax`, + 37 accept-controls + 5 known-wrong pins; the two layers catch different things and neither replaces the other — see its CLAUDE.md). Cannot live in .rxt: a `perr` block asserts only THAT a pattern is rejected, never WHY, and the module name is the point. 20 of the rejections are the base-grammar brace errors from FIX-1 (K5/K6), which have no registry row and name a PCRE2 error instead of a module
