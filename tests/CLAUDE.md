...
- **reject/** — the "unsupported constructs fail cleanly, never miscompile" mandate, asserted per construct (93 rejections + 19 accept-controls). Cannot live in .rxt: a `perr` block requires the PYTHON oracle to fail too, and python compiles `\d`, `\b`, `(?i)` and most of the rest
- **registry/** — the SR-1 syntax construct table checked against the parser in both directions, including a 255-byte sweep of each of the four doorways that catches a construct added to parse.c with no registry row (D24)
