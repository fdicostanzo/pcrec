...
if [ "$rc" -eq 0 ] && printf '%s' "$log" | grep -q 'dropped the premultiplied DFA transition table'; then
    ...
    #
    # RE-PINNED AGAIN 762114 -> 762125, 2026-09-21 (lane rel1b, [REL-1.4]),
    # and it is the SAME READER CLASS a third time -- FIFTH recorded
    # instance. D115's version stamp rides the abi 27 -> 28 bump and put
    # `PCREC_VERSION` (" 0.1.0-beta") into the SAME essential generated-by
    # line the abi digit already lives on, which every artifact carries
    # whether or not comments are on -- so this cell moves again for the
    # identical structural reason, one line, no abi-number grep can reach.
    #
    ...
    #   +9      the rider's ` (abi 27)` insertion -> 762114, the previous pin
    #   +11     [REL-1.4]'s ` 0.1.0-beta` insertion -> 762125, today
    # Confirmed by diffing the two artifacts, not inferred from the size: the
    # only differing lines are the generated-by header (now "Generated by
    # pcrec 0.1.0-beta (abi 29).").
    #
    # RE-PINNED AGAIN 762125 -> 762312, 2026-09-22 ([OPTLOOP.1] batch 1,
    # D119), and this time the abi grep WOULD have found it (the abi digit
    # moves 28 -> 29 in the same line) -- but the BYTES do not come from that
    # digit, which is a same-length substitution. They come from the batch's
    # own emitted text, and this cell is a reader of it for the SECOND READER
    # CLASS's reason a sixth time. The arithmetic, measured by diffing two
    # artifacts written to the SAME `-o` basename (the house's recorded
    # basename trap, fifth instance at lane evtriage3):
    #   762125  the previous pin
    #   +29     `#define RX_END_WINDOW "none"`   ([OPT-ENDWIN], declines here)
    #   +26     `#define RX_REQ_BYTE "97"`       ([OPT-REQBYTE], the `a`)
    #   +132    the three-line memchr pre-check in `<prefix>_search`
    #   = 762312
    # Those four lines are the ONLY differences besides the abi digit itself
    # and `.abi = 29` -- verified by reading the diff, not by subtracting the
    # sizes. The `-fcomments` figure quoted above moves by the same 187.
    #
    # RE-PINNED AGAIN 762312 -> 762338, 2026-09-22 ([OPTLOOP.2] batch 2,
    # D119), the SECOND READER CLASS's seventh instance and the abi digit's
    # own second: the digit moves 29 -> 30 in this same line and in
    # `.abi = 30`, both same-length substitutions, so the +26 comes from the
    # batch's one new stamp line and nothing else. VERIFIED BY DIFFING the two
    # artifacts written to the SAME `-o` basename (the house's recorded
    # basename trap), which prints exactly four changed lines -- the two abi
    # digits and one INSERTED `#define RX_REQ_RUN "none"`:
    #   762312  the previous pin
    #   +26     `#define RX_REQ_RUN "none"`   ([OPT-REQPOS], declines here:
    #           `a{5,25000}` is a repeat over a ONE-byte body, and nothing is
    ...
    #   = 762367
    #
    # RE-PINNED AGAIN 762367 -> 762401, 2026-09-23 ([VAR] M7, abi 31 -> 32),
    # the SECOND READER CLASS's ninth instance and the abi digit's own fourth:
    # the digit moves 31 -> 32 in this same line and in `.abi = 32`, both
    # same-length substitutions, so the +34 comes from the change's two
    # APPENDED `rx_info` initializer lines and nothing else. VERIFIED BY
    # DIFFING the two artifacts written to the SAME `-o` basename (the house's
    # recorded basename trap), which prints exactly six changed lines -- the
    # two abi digits and two INSERTED initializer lines:
    #   762367  the previous pin
    #   +18     `    .vars = NULL,`
    #   +16     `    .nvars = 0,`
    #   = 762401
    #
    # +34 IS THE WHOLE CHANGE FOR EVERY VAR-FREE ARTIFACT, measured on four
    # witnesses spanning 13 KB to 762 KB at the same `-o` basename: the same
    # six changed lines and the same delta on every one. That is [VAR]'s own
    # MECH-M1 claim, and it is STRONGER than the design's wording ("unchanged
