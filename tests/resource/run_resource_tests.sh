...
if [ "$rc" -eq 0 ] && printf '%s' "$log" | grep -q 'dropped the premultiplied DFA transition table'; then
    ...
    # pcrec 0.1.0-beta (abi 28)."). The `-fcomments` figure quoted above
    # re-measures to 769,855 at this tree.
    sz=$(wc -c <"$out" | tr -d ' ')
    if [ "$sz" -eq 762125 ]; then
        ok "'a{5,25000}' -fno-scan-edge -fno-start-pinned is rescued by [K59-PREMUL]'s drop ladder at 762125 bytes (was 1104674 before the rung existed; 769835 before emitted comments went off by default; 762105 before the abi joined the generated-by line; 762114 before the version joined it) — the cap still works, this witness no longer reaches it"
    else
        bad "'a{5,25000}' -fno-scan-edge -fno-start-pinned rescued at $sz bytes, pinned 762125 — the rung's own byte count moved; re-measure and re-pin in the same commit if intended"
    fi
else
