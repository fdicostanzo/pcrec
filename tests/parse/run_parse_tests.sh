...
for n in 2 3 4 5; do
    for pos in 0 1 2; do
        ...
        a_out="$(pcrec_run "$PCREC" -p rx --no-captures -o - --pattern "$flat" 2>/dev/null | tail -n +2 | rx_search_body)"
        b_out="$(pcrec_run "$PCREC" -p rx --no-captures -o - --pattern "$grouped" 2>/dev/null | tail -n +2 | rx_search_body)"
...
def_out="$(pcrec_run "$PCREC" -p rx -o - --pattern 'a|(b|c)' 2>/dev/null)"
nc_out="$(pcrec_run "$PCREC" -p rx --no-captures -o - --pattern 'a|(b|c)' 2>/dev/null)"
...
for n in 249 250 251 252; do
    ...
    if pcrec_run "$PCREC" -p rx -o "$WORKDIR/depth.c" --pattern "$pat" >/dev/null 2>&1; then r=ok; else r=fail; fi
    case "$n:$r" in
...
for lead in 1 5 20; do
    ...
    if pcrec_run "$PCREC" -p rx -o "$WORKDIR/depth.c" --pattern "$pat" >/dev/null 2>&1; then
        balfail=$((balfail + 1))
