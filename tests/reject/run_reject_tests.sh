...
#   - a miss pcrec cannot classify (`\p{Greek}` is real and stage 5's,
#     `\p{Alphabetic}` is real and declined by design §3.4, `\p{Foo}` is
#     genuinely unknown, `\p{Script=Greek}` is a whole axis the table does
#     not cover): the UNBUILT tier, which is the true sentence for the first
#     and the closest honest one for the rest.
...
reject_gated unicode-props '\p{Greek}'        "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 9)"
...
reject_gated unicode-props '\p{Script=Greek}' "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 16)"
