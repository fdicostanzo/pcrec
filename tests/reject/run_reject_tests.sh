...
"*) bad "reject: a logged pattern contains a newline, which would split the coverage log and confuse the duplicate check. Pass a single-line display label as the third argument"
    ...
    out="$("$TIMEOUT_BIN" 60 "$PCREC" -p rx -o "$WORKDIR/out.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$?
    if [ "$rc" -eq 0 ]; then
...
accept() { # accept <pattern> [display-label] [features]
    ...
    if [ -n "$feats" ]; then
        out="$("$TIMEOUT_BIN" 60 "$PCREC" --features "$feats" -p rx -o "$WORKDIR/ok.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$?
    else
        out="$("$TIMEOUT_BIN" 60 "$PCREC" -p rx -o "$WORKDIR/ok.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$?
    fi
    if [ "$rc" -ge 124 ]; then
...
reject_gated() { # reject_gated <features> <pattern> <expected-substring>
    ...
    ngated=$((ngated + 1))
    rm -f "$WORKDIR/out.c" "$WORKDIR/out.h"
    out="$("$TIMEOUT_BIN" 60 "$PCREC" --features "$feats" -p rx -o "$WORKDIR/out.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$?
    if [ "$rc" -eq 0 ]; then
...
# [M5.0 stage 2] the BRACED form is BASE GRAMMAR now, range-checked against
# the encoding's own universe (utf8_design.md §2.7.3 — 10.46 accepts \x{41}
# at options=0 and refuses \x{3b1} there with err 134, accepting it only
# under UTF): in range compiles, above the default `byte` universe refuses
# with the err-134 analogue naming the encoding, and a surrogate is refused
# by name under any encoding that could reach one.
accept '\x{41}'
accept '\x{ff}'
accept '[\x{20}-\x{7e}]'
reject '\x{100}'     "character code point value in \\x{...} is too large"
reject '\x{3b1}'     "character code point value in \\x{...} is too large"
reject '\x{}'        "digits missing after \\x"
reject '\x{zz}'      "non-hex character in \\x{...}"
reject '\x{41'       "non-hex character in \\x{...} (closing brace missing?)"
...
echo
echo "== caller variables (module 'vars') =="
# [VAR] M10 THE MODULE GATE. `${` is a doorway ONLY with the module enabled,
# which is design_callout_abi.md §3's collision rule satisfied by the ordinary
# mechanism -- and here the gate protects a population of patterns that COULD
# NEVER MATCH: `$` asserts a position whose next byte is a newline and `{` is
# not one, so `${name}` is a spelling PCRE2 accepts and no subject can satisfy
# (variables_common.md §0.2, proved structurally and corroborated at 0 of
# 3,954 shipped `pattern` lines).
#
# SO THIS ROW IS A REFUSAL-SET MOVE ON AN EMPTY POPULATION, deliberately:
# before this module a gate-closed `${name}` COMPILED and matched nothing,
# and now it says which module would implement it. The alternative -- handing
# a caller who forgot `--features vars` an artifact that cannot match anything
# with no diagnostic -- is what D34 ruling 5's "recognisers are always live,
# production is gated" exists to prevent.
reject '${name}'      "\${...} requires module 'vars'"
reject 'a${v}b'       "\${...} requires module 'vars'"
reject '${!name}'     "\${...} requires module 'vars'"
reject '${v:-dev}'    "\${...} requires module 'vars'"

# A LONE `$` IS UNCHANGED and this is the control that says so: it is still
# the end-of-line assertion, with the module off AND on. Without it the four
# rows above would be consistent with a gate that swallowed every `$`.
accept '$x'
accept '$x' '$x (module vars enabled)' vars
accept 'a$b' 'a$b (module vars enabled)' vars

# THE GRAMMAR'S OWN REFUSALS, with the module ON -- each names the RULE it
# broke rather than a generic parse error, because a caller who mistypes a
# variable reference is owed which half of the production they missed.
reject_gated vars '${}'        "must start with a letter or an underscore"
reject_gated vars '${1}'       "must start with a letter or an underscore"
reject_gated vars '${-x}'      "must start with a letter or an underscore"
reject_gated vars '${v'        "unterminated \${...} variable reference"
reject_gated vars '${v:-d'     "unterminated \${...} variable reference"
reject_gated vars '${v*d}'     "expected an operator"
reject_gated vars '${v:*d}'    "expected ':-', ':+' or ':?'"
# THE BARE `?` FORM IS REFUSED BY NAME rather than by falling into the
# generic "expected an operator" arm: bash has `${name?word}` and this
# grammar deliberately does not (variables_common.md §1.1's table lists the
# colon form alone), so a caller reading bash's meaning into it is told
# exactly what to write instead.
reject_gated vars '${v?d}'     "write \${name:?word}"

# THE MVP's ONE NARROWING, refused by name with what to write instead: a
# default WORD that MIXES literal text with a nested reference would have to
# be ASSEMBLED at match time, and sizing that storage is a question the
# design does not rule and the MVP does not need. The fallback chain -- the
# one case the design calls "the only one with a real customer" -- is a
# SINGLE nested reference and is accepted, which is the row below it.
reject_gated vars '${a:-x${b}y}'  "would have to be assembled at match time"
accept '${a:-${b}}' '${a:-${b}}' vars

# THE ENGINE DECLINE is the registry row's, free through `forces_registry`
# (SR-8) -- no per-module line in select_engine.c, which is what that
# mechanism exists for.
reject_engine_dfa_vars() {
    callidx=$((callidx + 1))
    [ $((callidx % SHARD_TOTAL)) -eq "$SHARD_INDEX" ] || return 0
    local out rc
    out="$("$TIMEOUT_BIN" 60 "$PCREC" --features vars --engine=dfa -p rx \
           -o "$WORKDIR/out.c" --pattern '${v}' 2>&1 >/dev/null)"; rc=$?
    case "$rc:$out" in
        1:*"requires the VM engine, which --engine=dfa excludes"*)
            ok "--engine=dfa on a var-bearing pattern refuses, naming the construct" ;;
        *) bad "--engine=dfa on '\${v}': exit $rc, message: $out" ;;
    esac
}
reject_engine_dfa_vars
...
# [M5.0 stage 3] THE OPEN-GATE HALF. Every pin above is at the CLOSED gate
# and every one of them is byte-identical to what it was before the producer
# landed — which is the property the stage's wording split was designed for
# and is worth more than a re-baseline would have been.
#
# What the OPEN gate must not say is "requires module 'unicode-props'", to a
# user who has just enabled `unicode-props`. That is K14's shape and ext.c's
# own UNBUILT epilogue exists to avoid it, so mod_uprops.c splits three ways
# and each way is pinned here:
#
#   - a name pcrec KNOWS: no refusal at all (the accept controls below).
#   - a miss on the axis pcrec's table is EXHAUSTIVE for (one significant
#     character, no `=`): an UNKNOWN NAME, and no module will ever implement
#     it, so the module clause is DROPPED.
#   - a miss pcrec cannot classify (`\p{Alphabetic}` is real and declined by
#     design §3.4, `\p{bc=L}` is a whole AXIS the table does not cover,
#     `\p{sc=Nosuch}` is a known axis with an unknown value, `\p{Hrkt}` is a
#     UCD script value no libpcre2 has, `\p{Foo}` is genuinely unknown): the
#     UNBUILT tier, the closest honest sentence for all of them.
#
# **[M5.0] STAGE 5 RETIRED TWO OF THESE ROWS BY SHIPPING THEM.** `\p{Greek}`
# and `\p{Script=Greek}` were pinned here as the "real but stage 5's" cell;
# they COMPILE now, so pinning them would pin a lie. They are replaced one for
# one by `\p{bc=L}` (the axis cell, which stage 5 does NOT cover — `Bidi_Class`
# is declined by design) and `\p{sc=Nosuch}` (a known axis with an unknown
# value, which is the new miss shape the prefix mechanism creates), and their
# gate-open ACCEPT controls are below, since "this now compiles" is the claim
# a retired refusal row leaves unproven.
reject_gated unicode-props '\p{Q}'   "\\p{...}: not a one-letter Unicode property code pcrec recognises (pattern offset 5)"
reject_gated unicode-props '\p{}'    "\\p{...}: not a one-letter Unicode property code pcrec recognises (pattern offset 4)"
reject_gated unicode-props '\pA'     "\\p: not a one-letter Unicode property code pcrec recognises — requires module 'unicode-props' (pattern offset 3)"
reject_gated unicode-props '\p{bc=L}'         "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 8)"
reject_gated unicode-props '\p{Alphabetic}'   "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 14)"
reject_gated unicode-props '\p{Foo}'          "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 7)"
reject_gated unicode-props '\p{sc=Nosuch}'    "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 13)"
reject_gated unicode-props '\p{Assigned}'     "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 12)"
# `Hrkt`/`Katakana_Or_Hiragana` is the ONE `sc` value in the vendored UCD that
# stage 5 declines, and it is declined because MEASURED: all three libpcre2
# versions this project can reach refuse all three of its spellings (error
# 147).  It is the `\p{Assigned}` case in the other direction — a name the UCD
# has and PCRE2 does not — so it must refuse, and as the module's gap rather
# than as an unknown name, since pcrec cannot tell it from a real one.
reject_gated unicode-props '\p{Hrkt}'         "module 'unicode-props' is enabled but \\p{...}: this Unicode property is not implemented yet (pattern offset 8)"
# The MALFORMED tier is gate-INDEPENDENT and stays so: a shape that is not a
# property escape at all is still owed the module, at either gate.
reject_gated unicode-props '\p{L'    "\\p: malformed property escape — requires module 'unicode-props' (pattern offset 4)"
reject_gated unicode-props '\p!'     "\\p: malformed property escape — requires module 'unicode-props' (pattern offset 3)"
# THE RANGE-ENDPOINT RULE, PRODUCED. `[0-\p{L}]` and `[\p{L}-z]` are
# `\p requires module` at the closed gate (pinned above, unmoved) and PCRE2's
# own error 150 once the set is real — measured, libpcre2 gives 150 for both.
reject_gated unicode-props '[0-\p{L}]' "invalid range in character class"
reject_gated unicode-props '[\p{L}-z]' "invalid range in character class"
# [M5.0] STAGE 5's ACCEPT CONTROLS, and they are the failing direction of the
# two rows this stage retired above: four spellings of one script — the bare
# name, both prefixed namespaces, and the four-letter alias — each of which
# refused before this stage and must compile now.  They are gate-OPEN accepts
# (`accept`'s third argument) because the module OFF they still refuse, which
# the closed-gate row above already pins.
accept '\p{Greek}'     '\p{Greek}'     unicode-props
accept '\p{sc=Greek}'  '\p{sc=Greek}'  unicode-props
accept '\p{scx=Greek}' '\p{scx=Greek}' unicode-props
accept '\p{Grek}'      '\p{Grek}'      unicode-props
...
pinned() { # pinned <pattern> <accept|reject> <expected-msg-or-dash> <why it is wrong>
    ...
    nwrong=$((nwrong + 1))
    out="$("$TIMEOUT_BIN" 60 "$PCREC" -p rx -o "$WORKDIR/kw.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$?
...
row_reject() { # like reject(), but counted separately so the floors stay honest
    ...
    case "$mod" in
        classes|modifiers)
            out="$("$TIMEOUT_BIN" 60 "$PCREC" --features none -p rx -o "$WORKDIR/out.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$? ;;
        *)
            out="$("$TIMEOUT_BIN" 60 "$PCREC" -p rx -o "$WORKDIR/out.c" --pattern "$pat" 2>&1 >/dev/null)"; rc=$? ;;
    esac
    if [ "$rc" -ne 1 ]; then
...
"$TIMEOUT_BIN" 120 "$PCREC" --list-syntax > "$WORKDIR/syntax.tsv"   # [K37] bounded; pcrec_run is not sourced here by design (see the TT-6 note above) 2>"$WORKDIR/syntax.err"
if [ ! -s "$WORKDIR/syntax.tsv" ]; then
    ...
    if [ -z "${REJECT_SHARD_TOTAL:-}" ]; then
        ...
        #
        # 127 -> 128 at [VAR] M10: module `vars`' ONE row, the `${name}`
        # doorway. It joins this iteration on the same terms as every other
        # row -- MEASURED at the closed gate, its own `syntax` really does
        # print "requires module 'vars'". It is an `RK_BARE` row and that is
        # the interesting part: `RK_BARE`'s own charter says its rows exist
        # for the DUMP (and for D85's definitions machinery) because the
        # constructs have no doorway, and `--list-syntax` is exactly the
        # surface this loop reads -- so a kind that no parse-path lookup
        # consults is nonetheless probed here, through its `syntax` string,
        # like any other. NOTHING RETIRED.
        if [ "$niter" -eq "$nexpected" ] && [ "$niter" -eq 128 ]; then
