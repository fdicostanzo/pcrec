...
one_pattern() {   # one_pattern <pattern> <engine-args...>
    ...
    # shellcheck disable=SC2086
    if ! pcrec_run "$PCREC" -p pb $eng -fno-length-prune -fno-req-byte --step-budget=$MRLDIFF_STEPS \
            -o "$d/pb.c" --pattern "$pat" >/dev/null 2>"$d/err_b"; then
    ...
    if pcrec_run "$PCREC" -p pc --no-captures --step-budget=$MRLDIFF_STEPS \
            -o "$d/pc.c" --pattern "$pat" >/dev/null 2>"$d/err_c"; then
...
if [ "${1:-}" = "--corpus" ]; then
    ...
    while IFS= read -r cp; do
        [ -n "$cp" ] || continue
        pcrec_run "$PCREC" -p pc --engine=vm -o "$WORKDIR/probe.c" --pattern "$cp" >/dev/null 2>&1 || continue
