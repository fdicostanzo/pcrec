...
# [VAR] 225 -> 226 (2026-09-23), MEASURED the same way: `${name}` nets
# exactly ONE new PASS line ("bare ${name}: refused, as the row claims") --
# check_table_to_parser's RK_BARE arm now emits one assertion per row
# either way (compiles or refused), so the population didn't grow, only
# the row did; every other touched check (family/engine-capability/
# built-status/row-ranks/no-ambiguity-sweep) prints ONE summary line each,
# unconditionally, so none of them move the count on their own.
regn="$(grep -c '^PASS: ' "$REGOUT" || true)"
if [ "$regn" -ne 226 ]; then
    if grep -q "^checks failed: 0" "$REGOUT"; then
        echo "registry: registry_check COVERAGE CHANGED — $regn passing checks, expected 226." >&2
        ...
        echo "registry: registry_check shows $regn passing checks (226 expected; ${rnf:-?} failed," >&2
        echo "registry:   so a lower count is expected here). Fix the failures first; then this" >&2
        echo "registry:   number must return to 226 — if it does not, coverage was removed too" >&2
    fi
    rc=1
fi
if grep -q "^checks failed: 0" "$REGOUT"; then
...
built-status: 139 rows classified with 0 defects|D65 (docs/design/registry_built_status_memo.md, ratified wholesale 2026-08-21): the ONLY guard that pcrec_construct_built_status classifies every RS_MODULE row's own syntax cleanly (built or unbuilt), never landing in the PCREC_BUILT_DEFECT bucket the generated compliance index must never silently render — deleting it removes the one thing that would fail loudly the day a row's own well-formed syntax stops classifying, e.g. a reworded refusal or a doorway change the classifier's res.what/res.answered_at reading no longer matches
REGMANIFEST
fi
...
if [ -s "$PC3OUT" ] && ! grep -q "^SKIP:" "$PC3OUT"; then
    ...
    # 209 -> 210 ([VAR], 2026-09-23): `check_rows` (pcre2_check.c) iterates
    # RK_COUNT and reaches every row including `RK_BARE`, whose own comment
    # says so ("no doorway, no PC-3 sweep of its own, but check_rows
    # iterates RK_COUNT"); `${name}`'s row is a fourth `RK_BARE` member and
    # contributes one more PASS the same generic way its three predecessors
    # already do. MEASURED on this box: 210 passing, 0 failing.
    if [ "$pc3n" -ne 210 ]; then
        ...
        if grep -q "^checks failed: 0" "$PC3OUT"; then
            echo "registry: PC-3 COVERAGE CHANGED — $pc3n passing checks, expected 210." >&2
            echo "registry:   if you added or removed checks on purpose, update this number" >&2
            echo "registry:   in the same commit; if not, coverage was removed" >&2
        else
            nf="$(sed -n 's/^checks failed: //p' "$PC3OUT" | tail -1)"
            echo "registry: PC-3 shows $pc3n passing checks (210 expected; ${nf:-?} failed, so a" >&2
...
# +6 at [EMIT-VERB] (2026-09-19), 102 -> 108: the `comments` axis's TWO rows
# (`essential-only` carries the deny bit 26, `full` the force bit 27) each
# self-register the same triple the K50 entry above describes — the bit
# constant against lib/pcrec.h, the cli_flag the shipped parser must accept,
# and the `(bit N)` heading in tuning.md. THIS PIN IS THE D94 ADDENDUM'S OWN
# SHAPE, found the way that addendum says it is found: it cites no axis, no
...
#
# 24 -> 27 ([LIM-OVR], 2026-09-22): part 4, OVERRIDE HONESTY, added THREE
# new PASS lines — `[override-src]` (the raise_only_limits[] manifest check
# + the bespoke --warn-emit-bytes= site check) and `[override-honesty]`
# (the BUILD_D/FLAG_D cross-check, both directions, folded into one PASS
# line the way `[code]`'s dispositioning already is). THIS PIN WAS MISSED
# AT THAT LANDING TOO — same class as the two misses the paragraph above
# already names, caught here by lane `regred` (2026-09-22): `make test`'s
# registry section exited 1 with every check inside it, including
# limits_check itself, reporting `checks failed: 0` — this guard's own
# message never says `FAIL:`, so a log grepped for that string reads
# entirely green while the section is red. Read from a run (27), not
# predicted, per this guard's own standing rule.
#
# IT IS THE SECOND READER OF THE ROW COUNT AND IT NEVER SPELLS IT. The
# manifest inside limits_check.sh says 58; this guard says 27, because it
...
# a limits.def row with an anchor will miss it exactly the way three lanes
# now have. The same class battriage_report.md named for EMITTED_BYTES
# manifests: a reader whose text never cites the number still moves with it.
#
# 27 -> 29 ([VAR], 2026-09-23): module `vars`' two new numeric limits
# (docs/spec/limits.md §3.6) added two rows to limits.def, each anchored,
# so this guard is the FOURTH reader to miss the shape its own comment
