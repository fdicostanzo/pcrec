#!/usr/bin/env python3
"""studies/tt4m_batchrun/collect_patterns.py — [TT-4M] pattern-pool collection.

Adapted, near-verbatim, from studies/tt4_batching/proto/collect_patterns.py
([TT-4.1], Linux) for this darwin re-open ([TT-4M]) — same job, same
contract, no behavioural change: only the docstring/header is darwin-dated.
See that file's own header for the full rationale (why prefixes are
distinct, why --require-features exists, why a rejected pattern doesn't
count against --count).

Extracts real `pattern <regex>` blocks from .rxt files the same way
tests/harness/run.sh's own parser maps a block to pcrec flags (`flags i` ->
`-i`, `features LIST` -> `--features LIST`; `perr` blocks skipped).
Compiles each surviving block through the WORKTREE's own build/pcrec with a
DISTINCT prefix per pattern (rx0000, rx0001, ...) into an output directory,
stopping once --count patterns have compiled successfully.
