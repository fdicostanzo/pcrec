/* studies/linktest_probe/pcre2_check_linked.c — PROTOTYPE direct-link twin
 * of tests/registry/pcre2_check.c (linktest lane, 2026-09-09). The ENTIRE
 * diff against the original is the #include two lines down
 * (pcre2_abi_linked.h in place of ../fuzz/pcre2_abi.h) plus this header
 * comment — every check function below is byte-identical to the original,
 * verified by `diff` in the lane report. See pcre2_abi_linked.h for why
 * that one swap is sufficient: the twin's Pcre2Abi struct keeps the same
 * field names pcre2_check.c calls (`pcre2.compile`/`.code_free`/
 * `.get_error_message` — the only three, grep-verified) and the same
 * pcre2_abi_load/version/unicode_version/path function names, just backed
 * by direct linkage against libpcre2 instead of dlopen/dlsym.
 *
...
/* THE ONE SUBSTANTIVE LINE OF DIFF: the direct-link adapter in place of the
 * dlopen shim. Same struct/function names — see pcre2_abi_linked.h. */
